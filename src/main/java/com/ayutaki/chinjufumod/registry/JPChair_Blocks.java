...
public class JPChair_Blocks {
	...
	public static final RegistryObject<Block> WARAZABUTON = register("block_wara_zabuton", 
			() -> new Zabuton(BlockBehaviour.Properties.of(Material.WOOD).strength(1.0F, 1.0F).sound(SoundType.GRASS).noOcclusion().isValidSpawn(JPChair_Blocks::neverEntity).isSuffocating(JPChair_Blocks::never)));
	...
	private static Zabuton zabuton() {
		return new Zabuton(BlockBehaviour.Properties.of(Material.WOOL).strength(1.0F, 1.0F).sound(SoundType.WOOL)
				.noOcclusion().isValidSpawn(JPChair_Blocks::neverEntity).isSuffocating(JPChair_Blocks::never));
	}

	private static Zaisu zaisu() {
		return new Zaisu(BlockBehaviour.Properties.of(Material.WOOD).strength(1.0F, 3.0F).sound(SoundType.WOOD)
				.noOcclusion().isValidSpawn(JPChair_Blocks::neverEntity).isSuffocating(JPChair_Blocks::never));
	}

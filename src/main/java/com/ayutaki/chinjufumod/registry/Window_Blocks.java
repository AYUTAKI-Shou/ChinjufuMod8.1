...
public class Window_Blocks {
	...
	/* Share variables */
	private static ToIntFunction<BlockState> litAntiShadow(int value) {
		return (state) -> { return (Config_CM.INSTANCE.antiShadow.get() == true && state.getValue(BaseStage3_FaceWater.STAGE_1_3) == 3) ? value : 0; };
	}
	...
	private static Properties woodState() {
		return BlockBehaviour.Properties.of(Material.WOOD).strength(1.0F, 1.0F).sound(SoundType.WOOD)
	...
	private static Properties curtainState() {
		return BlockBehaviour.Properties.of(Material.WOOD).noCollission().strength(1.0F, 1.0F).sound(SoundType.WOOL)
				.noOcclusion().isValidSpawn(Window_Blocks::neverEntity).isSuffocating(Window_Blocks::never);
	}
	...
	private static WindowTall_Bot windowTallBot() {
		return new WindowTall_Bot(woodState().lightLevel(litAntiShadow(1)));
	}

	private static WindowTall_Top windowTallTop() {
		return new WindowTall_Top(woodState().lightLevel(litAntiShadow(1)));
	}

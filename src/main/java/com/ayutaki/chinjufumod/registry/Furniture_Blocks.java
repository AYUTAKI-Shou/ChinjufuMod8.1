...
public class Furniture_Blocks {
	...
	public static Block LAMP = register("block_lamp", new LampHang(lampState().lightLevel(litBlockEmission(15))));
	public static Block STANDARM = register("block_standarm", new StandArm(lampState().lightLevel(litBlockEmission(15))));
	public static Block STAND = register("block_standbedroom", new StandBedroom(lampState().lightLevel(litBlockEmission(15))));
	public static Block M_LAMP = register("block_marinelamp", new LampMarine(lampState().lightLevel(litBlockEmission(15))));
	public static Block E_LIGHT = register("block_lightembed", new LightEmbed(lampState().lightLevel(lightBlock(15))));
	...
	private static Properties lampState() {
		return AbstractBlock.Properties.of(Material.WOOD).noCollission().strength(1.0F, 2.0F).sound(SoundType.METAL).noOcclusion()
				.isValidSpawn(Furniture_Blocks::neverEntity).isSuffocating(Furniture_Blocks::never);
	}

	private static Candle candle() {
		return new Candle(AbstractBlock.Properties.of(Material.STONE).noCollission().strength(1.0F, 1.0F).sound(SoundType.STONE)
	...
	private static Tansu tansu() {
		return new Tansu(AbstractBlock.Properties.of(Material.WOOD).strength(1.0F, 3.0F).sound(SoundType.WOOD), () -> { return TileEntity_CM.TANSU; });
	}

	private static OfficeDesk officeDesk() {
		return new OfficeDesk(AbstractBlock.Properties.of(Material.WOOD).strength(1.0F, 3.0F).sound(SoundType.WOOD)
				.noOcclusion().isValidSpawn(Furniture_Blocks::neverEntity).isSuffocating(Furniture_Blocks::never), () -> { return TileEntity_CM.OFFICEDESK; });
	}
	
	private static DeskCloth deskCloth() {
		return new DeskCloth(AbstractBlock.Properties.of(Material.WOOD).noCollission().strength(1.0F, 3.0F).sound(SoundType.WOOL)
				.noOcclusion().isValidSpawn(Furniture_Blocks::neverEntity).isSuffocating(Furniture_Blocks::never));
	}
	
	private static Properties bookState() {
		return AbstractBlock.Properties.of(Material.WOOD).noCollission().strength(0.5F).sound(SoundType.WOOD)
				.noOcclusion().isValidSpawn(Furniture_Blocks::neverEntity).isSuffocating(Furniture_Blocks::never);
	}

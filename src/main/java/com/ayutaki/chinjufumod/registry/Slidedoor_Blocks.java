...
public class Slidedoor_Blocks {
	...
	private static ToIntFunction<BlockState> litAntiShadow(int value) {
		return (state) -> { return (Config_CM.INSTANCE.antiShadow.get() == true) ? value : 0; };
	}
	...
	private static Properties woodSlidedoor() {
		return BlockBehaviour.Properties.of(Material.WOOD).strength(1.0F, 3.0F).sound(SoundType.WOOD)
				.noOcclusion().isValidSpawn(Slidedoor_Blocks::neverEntity).isSuffocating(Slidedoor_Blocks::never).lightLevel(litAntiShadow(1));
	}
	
	private static Properties amadoState() {
		return BlockBehaviour.Properties.of(Material.WOOD).strength(2.0F, 5.0F).sound(SoundType.WOOD).noOcclusion()

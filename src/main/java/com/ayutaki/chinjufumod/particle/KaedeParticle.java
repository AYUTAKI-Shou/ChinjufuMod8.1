...
@OnlyIn(Dist.CLIENT)
public class KaedeParticle extends TextureSheetParticle {

	private final float rotSpeed;
	@SuppressWarnings("unused")
	private final SpriteSet sprites;
	
	public KaedeParticle(ClientLevel worldIn, double xIn, double yIn, double zIn, double xSpeed, double ySpeed, double zSpeed, SpriteSet iSprite) {
		super(worldIn, xIn, yIn, zIn);

		this.quadSize *= 1.0F; //<- 0.5
	...
	public static class Provider implements ParticleProvider<SimpleParticleType> {
		...
		public Particle createParticle(SimpleParticleType typeIn, ClientLevel worldIn, double xIn, double yIn, double zIn,
			...
			KaedeParticle particle = new KaedeParticle(worldIn, xIn, yIn, zIn, xSpeed, ySpeed, zSpeed, this.sprite);

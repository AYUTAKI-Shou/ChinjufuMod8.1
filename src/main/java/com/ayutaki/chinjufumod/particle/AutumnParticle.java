...
@OnlyIn(Dist.CLIENT)
public class AutumnParticle extends TextureSheetParticle {

	private final float rotSpeed;
	@SuppressWarnings("unused")
	private final SpriteSet sprites;
	
	public AutumnParticle(ClientLevel worldIn, double xIn, double yIn, double zIn, double xSpeed, double ySpeed, double zSpeed, SpriteSet iSprite) {
		super(worldIn, xIn, yIn, zIn);

		this.quadSize *= 1.125F; //<- 0.75
	...
	public static class Provider implements ParticleProvider<SimpleParticleType> {
		...
		public Particle createParticle(SimpleParticleType typeIn, ClientLevel worldIn, double xIn, double yIn, double zIn,
			...
			AutumnParticle particle = new AutumnParticle(worldIn, xIn, yIn, zIn, xSpeed, ySpeed, zSpeed, this.sprite);

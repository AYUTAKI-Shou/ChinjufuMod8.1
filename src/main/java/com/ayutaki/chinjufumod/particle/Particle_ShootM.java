...
@OnlyIn(Dist.CLIENT)
public class Particle_ShootM extends SpriteTexturedParticle {
	
	private Particle_ShootM(World worldIn, double xCoordIn, double yCoordIn, double zCoordIn, double xSpeed, double ySpeed, double zSpeed) {
	...
	public void tick() {
		...
		if (this.age++ >= this.maxAge) {
			...
			this.particleAlpha -= 0.13F;
			float grow = this.age * 0.15F;
			this.multipleParticleScaleBy(grow + 1.0F);
		}
	}
	...
	public static class Factory implements IParticleFactory<BasicParticleType> {
		...
		public Particle makeParticle(BasicParticleType typeIn, World worldIn, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
			Particle_ShootM AmmoParticle = new Particle_ShootM(worldIn, x, y, z, xSpeed, ySpeed, zSpeed);
			AmmoParticle.selectSpriteRandomly(this.spriteSet);
			AmmoParticle.setAlphaF(0.95F);
			return AmmoParticle;
		}
	}
}

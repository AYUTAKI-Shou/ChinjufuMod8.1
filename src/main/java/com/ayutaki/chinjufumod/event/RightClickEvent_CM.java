	...
	public void editSign(RightClickBlock event) {
		...
		if (player != null) {
			...
			if (state.getBlock() instanceof AbstractSignBlock) {
				...
				boolean mode = player.abilities.instabuild;
				...
				if (tileEntity instanceof SignTileEntity) {
					if (hStack.getItem() instanceof Abstract_Hake) {
						...
						if(!world.isClientSide()) {
							ObfuscationReflectionHelper.setPrivateValue(SignTileEntity.class, sign, true, "field_145916_j"); // <- sign.setEditable(true); accesstransformer.cfg
							player.openTextEdit(sign); 
							
							hStack.hurtAndBreak(mode? 0 : 1, player, user -> {
								ItemStack take = new ItemStack(Items_Wadeco.HAKE, 1);
								if (!player.inventory.add(take)) { player.drop(take, false); }

...
public class FirstAidEvent {
	...
	public static void damageControl(LivingDeathEvent event) {
		...
		if(!event.isCanceled() && event.getEntityLiving() instanceof Player) {
			...
			Player playerIn = (Player) event.getEntityLiving();
			...
			if(!stack.isEmpty()) {
				...
				playerIn.displayClientMessage(new TranslatableComponent("text.chinjufumod.firstaid").withStyle(ChatFormatting.BOLD), true);

...
import com.ayutaki.chinjufumod.items.kansaiki.Abstract_BowKansaiki;
...
public class CMScopeEvent {
	...
	public static void updateFOV(ComputeFovModifierEvent event) {
		...
		boolean useItem = (mClient.options.getCameraType().isFirstPerson() && player.isUsingItem());
		
		if (useItem && (main instanceof SouganKyou || off instanceof SouganKyou)) { event.setNewFovModifier(event.getFovModifier() / 10); }
		
		if (useItem && (main instanceof Base_RensouHou || main instanceof Triple_Kijyuu || main instanceof Abstract_BowKansaiki
				|| off instanceof Base_RensouHou || off instanceof Triple_Kijyuu || off instanceof Abstract_BowKansaiki)) {
	...
	private static final ResourceLocation LENS = ResourceLocation.fromNamespaceAndPath(ChinjufuMod.MOD_ID, "textures/gui/binoculars_68l.png");
	private static final ResourceLocation FRAME = ResourceLocation.fromNamespaceAndPath(ChinjufuMod.MOD_ID, "textures/gui/binoculars_68f.png");
	
	@SubscribeEvent
	public static void textureOverlay(CustomizeGuiOverlayEvent.Chat event) { //must .Chat
		Minecraft mClient = Minecraft.getInstance();
	...
	private static void lensOverlay(GuiGraphics gui, float size) {
		...
		gui.blit(RenderType::guiTextured, LENS, k, l, 0, 0.0F, i, j, i, j); //-90, 0.0F
	}
	...
	private static void renderBack(GuiGraphics gui, float size) {
		...
		gui.blit(RenderType::guiTextured, FRAME, k, l, 0, 0.0F, i, j, i, j);

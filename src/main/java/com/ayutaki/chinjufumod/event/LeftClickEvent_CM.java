...
public class LeftClickEvent_CM {
	...
	public void cleanEraser(LeftClickEmpty event) {
		...
		if (player != null) {
			...
			int offLife = offStack.getDamage();
			...
			if (offStack.getItem() instanceof Board_Eraser && offLife >= 1 && mainStack.getItem() == Items.STICK) {
				...
				Vec3d pos = player.getEyePosition(1.0F).add(0.0D, -0.15D, 0.0D).subtract(player.getLookVec().mul(-0.75D, -0.75D, -0.75D));
				...
				if (!life11) { ChinjufuMod.CHANNEL.sendToServer(new Server_GenEraser()); }
			}
		}
	}
}

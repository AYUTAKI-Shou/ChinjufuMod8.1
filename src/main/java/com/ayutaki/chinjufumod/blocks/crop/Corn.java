...
public class Corn extends CropsBlock implements IGrowable {
	...
	public Corn(AbstractBlock.Properties props) {
		super(props);
	...
	public BlockState updateShape(BlockState stateIn, Direction facing, BlockState newState, IWorld worldIn, BlockPos pos, BlockPos newPos) {
		...
		if (facing.getAxis() == Direction.Axis.Y && half == DoubleBlockHalf.LOWER == (facing == Direction.UP)) {
			return (newState.getBlock() == Crop_Blocks.CORN) &&
					newState.getValue(HALF) != half ? stateIn.setValue(AGE_0_7, newState.getValue(AGE_0_7)) : Blocks.AIR.defaultBlockState();
		}
		else {
			return half == DoubleBlockHalf.LOWER && facing == Direction.DOWN && !stateIn.canSurvive(worldIn, pos) ? Blocks.AIR
					.defaultBlockState() : super.updateShape(stateIn, facing, newState, worldIn, pos, newPos);
		}
	}

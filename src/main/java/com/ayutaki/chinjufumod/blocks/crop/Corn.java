...
public class Corn extends CropsBlock implements IGrowable {
	...
	/* Collision */
	private static final VoxelShape AABB_BOT_0 = Block.makeCuboidShape(1.0D, -1.0D, 1.0D, 15.0D, 1.0D, 15.0D);
	private static final VoxelShape AABB_BOT_1 = Block.makeCuboidShape(2.0D, -1.0D, 2.0D, 14.0D, 1.0D, 14.0D);
	private static final VoxelShape AABB_BOT_2 = Block.makeCuboidShape(2.0D, -1.0D, 2.0D, 14.0D, 7.0D, 14.0D);
	private static final VoxelShape AABB_BOT_3 = Block.makeCuboidShape(2.0D, -1.0D, 2.0D, 14.0D, 11.0D, 14.0D);
	private static final VoxelShape AABB_BOT_4 = Block.makeCuboidShape(2.0D, -1.0D, 2.0D, 14.0D, 18.0D, 14.0D);
	private static final VoxelShape AABB_BOT_5 = Block.makeCuboidShape(2.0D, 0.0D, 2.0D, 14.0D, 24.0D, 14.0D);
	private static final VoxelShape AABB_BOT_6 = Block.makeCuboidShape(2.0D, 0.0D, 2.0D, 14.0D, 28.0D, 14.0D);

	private static final VoxelShape AABB_BOT_7 = Block.makeCuboidShape(2.0D, 0.0D, 2.0D, 14.0D, 16.0D, 14.0D);
	private static final VoxelShape AABB_TOP_7 = Block.makeCuboidShape(2.0D, 0.0D, 2.0D, 14.0D, 16.0D, 14.0D);
	
	public Corn(Block.Properties props) {
		super(props);
	...
	public BlockState updatePostPlacement(BlockState stateIn, Direction facing, BlockState newState, IWorld worldIn, BlockPos pos, BlockPos newPos) {
		...
		if (facing.getAxis() == Direction.Axis.Y && half == DoubleBlockHalf.LOWER == (facing == Direction.UP)) {
			return (newState.getBlock() == Crop_Blocks.CORN) &&
					newState.get(HALF) != half ? stateIn.with(AGE_0_7, newState.get(AGE_0_7)) : Blocks.AIR.getDefaultState();
		}
		else {
			return half == DoubleBlockHalf.LOWER && facing == Direction.DOWN && !stateIn.isValidPosition(worldIn, pos) ? Blocks.AIR
					.getDefaultState() : super.updatePostPlacement(stateIn, facing, newState, worldIn, pos, newPos);
		}
	}

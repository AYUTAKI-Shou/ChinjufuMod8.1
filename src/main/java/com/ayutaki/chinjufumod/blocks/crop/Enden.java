...
public class Enden extends Abstract_Harvest {
	...
	private static final VoxelShape AABB_1 = Block.makeCuboidShape(0.0D, 0.0D, 0.0D, 16.0D, 14.5D, 16.0D);
	private static final VoxelShape AABB_9 = Block.makeCuboidShape(0.0D, 0.0D, 0.0D, 16.0D, 16.0D, 16.0D);

	public Enden(Block.Properties props) {
		super(props);
		/** Default state **/
	...
	public void randomTick(BlockState state, ServerWorld worldIn, BlockPos pos, Random rand) {
		...
		if (worldIn.canSeeSky(pos.up())) {
			...
			if (worldIn.isRainingAt(pos.up())) {
				...
				/** RawBrightness is >= 14. **/
				boolean canDRY = worldIn.isDaytime() && worldIn.getLightSubtracted(pos, 0) >= 14 && i != 9;
				if (canDRY) {
					if (rand.nextInt(6) == 0) { 
						worldIn.setBlockState(pos, state.with(WET_1_9, Integer.valueOf(i + 1)), 3); } }
	...
	public void addInformation(ItemStack stack, @Nullable IBlockReader worldIn, List<ITextComponent> blockTip, ITooltipFlag tipFlag) {
		blockTip.add(new TranslationTextComponent("tips.block_enden").applyTextStyle(TextFormatting.GRAY));
	}
}

...
public class SeedsBox extends BaseFacingWater {
	/* Collision */
	private static final VoxelShape AABB_SOUTH = Block.makeCuboidShape(4.0D, 0.0D, 6.0D, 12.0D, 8.0D, 10.0D);
	private static final VoxelShape AABB_WEST = Block.makeCuboidShape(6.0D, 0.0D, 4.0D, 10.0D, 8.0D, 12.0D);
	private static final VoxelShape AABB_NORTH = Block.makeCuboidShape(4.0D, 0.0D, 6.0D, 12.0D, 8.0D, 10.0D);
	private static final VoxelShape AABB_EAST = Block.makeCuboidShape(6.0D, 0.0D, 4.0D, 10.0D, 8.0D, 12.0D);

	public SeedsBox(Block.Properties props) {
		super(props);
	}
	...
	public void addInformation(ItemStack stack, @Nullable IBlockReader worldIn, List<ITextComponent> blockTip, ITooltipFlag tipFlag) {
		blockTip.add(new TranslationTextComponent("tips.block_seedsbox").applyTextStyle(TextFormatting.GRAY));
	}
}
/* サクランボの種は袋に入れない */

...
public class SeedsBox extends BaseFacingWater {
	/* Collision */
	private static final VoxelShape AABB_SOUTH = Block.box(4.0D, 0.0D, 6.0D, 12.0D, 8.0D, 10.0D);
	private static final VoxelShape AABB_WEST = Block.box(6.0D, 0.0D, 4.0D, 10.0D, 8.0D, 12.0D);
	private static final VoxelShape AABB_NORTH = Block.box(4.0D, 0.0D, 6.0D, 12.0D, 8.0D, 10.0D);
	private static final VoxelShape AABB_EAST = Block.box(6.0D, 0.0D, 4.0D, 10.0D, 8.0D, 12.0D);

	public SeedsBox(AbstractBlock.Properties props) {
		super(props);
	}
	...
	public void appendHoverText(ItemStack stack, @Nullable IBlockReader worldIn, List<ITextComponent> blockTip, ITooltipFlag tipFlag) {
		blockTip.add(new TranslationTextComponent("tips.block_seedsbox").withStyle(TextFormatting.GRAY));
	}
}

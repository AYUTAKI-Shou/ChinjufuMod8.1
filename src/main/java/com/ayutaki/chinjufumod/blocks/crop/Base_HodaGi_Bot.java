...
public class Base_HodaGi_Bot extends BaseStage4_FaceWater {
	/* Collision */
	private static final VoxelShape AABB_SOUTH = Block.makeCuboidShape(5.0D, 0.0D, 4.0D, 11.0D, 12.0D, 15.0D);
	private static final VoxelShape AABB_WEST = Block.makeCuboidShape(1.0D, 0.0D, 5.0D, 12.0D, 12.0D, 11.0D);
	private static final VoxelShape AABB_NORTH = Block.makeCuboidShape(5.0D, 0.0D, 1.0D, 11.0D, 12.0D, 12.0D);
	private static final VoxelShape AABB_EAST = Block.makeCuboidShape(4.0D, 0.0D, 5.0D, 15.0D, 12.0D, 11.0D);

	public Base_HodaGi_Bot(Block.Properties props) {
		super(props);
	}
	...
	public void addInformation(ItemStack stack, @Nullable IBlockReader worldIn, List<ITextComponent> blockTip, ITooltipFlag tipFlag) {
		blockTip.add(new TranslationTextComponent("tips.block_hodagi_a_bot").applyTextStyle(TextFormatting.GRAY));
	}
}

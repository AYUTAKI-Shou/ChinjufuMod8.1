...
public class Rice_8 extends Abstract_Harvest implements IGrowable {
	/* Property */
	public static final IntegerProperty AGE = IntegerProperty.create("age", 0, 8);

	/* Collision */
	private static final VoxelShape[] SHAPES = new VoxelShape[]{ Block.makeCuboidShape(0.0D, -3.0D, 0.0D, 16.0D, 1.0D, 16.0D),
	...
	public Rice_8(Block.Properties props) {
		super(props);
	...
	public BlockState updatePostPlacement(BlockState stateIn, Direction facing, BlockState newState, IWorld worldIn, BlockPos pos, BlockPos newPos) {
		...
		if (i < 8 && worldIn.getLightSubtracted(pos, 0) >= 9) {
			worldIn.getPendingBlockTicks().scheduleTick(pos, Crop_Blocks.RICE_8, 4500 + (500 * worldIn.getRandom().nextInt(5)));
		}

		return !stateIn.isValidPosition(worldIn, pos) ? Blocks.AIR.getDefaultState() : super.updatePostPlacement(stateIn, facing, newState, worldIn, pos, newPos);
	}

...
public class Rice_8 extends Abstract_Harvest implements IGrowable {
	...
	public Rice_8(AbstractBlock.Properties props) {
		super(props);
	...
	public BlockState updateShape(BlockState stateIn, Direction facing, BlockState newState, IWorld worldIn, BlockPos pos, BlockPos newPos) {
		...
		if (i < 8 && worldIn.getRawBrightness(pos, 0) >= 9) {
			worldIn.getBlockTicks().scheduleTick(pos, Crop_Blocks.RICE_8, 4500 + (500 * worldIn.getRandom().nextInt(5))); }

		return !stateIn.canSurvive(worldIn, pos) ? Blocks.AIR.defaultBlockState() : super.updateShape(stateIn, facing, newState, worldIn, pos, newPos);
	}

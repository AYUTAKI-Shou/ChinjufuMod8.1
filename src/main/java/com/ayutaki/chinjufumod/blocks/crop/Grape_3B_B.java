...
public class Grape_3B_B extends BaseStage4_Face {
	...
	public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
		...
		if (i != 4 && worldIn.getLightFromNeighbors(pos.up()) >= 9) {
			if (rand.nextInt(8) == 0) {
				int i1 = i + 1;
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		if (i == 4) {
			if (hStack.isEmpty()) {
				CMEvents.emptyTakeN_SnowB(worldIn, pos, playerIn, Items_Teatime.FOOD_GRAPE, 2, 0);
	...
	public List<ItemStack> getDrops(IBlockAccess worldIn, BlockPos pos, IBlockState state, int fortune) {
		...
		stack.add(new ItemStack(cloneItem(), (i == 4)? 3 : 1, 0));
		return stack;
	}

...
public class HodaGi_A_Bot extends Base_HodaGi_Bot {
	...
	public HodaGi_A_Bot(String name) {
		super(name);
	}
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		if (i == 1) { CMEvents.textEarlyCollect(worldIn, pos, playerIn); }
			...
			if (hStack.isEmpty() && hitY < 0.75F) {
				CMEvents.take_KINOKO(worldIn, pos, playerIn);
				worldIn.setBlockState(pos, Crop_Blocks.HODAGI_B_BOT.getDefaultState()
						.withProperty(Base_HodaGi_Bot.H_FACING, state.getValue(H_FACING))
						.withProperty(Base_HodaGi_Bot.STAGE_1_4, Integer.valueOf(i - 1))); }

...
public class Turmeric extends CropsBlock {
	/* Collision */
	private static final VoxelShape[] SHAPES = new VoxelShape[]{ Block.makeCuboidShape(4.0D, -1.0D, 4.0D, 12.0D, 1.0D, 12.0D),
			Block.makeCuboidShape(4.0D, -1.0D, 4.0D, 12.0D, 2.0D, 12.0D),
			...
			Block.makeCuboidShape(4.0D, -1.0D, 4.0D, 12.0D, 11.0D, 12.0D),
			Block.makeCuboidShape(4.0D, -1.0D, 4.0D, 12.0D, 11.0D, 12.0D),
			Block.makeCuboidShape(4.0D, -1.0D, 4.0D, 12.0D, 11.0D, 12.0D) };

	public Turmeric(Block.Properties props) {
		super(props);
	}
	...
	public void addInformation(ItemStack stack, @Nullable IBlockReader worldIn, List<ITextComponent> blockTip, ITooltipFlag tipFlag) {
		blockTip.add(new TranslationTextComponent("tips.item_seeds_turmeric").applyTextStyle(TextFormatting.GRAY));
		blockTip.add(new TranslationTextComponent("tips.item_crop_pepperdry").applyTextStyle(TextFormatting.GRAY));
	}
}

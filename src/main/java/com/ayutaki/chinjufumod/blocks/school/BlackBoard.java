...
public class BlackBoard extends AbstractBlackBoard {
	/* Collision */
	private static final VoxelShape AABB_SOUTH = Block.makeCuboidShape(0.0D, 0.0D, 0.0D, 16.0D, 16.0D, 1.5D);
	private static final VoxelShape AABB_WEST = Block.makeCuboidShape(14.5D, 0.0D, 0.0D, 16.0D, 16.0D, 16.0D);
	private static final VoxelShape AABB_NORTH = Block.makeCuboidShape(0.0D, 0.0D, 14.5D, 16.0D, 16.0D, 16.0D);
	private static final VoxelShape AABB_EAST = Block.makeCuboidShape(0.0D, 0.0D, 0.0D, 1.5D, 16.0D, 16.0D);

	public BlackBoard(Block.Properties props) {
		super(props);
	}
				...
				if(board.isWaxed()) {
					...
					if (hItem instanceof Base_Chalk) {
						...
						if(!world.isRemote) {
							...
							ChinjufuMod.CHANNEL.send(PacketDistributor.ALL.noArg(), new OpenUI_BlackBoard(playerIn.getUniqueID(), board.getPos()));

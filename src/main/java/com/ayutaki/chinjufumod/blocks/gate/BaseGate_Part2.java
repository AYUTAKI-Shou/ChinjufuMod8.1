...
public abstract class BaseGate_Part2 extends Abstract_Gate {
	
	public BaseGate_Part2(String name) {
		super(name);
	}
			...
			if (upState.getBlock() == this) {
				...
				if (blockIn != this && (flag || blockIn.getDefaultState().canProvidePower()) && flag != ((Boolean)upState.getValue(POWERED)).booleanValue()) {
					...
					if (flag != ((Boolean)state.getValue(OPEN)).booleanValue()) {
						...
						case LEFT:
						default:
							worldIn.setBlockState(pos, Low_LEFT2, 2);
							
							switch (direction) {
							case NORTH :
							default :
								worldIn.setBlockState(new BlockPos(x + 1, y + 1, z), Up_LEFT1, 2);
								worldIn.setBlockState(new BlockPos(x + 1, y, z), Low_LEFT1, 2);
								break;
							...
							case EAST :
								worldIn.setBlockState(new BlockPos(x, y + 1, z + 1), Up_LEFT1, 2);
								worldIn.setBlockState(new BlockPos(x, y, z + 1), Low_LEFT1, 2);
								break;
						...
						case RIGHT:
							worldIn.setBlockState(pos, Low_RIGHT2, 2);
							
							switch (direction) {
							case NORTH :
							default :
								worldIn.setBlockState(new BlockPos(x - 1, y + 1, z), Up_RIGHT1, 2);
								worldIn.setBlockState(new BlockPos(x - 1, y, z), Low_RIGHT1, 2);
								break;
							...
							case EAST :
								worldIn.setBlockState(new BlockPos(x, y + 1, z - 1), Up_RIGHT1, 2);
								worldIn.setBlockState(new BlockPos(x, y, z - 1), Low_RIGHT1, 2);
								break;
	...
	private Block takeBlock1() {
		if (this == Gate_Blocks.GATE_SPRUCE2) { return Gate_Blocks.GATE_SPRUCE; }
		if (this == Gate_Blocks.GATE_SPRUCE_B2) { return Gate_Blocks.GATE_SPRUCE_B; }
		if (this == Gate_Blocks.GATE_IRON2) { return Gate_Blocks.GATE_IRON; }
	...
	public void onBlockHarvested(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn) {
		...
		if (half != HalfState.LOWER && downState.getBlock() == this) { 

...
public class BambooPressurePlate extends BlockPressurePlate {
	...
	public IBlockState getStateForPlacement(World worldIn, BlockPos pos, EnumFacing facing,
		...
		return this.getDefaultState().withProperty(H_FACING, placer.getHorizontalFacing().getOpposite())
				.withProperty(POWERED, Boolean.valueOf(i == 1));
	}
	...
	public List<ItemStack> getDrops(IBlockAccess worldIn, BlockPos pos, IBlockState state, int fortune) {
		...
		stack.add(new ItemStack(Items_Wadeco.TAKE_PLATE, 1, cloneMeta()));
		return stack;
	}

	@Override
	public ItemStack getPickBlock(IBlockState state, RayTraceResult target, World worldIn, BlockPos pos, EntityPlayer playerIn) {
		return new ItemStack(Items_Wadeco.TAKE_PLATE, 1, cloneMeta());
	}

	private int cloneMeta() {
		if (this == JPDeco_Blocks.TAKE_PLATE) { return 0; }
		if (this == JPDeco_Blocks.TAKE_PLATE_Y) { return 1; }
		else { return 2; }
	}
}

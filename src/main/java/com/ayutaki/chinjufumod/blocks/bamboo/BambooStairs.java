...
public class BambooStairs extends Regi_addState {
	...
	private static final AxisAlignedBB AABB_TOP_WEST = new AxisAlignedBB(0.5D, 0.5D, 0.0D, 1.0D, 1.0D, 1.0D);
	private static final AxisAlignedBB AABB_TOP_EAST = new AxisAlignedBB(0.0D, 0.5D, 0.0D, 0.5D, 1.0D, 1.0D);
	private static final AxisAlignedBB AABB_TOP_NORTH = new AxisAlignedBB(0.0D, 0.5D, 0.5D, 1.0D, 1.0D, 1.0D);
	private static final AxisAlignedBB AABB_TOP_SOUTH = new AxisAlignedBB(0.0D, 0.5D, 0.0D, 1.0D, 1.0D, 0.5D);

	private static final AxisAlignedBB AABB_SLAB_BOTTOM = new AxisAlignedBB(0.0D, 0.0D, 0.0D, 1.0D, 0.5D, 1.0D);
	private static final AxisAlignedBB AABB_BOT_WEST = new AxisAlignedBB(0.5D, 0.0D, 0.0D, 1.0D, 0.5D, 1.0D);
	private static final AxisAlignedBB AABB_BOT_EAST = new AxisAlignedBB(0.0D, 0.0D, 0.0D, 0.5D, 0.5D, 1.0D);
	private static final AxisAlignedBB AABB_BOT_NORTH = new AxisAlignedBB(0.0D, 0.0D, 0.5D, 1.0D, 0.5D, 1.0D);
	private static final AxisAlignedBB AABB_BOT_SOUTH = new AxisAlignedBB(0.0D, 0.0D, 0.0D, 1.0D, 0.5D, 0.5D);
	...
	public void addCollisionBoxToList(IBlockState state, World worldIn, BlockPos pos, AxisAlignedBB entityBox,
		...
		if (!(entityIn instanceof SitableEntity)) {
			...
			default:
				...
				else { super.addCollisionBoxToList(pos, entityBox, collidingBoxes, AABB_BOT_NORTH); }
				break;
				
			case SOUTH:
				if (half == TypeTB.BOTTOM) { super.addCollisionBoxToList(pos, entityBox, collidingBoxes, AABB_TOP_SOUTH); }
				else { super.addCollisionBoxToList(pos, entityBox, collidingBoxes, AABB_BOT_SOUTH); }
				break;
				
			case EAST:
				if (half == TypeTB.BOTTOM) { super.addCollisionBoxToList(pos, entityBox, collidingBoxes, AABB_TOP_EAST); }
				else { super.addCollisionBoxToList(pos, entityBox, collidingBoxes, AABB_BOT_EAST); }
				break;
				
			case WEST:
				if (half == TypeTB.BOTTOM) { super.addCollisionBoxToList(pos, entityBox, collidingBoxes, AABB_TOP_WEST); }
				else { super.addCollisionBoxToList(pos, entityBox, collidingBoxes, AABB_BOT_WEST); }
				break;
			}
			if (half == TypeTB.BOTTOM) { super.addCollisionBoxToList(pos, entityBox, collidingBoxes, AABB_SLAB_BOTTOM); }
			else { super.addCollisionBoxToList(pos, entityBox, collidingBoxes, AABB_SLAB_TOP); }
		}
	}
	...
	private Item cloneItem() {
		if (this == JPDeco_Blocks.TAKE_ST) { return Items_Wadeco.TAKE_ST; }
		if (this == JPDeco_Blocks.TAKE_STY) { return Items_Wadeco.TAKE_STY; }
		else { return Items_Wadeco.TAKE_STK; }
	}
}

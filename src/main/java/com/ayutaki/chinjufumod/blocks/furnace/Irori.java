...
import com.ayutaki.chinjufumod.blocks.dish.NabeNama_SNTA;
import com.ayutaki.chinjufumod.blocks.dish.NabeNama_TMGC;
...
public class Irori extends Regi_addState {
	/* Property 0=Empty, 1=Charcoal, 2=Ash */
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		/** No usable items. **/
		if ((hItem != Items.COAL || k != 1) && hItem != Items_Teatime.KUSHI_SAKANA && hItem != Items_Teatime.Item_MATCH && hItem != Items.FLINT_AND_STEEL) {
		
			if (hStack.isEmpty()) {
				if (i == 2) {
					CMEvents.emptyTake1_SnowB(worldIn, pos, playerIn, Items.DYE, 15);
	...
	public boolean canConnectTo(IBlockAccess worldIn, BlockPos pos) {
		return worldIn.getBlockState(pos).getBlock() instanceof Nabe_kara || worldIn
				.getBlockState(pos).getBlock() instanceof NabeNama_TMGC || worldIn
				.getBlockState(pos).getBlock() instanceof NabeNama_SNTA || worldIn
	...
	public List<ItemStack> getDrops(IBlockAccess worldIn, BlockPos pos, IBlockState state, int fortune) {
		...
		if (i == 1) { stack.add(new ItemStack(Items.COAL, 1, 1)); }
		if (i == 2) { stack.add(new ItemStack(Items.DYE, 1, 15)); }
		return stack;
	}
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> blockTip, ITooltipFlag advanced) {
		blockTip.add(I18n.format("tips.block_irori.name"));
	}
}

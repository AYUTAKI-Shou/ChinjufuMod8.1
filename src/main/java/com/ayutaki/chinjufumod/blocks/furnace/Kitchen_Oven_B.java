...
public class Kitchen_Oven_B extends BlockContainer {
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		default:
			if ((worldIn.getBlockState(pos.north()).getMaterial().isReplaceable() || worldIn.getBlockState(pos.north()).getBlock() instanceof BlockCarpet) && 
					...
					if (tileentity instanceof KitOvenB_TileEntity) {
						CMEvents.soundOpenOven(worldIn, pos);
						playerIn.displayGUIChest((KitOvenB_TileEntity)tileentity);
	...
	public static void setState(boolean active, World worldIn, BlockPos pos) {
		...
		if (active) {
			worldIn.setBlockState(pos, Kitchen_Blocks.LIT_KITOVEN_B.getDefaultState().withProperty(H_FACING, state.getValue(H_FACING)), 3);
			worldIn.setBlockState(pos, Kitchen_Blocks.LIT_KITOVEN_B.getDefaultState().withProperty(H_FACING, state.getValue(H_FACING)), 3); }
		
		else {
			worldIn.setBlockState(pos, Kitchen_Blocks.KIT_OVEN_B.getDefaultState().withProperty(H_FACING, state.getValue(H_FACING)), 3);
			worldIn.setBlockState(pos, Kitchen_Blocks.KIT_OVEN_B.getDefaultState().withProperty(H_FACING, state.getValue(H_FACING)), 3); }
	...
	public TileEntity createNewTileEntity(World worldIn, int meta) {
		return new KitOvenB_TileEntity();
	}
	...
	public void onBlockPlacedBy(World worldIn, BlockPos pos, IBlockState state, EntityLivingBase placer, ItemStack stack) {
		...
		if (stack.hasDisplayName()) {
			...
			if (tileentity instanceof KitOvenB_TileEntity) {
				((KitOvenB_TileEntity)tileentity).setCustomInventoryName(stack.getDisplayName());
			}
		}
	}
	...
	public void breakBlock(World worldIn, BlockPos pos, IBlockState state) {
		if (!keepInventory) {
			...
			if (tileentity instanceof KitOvenB_TileEntity) {
				InventoryHelper.dropInventoryItems(worldIn, pos, (KitOvenB_TileEntity)tileentity);
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> blockTip, ITooltipFlag advanced) {
		blockTip.add(I18n.format("tips.block_kit_oven.name"));
	}
}

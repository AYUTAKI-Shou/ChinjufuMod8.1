...
public class TobukuroWindowL extends BaseStage3_Face {
	...
	public TobukuroWindowL(String name) {
		super(name);
		setHardness(2.0F);
		setResistance(5.0F);
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		if (i == 3) { CMEvents.textIsEmpty(worldIn, pos, playerIn); }
			...
			IBlockState AMADO_FACE_2 = this.takeBlock().getDefaultState().withProperty(H_FACING, state.getValue(H_FACING))
	...
	private Block takeBlock() {
		if (this == Slidedoor_Blocks.TOBUKURO_WINL) { return Slidedoor_Blocks.AMADO_WIN; }
	...
	private Item cloneItem() {
		if (this == Slidedoor_Blocks.TOBUKURO_WINL) { return Items_Wadeco.TOBUKURO_WINR; }

...
public class Amado extends Abstract_WaterLogged {
	...
	/* Collision */
	private static final VoxelShape AABB_SOUTH = Block.makeCuboidShape(0.0D, 0.0D, -0.5D, 16.0D, 16.0D, 0.5D);
	private static final VoxelShape AABB_WEST = Block.makeCuboidShape(15.5D, 0.0D, 0.0D, 16.5D, 16.0D, 16.0D);
	private static final VoxelShape AABB_NORTH = Block.makeCuboidShape(0.0D, 0.0D, 15.5D, 16.0D, 16.0D, 16.5D);
	private static final VoxelShape AABB_EAST = Block.makeCuboidShape(-0.5D, 0.0D, 0.0D, 0.5D, 16.0D, 16.0D);

	public Amado(Block.Properties props) {
		super(props);
		/** Default state **/
	...
	private void putAIR_UP(World worldIn, BlockPos pos) {
	...
	private Block takeBlock() {
		...
		else { return Slidedoor_Blocks.TOBUKURO_S; }
	}

...
public class Tobukuro_BotR2 extends BaseStage2_Face {
	...
	/** 1=1 sheet, 2=Zero **/
	public Tobukuro_BotR2(String name) {
		super(name);
		setHardness(2.0F);
		setResistance(5.0F);
		...
		if (i == 2) { CMEvents.textIsEmpty(worldIn, pos, playerIn); }
			...
			default :
				if (westState.getMaterial().isReplaceable()) {
					worldIn.setBlockState(new BlockPos(x - 1, y, z), AMADO_FACELO_1);
					worldIn.setBlockState(new BlockPos(x - 1, y + 1, z), AMADO_FACEUP_1);
			...
			case EAST :
				if (northState.getMaterial().isReplaceable()) {
					worldIn.setBlockState(new BlockPos(x, y, z - 1), AMADO_FACELO_1);
					worldIn.setBlockState(new BlockPos(x, y + 1, z - 1), AMADO_FACEUP_1);
	...
	private void updateTOBUKURO_UP(IBlockState state, World worldIn, BlockPos pos) {
		...
		worldIn.setBlockState(pos.up(), tobuTopR2_FACE.withProperty(STAGE_1_2, Integer.valueOf(i + 1)));
	}
	
	private Block takeRightTop2() {
		if (this == Slidedoor_Blocks.TOBUKURO_BOTR2) { return Slidedoor_Blocks.TOBUKURO_TOPR2; }
		else { return Slidedoor_Blocks.TOBUKUROS_TOPR2; }
	}
	
	private Block takeTop() {
		if (this == Slidedoor_Blocks.TOBUKURO_BOTR2) { return Slidedoor_Blocks.AMADO_TOP; }
		else { return Slidedoor_Blocks.AMADOS_TOP; }
	}

	private Block takeBot() {
		if (this == Slidedoor_Blocks.TOBUKURO_BOTR2) { return Slidedoor_Blocks.AMADO_BOT; }
	...
	public void onBlockHarvested(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn) {
		if (worldIn.getBlockState(pos.up()).getBlock() instanceof Tobukuro_TopR2) {
	...
	private Item cloneItem() {
		if (this == Slidedoor_Blocks.TOBUKURO_BOTR2) { return Items_Wadeco.TOBUKURO_BOTR; }

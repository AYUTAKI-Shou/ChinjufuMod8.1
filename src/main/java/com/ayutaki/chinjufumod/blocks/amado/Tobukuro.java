...
public class Tobukuro extends Base_Tobukuro {
	...
	public Tobukuro(Block.Properties props) {
		super(props);
	}
	...
	public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity playerIn, Hand hand, BlockRayTraceResult hit) {
		...
		BlockState AMADO_FACEUP_1 = this.takeBlock().getDefaultState().with(Amado.H_FACING, state.get(H_FACING))
				.with(Amado.HALF, DoubleBlockHalf.UPPER).with(Amado.STAGE_1_4, Integer.valueOf(1));
		BlockState AMADO_FACELO_1 = this.takeBlock().getDefaultState().with(Amado.H_FACING, state.get(H_FACING))
	...
	public BlockState getStateForPlacement(BlockItemUseContext context) {
		...
		if (pos.getY() < 255 && worldIn.getBlockState(pos.up()).isReplaceable(context)) {
			if (playerIn.isSneaking()) {
				return this.takeLBlock().getDefaultState().with(H_FACING, context.getPlacementHorizontalFacing().getOpposite())

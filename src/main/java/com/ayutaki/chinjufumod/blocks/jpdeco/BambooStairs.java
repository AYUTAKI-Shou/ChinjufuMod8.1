...
public class BambooStairs extends Abstract_WaterLogged {
	...
	/* Collision */
	private static final VoxelShape BOT_BASE = Block.makeCuboidShape(0.0D, 0.0D, 0.0D, 16.0D, 8.0D, 16.0D);
	private static final VoxelShape BOT_SOUTH = VoxelShapes.or(BOT_BASE, Block.makeCuboidShape(0.0D, 8.0D, 0.0D, 16.0D, 16.0D, 8.0D));
	private static final VoxelShape BOT_WEST = VoxelShapes.or(BOT_BASE, Block.makeCuboidShape(8.0D, 8.0D, 0.0D, 16.0D, 16.0D, 16.0D));
	private static final VoxelShape BOT_NORTH = VoxelShapes.or(BOT_BASE, Block.makeCuboidShape(0.0D, 8.0D, 8.0D, 16.0D, 16.0D, 16.0D));
	private static final VoxelShape BOT_EAST = VoxelShapes.or(BOT_BASE, Block.makeCuboidShape(0.0D, 8.0D, 0.0D, 8.0D, 16.0D, 16.0D));
	
	private static final VoxelShape TOP_BASE = Block.makeCuboidShape(0.0D, 8.0D, 0.0D, 16.0D, 16.0D, 16.0D);
	private static final VoxelShape TOP_SOUTH = VoxelShapes.or(TOP_BASE, Block.makeCuboidShape(0.0D, 0.0D, 0.0D, 16.0D, 8.0D, 8.0D));
	private static final VoxelShape TOP_WEST = VoxelShapes.or(TOP_BASE, Block.makeCuboidShape(8.0D, 0.0D, 0.0D, 16.0D, 8.0D, 16.0D));
	private static final VoxelShape TOP_NORTH = VoxelShapes.or(TOP_BASE, Block.makeCuboidShape(0.0D, 0.0D, 8.0D, 16.0D, 8.0D, 16.0D));
	private static final VoxelShape TOP_EAST = VoxelShapes.or(TOP_BASE, Block.makeCuboidShape(0.0D, 0.0D, 0.0D, 8.0D, 8.0D, 16.0D));

	private final Block modelBlock;
	private final BlockState modelState;

	public BambooStairs(BlockState state, Block.Properties props) {
		super(props);
		/** Default state **/
	...
	public BlockState getStateForPlacement(BlockItemUseContext context) {
		...
		return hitFace != Direction.DOWN && (hitFace == Direction.UP || context.getHitVec().y - (double)pos.getY() <= 0.5D) ? blockState2 : blockState2.with(TYPE, Half.TOP);
	}
	...
	public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity playerIn, Hand hand, BlockRayTraceResult hit) {
		return this.modelState.onBlockActivated(worldIn, playerIn, hand, hit);
	}

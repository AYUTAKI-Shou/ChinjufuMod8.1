...
public class Kit_TanaChawan_A extends BaseKit_Tana2Stage {
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
		...
		if (hItem != Items_Teatime.Item_DISH || k != 3) {
			if (hStack.isEmpty()) {
				CMEvents.emptyTake_1Item(worldIn, pos, playerIn, Items_Teatime.Item_DISH, 3);
	
				if (i == 1) { worldIn.setBlockState(pos, Kitchen_Blocks.KIT_CHAWAN1.getDefaultState()
							.withProperty(Kit_TanaChawan_1.H_FACING, state.getValue(H_FACING))
							.withProperty(Kit_TanaChawan_1.STAGE_1_4, Integer.valueOf(4))); }
	...
	public List<ItemStack> getDrops(IBlockAccess worldIn, BlockPos pos, IBlockState state, int fortune) {
		...
		stack.add(new ItemStack(Items_Teatime.Item_DISH, i + 4, 3));

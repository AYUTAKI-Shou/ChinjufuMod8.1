...
public class Kit_Sink_top extends BaseStage2_Face {
	/* Collision */
	private static final AxisAlignedBB AABB_SOUTH = CollisionHelper.getBlockBounds(EnumFacing.SOUTH, 0.15625, 0.0, 0.21875, 0.28125, 0.0625, 0.78125);
	private static final AxisAlignedBB AABB_EAST = CollisionHelper.getBlockBounds(EnumFacing.EAST, 0.15625, 0.0, 0.21875, 0.28125, 0.0625, 0.78125);
	private static final AxisAlignedBB AABB_WEST = CollisionHelper.getBlockBounds(EnumFacing.WEST, 0.15625, 0.0, 0.21875, 0.28125, 0.0625, 0.78125);
	private static final AxisAlignedBB AABB_NORTH = CollisionHelper.getBlockBounds(EnumFacing.NORTH, 0.15625, 0.0, 0.21875, 0.28125, 0.0625, 0.78125);
		...
		if (i != 1) {
			...
			if (hStack.isEmpty()) {
				...
				worldIn.setBlockState(pos.down(), Kitchen_Blocks.KIT_SINK_BOT.getDefaultState()
						.withProperty(Kit_Sink_bot.H_FACING, state.getValue(H_FACING))
						.withProperty(Kit_Sink_bot.STAGE_1_2, Integer.valueOf(1)));
				return true; }
		}
		...
		if (i == 1) {
			if (hStack.isEmpty()) {
				if (worldIn.getBlockState(new BlockPos(x, y - 3, z)).getBlock() == Blocks.WATER) {
					...
					worldIn.setBlockState(pos.down(), Kitchen_Blocks.KIT_SINK_BOT.getDefaultState()
							.withProperty(Kit_Sink_bot.H_FACING, state.getValue(H_FACING))
							.withProperty(Kit_Sink_bot.STAGE_1_2, Integer.valueOf(2))); }

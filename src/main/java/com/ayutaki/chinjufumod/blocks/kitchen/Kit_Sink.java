...
public class Kit_Sink extends Abstract_Harvest implements IWaterLoggable, IBucketPickupHandler, ILiquidContainer {
	...
	/* Collision */
	private static final VoxelShape TOP_SOUTH = VoxelShapes.or(Block.box(3.5D, 0.0D, 2.5D, 5.5D, 1.0D, 4.5D), 
			Block.box(10.5D, 0.0D, 2.5D, 12.5D, 1.0D, 4.5D));
	private static final VoxelShape TOP_WEST = VoxelShapes.or(Block.box(11.5D, 0.0D, 3.5D, 13.5D, 1.0D, 5.5D), 
			Block.box(11.5D, 0.0D, 10.5D, 13.5D, 1.0D, 12.5D));
	private static final VoxelShape TOP_NORTH = VoxelShapes.or(Block.box(3.5D, 0.0D, 11.5D, 5.5D, 1.0D, 13.5D), 
			Block.box(10.5D, 0.0D, 11.5D, 12.5D, 1.0D, 13.5D));
	private static final VoxelShape TOP_EAST = VoxelShapes.or(Block.box(2.5D, 0.0D, 3.5D, 4.5D, 1.0D, 5.5D), 
	...
	private static final VoxelShape BOT_SOUTH = VoxelShapes.or(BASE, Block.box(0.0D, 0.0D, 1.0D, 16.0D, 15.0D, 15.0D));
	private static final VoxelShape BOT_WEST = VoxelShapes.or(BASE, Block.box(1.0D, 0.0D, 0.0D, 15.0D, 15.0D, 16.0D));
	private static final VoxelShape BOT_NORTH = VoxelShapes.or(BASE, Block.box(0.0D, 0.0D, 1.0D, 16.0D, 15.0D, 15.0D));
	private static final VoxelShape BOT_EAST = VoxelShapes.or(BASE, Block.box(1.0D, 0.0D, 0.0D, 15.0D, 15.0D, 16.0D));

	public Kit_Sink(AbstractBlock.Properties props) {
		super(props);
	...
	public void randomTick(BlockState state, ServerWorld worldIn, BlockPos pos, Random rand) {
		if (state.getValue(LIT) && !hasWater2(worldIn, pos) && !hasWater3(worldIn, pos)) { 
			worldIn.setBlock(pos, state.setValue(LIT, Boolean.valueOf(false)), 3); }
	...
	public void appendHoverText(ItemStack stack, @Nullable IBlockReader worldIn, List<ITextComponent> blockTip, ITooltipFlag tipFlag) {
		blockTip.add(new TranslationTextComponent("tips.block_kit_sink").withStyle(TextFormatting.GRAY));
	}
}

...
public class Kit_TanaYunomi_1 extends BaseKit_Tana4Stage {
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
		...
		if (hItem == Items_Teatime.Item_DISH && k == 1) {
			...
			if (i == 4) { worldIn.setBlockState(pos, Kitchen_Blocks.KIT_YUNOMIA.getDefaultState()
					.withProperty(Kit_TanaYunomi_A.H_FACING, state.getValue(H_FACING))); }
			
			else { //i != 4
				worldIn.setBlockState(pos, state.withProperty(STAGE_1_4, Integer.valueOf(i + 1)), 3); }
		}
		
		if (hItem == Items_Teatime.KYUSU_kara) { CMEvents.textNotHave(worldIn, pos, playerIn); }
	...
	public List<ItemStack> getDrops(IBlockAccess worldIn, BlockPos pos, IBlockState state, int fortune) {
		...
		stack.add(new ItemStack(Items_Teatime.Item_DISH, i, 1));

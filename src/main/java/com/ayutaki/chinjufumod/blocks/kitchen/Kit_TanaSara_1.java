...
public class Kit_TanaSara_1 extends BaseKit_Tana4Stage {
	...
	public Kit_TanaSara_1(String name) {
		super(name);
	}
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
		...
		if (hItem != Items_Teatime.Item_SARA) {
			if (hStack.isEmpty()) {
				CMEvents.emptyTake_1Item(worldIn, pos, playerIn, Items_Teatime.Item_SARA, 0);
		...
		if (hItem == Items_Teatime.Item_SARA) {
			...
			if (i == 4) {worldIn.setBlockState(pos, Kitchen_Blocks.KIT_SARAA.getDefaultState()
					.withProperty(Kit_TanaSara_A.H_FACING, state.getValue(H_FACING))); }
	...
	public List<ItemStack> getDrops(IBlockAccess worldIn, BlockPos pos, IBlockState state, int fortune) {
		...
		stack.add(new ItemStack(Items_Teatime.Item_SARA, i, 0));

...
public class Kit_TanaDrinkglass_A extends BaseKit_Tana4Stage {
	...
	public Kit_TanaDrinkglass_A(String name) {
		super(name);
	}
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
		...
		if (hItem != Items_Teatime.Item_DISH || k != 7) {
			if (hStack.isEmpty()) {
				CMEvents.emptyTake_1Item(worldIn, pos, playerIn, Items_Teatime.Item_DISH, 7);
				...
				if (i == 1) {
					worldIn.setBlockState(pos, Kitchen_Blocks.KIT_DRINKGLASS1.getDefaultState()
							...
							.withProperty(Kit_TanaDrinkglass_1.STAGE_1_4, Integer.valueOf(4))); }
		...
		if (hItem == Items_Teatime.Item_DISH && k == 7) {
			...
			if (i == 4) { worldIn.setBlockState(pos, Kitchen_Blocks.KIT_DRINKGLASSE.getDefaultState()
					.withProperty(Kit_TanaDrinkglass_E.H_FACING, state.getValue(H_FACING))); }
	...
	public List<ItemStack> getDrops(IBlockAccess worldIn, BlockPos pos, IBlockState state, int fortune) {
		...
		stack.add(new ItemStack(Items_Teatime.Item_DISH, i + 4, 7));

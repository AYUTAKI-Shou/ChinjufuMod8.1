...
public class Kitchen extends Base_Tana7 {
	...
	private static final VoxelShape AABB_SOUTH = VoxelShapes.or(TOP, Block.box(0.0D, 0.0D, 1.0D, 16.0D, 15.0D, 15.0D));
	private static final VoxelShape AABB_WEST = VoxelShapes.or(TOP, Block.box(1.0D, 0.0D, 0.0D, 15.0D, 15.0D, 16.0D));
	private static final VoxelShape AABB_NORTH = VoxelShapes.or(TOP, Block.box(0.0D, 0.0D, 1.0D, 16.0D, 15.0D, 15.0D));
	private static final VoxelShape AABB_EAST = VoxelShapes.or(TOP, Block.box(1.0D, 0.0D, 0.0D, 15.0D, 15.0D, 16.0D));

	public Kitchen(AbstractBlock.Properties props) {
		super(props);
	}
	...
	public ActionResultType use(BlockState state, World worldIn, BlockPos pos, PlayerEntity playerIn, Hand hand, BlockRayTraceResult hit) {
		...
		if (hStack.isEmpty()) {
			...
			if (i == 2) {
				CMEvents.emptyTake_1Item(worldIn, pos, playerIn, Items_Teatime.KETTLE_kara);
				worldIn.setBlock(pos, state.setValue(STAGE_1_7, Integer.valueOf(1)), 3); }
			
			if (i == 3) {
				CMEvents.emptyTake_1Item(worldIn, pos, playerIn, Items_Teatime.ZUNDOU);
				worldIn.setBlock(pos, state.setValue(STAGE_1_7, Integer.valueOf(1)), 3); }
			
			if (i == 4 && hItem != Items_Teatime.NABE_kara) {
				CMEvents.emptyTake_1Item(worldIn, pos, playerIn, Items_Teatime.NABE_kara);
				worldIn.setBlock(pos, state.setValue(STAGE_1_7, Integer.valueOf(1)), 3); }
			...
			if (i == 6 && hItem != Items_Teatime.FRYPAN_kara) {
				CMEvents.emptyTake_1Item(worldIn, pos, playerIn, Items_Teatime.FRYPAN_kara);
				worldIn.setBlock(pos, state.setValue(STAGE_1_7, Integer.valueOf(1)), 3); }
			...
			if (i == 1) {
				...
				if (hItem != Items_Teatime.KETTLE_kara && hItem != Items_Teatime.ZUNDOU &&
						hItem != Items_Teatime.NABE_kara && hItem != Items_Teatime.FRYPAN_kara) { 
			...
			if (i == 4) {
				...
				if (hItem != Items_Teatime.NABE_kara) { CMEvents.textFullItem(worldIn, pos, playerIn); }
			}
			...
			if (i == 6) {
				...
				if (hItem != Items_Teatime.FRYPAN_kara) { CMEvents.textFullItem(worldIn, pos, playerIn); }
			}
	...
	public void appendHoverText(ItemStack stack, @Nullable IBlockReader worldIn, List<ITextComponent> blockTip, ITooltipFlag tipFlag) {
		blockTip.add(new TranslationTextComponent("tips.block_kitchen").withStyle(TextFormatting.GRAY));
	}
}

...
public class Kit_Tana extends BaseFacingWater {
	...
	public Kit_Tana(Block.Properties props) {
		super(props);
	}
	...
	public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity playerIn, Hand hand, BlockRayTraceResult hit) {
		...
		if (hItem == Items_Teatime.SARA) {
			...
			worldIn.setBlockState(pos, Kitchen_Blocks.KIT_SARA1.getDefaultState()
					.with(Base_Tana6.H_FACING, state.get(H_FACING))
		...
		if (hItem == Items_Teatime.YUNOMI) {
			...
			worldIn.setBlockState(pos, Kitchen_Blocks.KIT_YUNOMI1.getDefaultState()
					.with(Base_Tana7.H_FACING, state.get(H_FACING))
		...
		if (hItem == Items_Teatime.DRINKGLASS) {
			...
			worldIn.setBlockState(pos, Kitchen_Blocks.KIT_DRINKGLASS1.getDefaultState()
					.with(Base_Tana9.H_FACING, state.get(H_FACING))
		...
		if (hItem == Items_Teatime.DONBURI) {
			...
			worldIn.setBlockState(pos, Kitchen_Blocks.KIT_DONBURI1.getDefaultState()
					.with(Base_Tana4.H_FACING, state.get(H_FACING))
	...
	public void addInformation(ItemStack stack, @Nullable IBlockReader worldIn, List<ITextComponent> blockTip, ITooltipFlag tipFlag) {
		blockTip.add(new TranslationTextComponent("tips.block_kit_tana").applyTextStyle(TextFormatting.GRAY));
	}
}

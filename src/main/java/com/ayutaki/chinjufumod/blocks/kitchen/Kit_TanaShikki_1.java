...
public class Kit_TanaShikki_1 extends BaseKit_Tana4Stage {
	...
	public Kit_TanaShikki_1(String name) {
		super(name);
	}
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
		...
		if (hItem != Items_Teatime.Item_DISH || k != 4) {
			if (hStack.isEmpty()) {
				CMEvents.emptyTake_1Item(worldIn, pos, playerIn, Items_Teatime.Item_DISH, 4);
		...
		if (hItem == Items_Teatime.Item_DISH && k == 4) {
			...
			if (i == 4) { worldIn.setBlockState(pos, Kitchen_Blocks.KIT_SHIKKIA.getDefaultState()
					.withProperty(Kit_TanaShikki_A.H_FACING, state.getValue(H_FACING))); }
	...
	public List<ItemStack> getDrops(IBlockAccess worldIn, BlockPos pos, IBlockState state, int fortune) {
		...
		stack.add(new ItemStack(Items_Teatime.Item_DISH, i, 4));

...
public class Reizou extends AbstractReizou<Reizou_TileEntity> {
	...
	/* Collision */
	private static final VoxelShape AABB_SOUTH = Block.makeCuboidShape(0.0D, 0.0D, 0.0D, 16.0D, 16.0D, 13.4D);
	private static final VoxelShape AABB_WEST = Block.makeCuboidShape(2.6D, 0.0D, 0.0D, 16.0D, 16.0D, 16.0D);
	private static final VoxelShape AABB_NORTH = Block.makeCuboidShape(0.0D, 0.0D, 2.6D, 16.0D, 16.0D, 16.0D);
	private static final VoxelShape AABB_EAST = Block.makeCuboidShape(0.0D, 0.0D, 0.0D, 13.4D, 16.0D, 16.0D);

	public Reizou(Block.Properties props, Supplier<TileEntityType<? extends Reizou_TileEntity>> entityType) {
	...
	public BlockState getStateForPlacement(BlockItemUseContext context) {
		...
		if (pos.getY() < 255 && worldIn.getBlockState(pos.up()).isReplaceable(context)) {
			...
			return this.getDefaultState().with(WATERLOGGED, fluid.getFluid() == Fluids.WATER)
					.with(RIGHT, Boolean.valueOf(playerIn.isSneaking()))
		...
		if (!worldIn.isRemote) {
			...
			if (state.get(POWERED) != flag) {
				...
				worldIn.setBlockState(pos.up(), Kitchen_Blocks.KIT_REIZOU_TOP.getDefaultState()
						.with(ReizouTop.H_FACING, state.get(H_FACING))
						.with(ReizouTop.POWERED, Boolean.valueOf(flag))
						.with(ReizouTop.WATERLOGGED, Boolean.valueOf(worldIn.getFluidState(pos.up()) == Fluids.WATER))
						.with(ReizouTop.RIGHT, state.get(RIGHT)), 3); }
		}
	}
	...
	public void onBlockPlacedBy(World worldIn, BlockPos pos, BlockState state, LivingEntity placer, ItemStack stack) {
		...
		worldIn.setBlockState(pos.up(), Kitchen_Blocks.KIT_REIZOU_TOP.getDefaultState()
				...
				.with(ReizouTop.POWERED, Boolean.valueOf(flag))
				.with(ReizouTop.WATERLOGGED, Boolean.valueOf(worldIn.getFluidState(pos.up()) == Fluids.WATER))
				.with(ReizouTop.RIGHT, Boolean.valueOf(placer.isCrouching())), 3);

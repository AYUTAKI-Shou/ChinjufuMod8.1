...
public class Reizou extends BlockContainer {
	...
	private static final AxisAlignedBB AABB_SOUTH = CollisionHelper.getBlockBounds(EnumFacing.SOUTH, 0.0, 0.0, 0.0, 0.8375, 1.0, 1.0);
	private static final AxisAlignedBB AABB_EAST = CollisionHelper.getBlockBounds(EnumFacing.EAST, 0.0, 0.0, 0.0, 0.8375, 1.0, 1.0);
	private static final AxisAlignedBB AABB_WEST = CollisionHelper.getBlockBounds(EnumFacing.WEST, 0.0, 0.0, 0.0, 0.8375, 1.0, 1.0);
	private static final AxisAlignedBB AABB_NORTH = CollisionHelper.getBlockBounds(EnumFacing.NORTH, 0.0, 0.0, 0.0, 0.8375, 1.0, 1.0);
	...
	public IBlockState getStateForPlacement(World worldIn, BlockPos pos, EnumFacing facing, float hitX, float hitY, float hitZ, int meta, EntityLivingBase placer) {
		...
		if (this == Kitchen_Blocks.KIT_REIZOU) {
			worldIn.setBlockState(pos.up(), Kitchen_Blocks.KIT_REIZOU_TOP.getDefaultState()
					.withProperty(ReizouTop.H_FACING, placer.getHorizontalFacing().getOpposite())
					.withProperty(ReizouTop.POWERED, Boolean.valueOf(flag))
					.withProperty(ReizouTop.RIGHT, Boolean.valueOf(placer.isSneaking()))); }
	...
	public void neighborChanged(IBlockState state, World worldIn, BlockPos pos, Block blockIn, BlockPos fromPos) {
		if (!worldIn.isRemote) {
			...
			if (state.getValue(POWERED) != flag) {
				...
				worldIn.setBlockState(pos.up(), Kitchen_Blocks.KIT_REIZOU_TOP.getDefaultState()
						.withProperty(ReizouTop.H_FACING, state.getValue(H_FACING))
						.withProperty(ReizouTop.POWERED, Boolean.valueOf(flag))
						.withProperty(ReizouTop.RIGHT, state.getValue(RIGHT)), 3); }
		}
	}
	...
	public int getMetaFromState(IBlockState state) {
		...
		if (((Boolean)state.getValue(RIGHT)).booleanValue()) { i |= 4; }
		if (((Boolean)state.getValue(POWERED)).booleanValue()) { i |= 8; }
		return i;
	}
	...
	protected BlockStateContainer createBlockState() {
		return new BlockStateContainer(this, new IProperty[] { H_FACING, RIGHT, POWERED });
	}

...
public class Zundou_Shio extends BaseZundou_2Cook {
	/** 1=cold, 2=hot **/
	public Zundou_Shio(Block.Properties props) {
		super(props);
	}
		...
		if (i == 2) {
			if (hItem == Items_Teatime.PASTA_nama) {
				...
				worldIn.setBlockState(pos, Dish_Blocks.ZUNDOU_PASTA.getDefaultState()
						.with(Zundou4_Pasta.H_FACING, state.get(H_FACING))
						.with(Zundou4_Pasta.STAGE_1_4, Integer.valueOf(1))); } //Large Items cool it down.
	
			if (hItem == Items.COD) {
				CMEvents.consume1_seSplash(worldIn, pos, playerIn, hand);
				
				worldIn.setBlockState(pos, Dish_Blocks.ZUNDOU_FISH.getDefaultState()
						.with(Zundou4_Fish.H_FACING, state.get(H_FACING))
						.with(Zundou4_Fish.STAGE_1_4, Integer.valueOf(1))); } //Large Items cool it down.
	...
	public void addInformation(ItemStack stack, @Nullable IBlockReader worldIn, List<ITextComponent> blockTip, ITooltipFlag tipFlag) {
		...
		blockTip.add(new TranslationTextComponent("tips.block_zundou_shiomizu").applyTextStyle(TextFormatting.GRAY));
	}
}

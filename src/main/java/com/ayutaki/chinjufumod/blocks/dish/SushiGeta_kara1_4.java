...
public class SushiGeta_kara1_4 extends BaseStage4_FDA {
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		if (hItem == Items_Teatime.SUSHI && k ==1) {
			...
			worldIn.setBlockState(pos, Dish_Blocks.SUSHIGETA_salmon.getDefaultState()
					.withProperty(SushiGeta_full.H_FACING, state.getValue(H_FACING))
					.withProperty(SushiGeta_full.DOWN, state.getValue(DOWN))
					.withProperty(SushiGeta_full.STAGE_1_4, Integer.valueOf(i))); }
		
		if (hItem == Items_Teatime.SUSHI && k ==2) {
			CMEvents.consume1_seSnowP(worldIn, pos, playerIn, hand);
		...
		if (hItem == Items_Teatime.SUSHI && k ==3) {
			CMEvents.consume1_seSnowP(worldIn, pos, playerIn, hand);
		...
		if (hItem == Items_Teatime.SUSHI && k == 4) {
			CMEvents.consume1_seSnowP(worldIn, pos, playerIn, hand);

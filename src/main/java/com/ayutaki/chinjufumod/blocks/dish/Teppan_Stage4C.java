...
public class Teppan_Stage4C extends Abstract_Teppan {
	...
	public Teppan_Stage4C(Block.Properties props) {
		super(props);
		/** Default state **/
		...
		if (i != 4) {
			if (hStack.isEmpty()) {
				...
				worldIn.setBlockState(pos, this.toNama().getDefaultState()
						.with(Teppan_Stage4.H_FACING, state.get(H_FACING))
						.with(Teppan_Stage4.DOWN, state.get(DOWN))
						.with(Teppan_Stage4.WATERLOGGED, state.get(WATERLOGGED))
						.with(Teppan_Stage4.STAGE_1_4, Integer.valueOf(i + 1)), 3); }
		...
		if (i == 4 && !yakiSoba) {
			if (hStack.isEmpty()) {
				...
				worldIn.setBlockState(pos, this.to5th().getDefaultState()
						.with(Teppan_5th.H_FACING, state.get(H_FACING))
						.with(Teppan_5th.DOWN, state.get(DOWN))
						.with(Teppan_5th.WATERLOGGED, state.get(WATERLOGGED))
	...
	private Block toNama() {
		...
		else { return Dish_Blocks.YAKISOBASHIO_nama; }
	}
	...
	private Block to5th() {
		...
		else { return Dish_Blocks.OKONOMISOBAC_5; }
	}
	...
	private Item cloneItem() {
		if (this == Dish_Blocks.OKONOMISOBA_click) { return Items_Teatime.OKONOMISOBA_nama; }
		if (this == Dish_Blocks.OKONOMISOBAS_click) { return Items_Teatime.OKONOMISOBAS_nama; }
		if (this == Dish_Blocks.OKONOMISOBAC_click) { return Items_Teatime.OKONOMISOBAC_nama; }
		if (this == Dish_Blocks.YAKISOBA_click) { return Items_Teatime.YAKISOBA_nama; }
		else { return Items_Teatime.YAKISOBASHIO_nama; }
	}
}

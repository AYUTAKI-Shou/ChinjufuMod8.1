...
public class Teppan_Stage4C extends Abstract_Teppan {
		...
		if (i != 4) {
			if (hStack.isEmpty()) {
				...
				worldIn.setBlockState(pos, this.toNama().getDefaultState()
						.withProperty(Teppan_Stage4.H_FACING, state.getValue(H_FACING))
						.withProperty(Teppan_Stage4.DOWN, state.getValue(DOWN))
						.withProperty(Teppan_Stage4.STAGE_1_4, Integer.valueOf(i + 1)), 3); }
		...
		if (i == 4 && !yakiSoba) {
			if (hStack.isEmpty()) {
				...
				worldIn.setBlockState(pos, this.to5th().getDefaultState()
						.withProperty(Teppan_5th.H_FACING, state.getValue(H_FACING))
						.withProperty(Teppan_5th.DOWN, state.getValue(DOWN))
	...
	private int cloneMeta() {
		if (this == Dish_Blocks.OKONOMISOBA_click) { return 5; }
		if (this == Dish_Blocks.OKONOMISOBAS_click) { return 6; }
		if (this == Dish_Blocks.OKONOMISOBAC_click) { return 7; }
		if (this == Dish_Blocks.YAKISOBA_click) { return 8; }
		else { return 9; }
	}
}

...
public class TeaPot extends BaseFood_Stage5WP {
	/* Collision */
	private static final VoxelShape AABB_SOUTH = Block.makeCuboidShape(4.0D, 0.0D, 6.0D, 10.0D, 4.0D, 10.0D);
	private static final VoxelShape AABB_WEST = Block.makeCuboidShape(6.0D, 0.0D, 4.0D, 10.0D, 4.0D, 10.0D);
	private static final VoxelShape AABB_NORTH = Block.makeCuboidShape(6.0D, 0.0D, 6.0D, 12.0D, 4.0D, 10.0D);
	private static final VoxelShape AABB_EAST = Block.makeCuboidShape(6.0D, 0.0D, 6.0D, 10.0D, 4.0D, 12.0D);
	...
	public TeaPot(Block.Properties props) {
		super(props);
	}
	...
	public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity playerIn, Hand hand, BlockRayTraceResult hit) {
		...
		if (i == 5) { CMEvents.textIsEmpty(worldIn, pos, playerIn); }
			...
			if (hItem == Items_Teatime.TCUP_kara) {
				...
				CMEvents.changeDish_seTea(worldIn, pos, playerIn, hand, Items_Teatime.TEACUP);
				worldIn.setBlockState(pos, state.with(STAGE_1_5, Integer.valueOf(i + 1))); }
			
			if (hItem != Items_Teatime.TCUP_kara) { CMEvents.textNotHave(worldIn, pos, playerIn); }
		}
	...
	public void addInformation(ItemStack stack, @Nullable IBlockReader worldIn, List<ITextComponent> blockTip, ITooltipFlag tipFlag) {
		blockTip.add(new TranslationTextComponent("tips.block_food_teapot_1").applyTextStyle(TextFormatting.GRAY));
	}
}

...
public class BaseIrori_Sakana extends Abstract_WaterLogged {
	...
	/* Collision */
	private static final VoxelShape AABB_BOX = VoxelShapes.or(Block.box(6.0D, -5.0D, 10.75D, 10.0D, 3.5D, 14.5D),
	...
	public BaseIrori_Sakana(AbstractBlock.Properties props) {
		super(props);
	...
	public BlockState updateShape(BlockState state, Direction facing, BlockState newState, IWorld worldIn, BlockPos pos, BlockPos newPos) {
		...
		if (waterOUT(state)) {
			if (cookingIn(worldIn, pos, Direction.DOWN)) { worldIn.getBlockTicks().scheduleTick(pos, this, COOK_TIME); }
			else { } }

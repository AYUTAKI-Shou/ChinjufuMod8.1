...
public class NabeTengusa extends Abstract_CookNabe {
	...
	public NabeTengusa(AbstractBlock.Properties props) {
		super(props);
		...
		if (i == 4) {
			...
			if (hItem == Items.APPLE) {
				if (gHC >= 3) {
					...
					worldIn.setBlock(pos, Dish_Blocks.NABEK_APPLE.defaultBlockState()
			...
			if (hItem == Items_Teatime.FOOD_CHERRY) {
				if (gHC >= 3) {
					CMEvents.consumeN_seSnowP(3, worldIn, pos, playerIn, hand);
					worldIn.setBlock(pos, Dish_Blocks.NABEK_CHERRY.defaultBlockState()
							.setValue(BaseNabe_Kanten.STAGE_0_15, Integer.valueOf(FACE)), 3); }
			...
			if (hItem == Items_Teatime.FOOD_MIKAN) {
				if (gHC >= 3) {
					CMEvents.consumeN_seSnowP(3, worldIn, pos, playerIn, hand);
					worldIn.setBlock(pos, Dish_Blocks.NABEK_CITRUS.defaultBlockState()
							.setValue(BaseNabe_Kanten.STAGE_0_15, Integer.valueOf(FACE)), 3); }
			...
			if (hItem == Items_Teatime.FOOD_GRAPE) {
				if (gHC >= 3) {
					CMEvents.consumeN_seSnowP(3, worldIn, pos, playerIn, hand);
					worldIn.setBlock(pos, Dish_Blocks.NABEK_GRAPE.defaultBlockState()
							.setValue(BaseNabe_Kanten.STAGE_0_15, Integer.valueOf(FACE)), 3); }
			...
			if (hItem == Items_Teatime.MIZUOKE_Milk) {
				...
				worldIn.setBlock(pos, Dish_Blocks.NABEK_MILK.defaultBlockState()
						.setValue(BaseNabe_Kanten.STAGE_0_15, Integer.valueOf(FACE)), 3); }
			...
			if (hItem == Items_Teatime.ANKO) {
				...
				worldIn.setBlock(pos, Dish_Blocks.NABEK_YOKAN.defaultBlockState()
						.setValue(BaseNabe_Kanten.STAGE_0_15, Integer.valueOf(FACE)), 3); }
	...
	public void appendHoverText(ItemStack stack, @Nullable IBlockReader worldIn, List<ITextComponent> blockTip, ITooltipFlag tipFlag) {
		...
		blockTip.add(new TranslationTextComponent("tips.block_food_nabetengusa_n").withStyle(TextFormatting.RED));
	}
}

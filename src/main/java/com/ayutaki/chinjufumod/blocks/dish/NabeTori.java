...
public class NabeTori extends BaseNabe {
	...
	public NabeTori(AbstractBlock.Properties props) {
		super(props);
	}
	...
	public ActionResultType use(BlockState state, World worldIn, BlockPos pos, PlayerEntity playerIn, Hand hand, BlockRayTraceResult hit) {
		...
		if (hItem == Items_Teatime.TONSUI) {
			...
			CMEvents.changeDish_seSnowB(worldIn, pos, playerIn, hand, Items_Teatime.TONSUITORI);
		...
		if (hItem != Items_Teatime.TONSUI) { CMEvents.textNotHave(worldIn, pos, playerIn); }
	...
	public void appendHoverText(ItemStack stack, @Nullable IBlockReader worldIn, List<ITextComponent> blockTip, ITooltipFlag tipFlag) {
		blockTip.add(new TranslationTextComponent("tips.block_food_nabe_1").withStyle(TextFormatting.GRAY));
	}
}

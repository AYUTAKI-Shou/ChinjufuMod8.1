...
public class Curry_set extends BaseStage4_FDP {
	/* Collision */
		...
		if (hStack.isEmpty()) {
			...
			if (i == 4) {
				worldIn.setBlockState(pos, Dish_Blocks.TEAPOT_kara.getDefaultState()
						...
						.withProperty(TeaPot_kara.STAGE_1_4, Integer.valueOf(4))); }
	...
	public List<ItemStack> getDrops(IBlockAccess worldIn, BlockPos pos, IBlockState state, int fortune) {
		...
		int i = ((Integer)state.getValue(STAGE_1_4)).intValue();
		if (i == 1) { stack.add(new ItemStack(Items_Teatime.CURRYSET, 1, cloneMeta())); }
		else { stack.add(new ItemStack(Items_Teatime.Item_SARA, 2, 0)); }
		return stack;
	}

	@Override
	public ItemStack getPickBlock(IBlockState state, RayTraceResult target, World worldIn, BlockPos pos, EntityPlayer playerIn) {
		return new ItemStack(Items_Teatime.CURRYSET, 1, cloneMeta());
	}

	private int cloneMeta() {
		if (this == Dish_Blocks.CURRYSET) { return 0; }
		if (this == Dish_Blocks.CURRYSET_C) { return 1; }
		else { return 2; }
	}
}

...
public class Kettle_full extends BaseStage2_Face {
	...
	/** 1=Water, 2=Hot **/
	public Kettle_full(String name) {
		super(name);
	...
	/* Cooking */
	private boolean cookingIn(World worldIn, BlockPos pos) {
		Block downBlock = worldIn.getBlockState(pos.down()).getBlock();
		return (downBlock == Blocks.LIT_FURNACE || downBlock == School_Blocks.LIT_CSTOVE_top || 
				downBlock == Kitchen_Blocks.LIT_KITSTOVE || downBlock == Kitchen_Blocks.LIT_KITOVEN || 
				downBlock == Kitchen_Blocks.LIT_KITOVEN_B || downBlock == Kitchen_Blocks.LIT_IRORI);
	}

...
public class SushiGeta_kara1_5 extends BaseFood_Stage5WA {
	...
	public SushiGeta_kara1_5(AbstractBlock.Properties props) {
		super(props);
	}
		...
		if (state.getValue(WATERLOGGED)) { CMEvents.textIsWaterlogged(worldIn, pos, playerIn); }
			...
			/** Soy sauce is not empty **/
				...
				if (hItem == Items_Teatime.SUSHI_S) {
					...
					worldIn.setBlock(pos, Dish_Blocks.SUSHIGETA_salmon.defaultBlockState()
				...
				if (hItem == Items_Teatime.SUSHI_F) {
					...
					worldIn.setBlock(pos, Dish_Blocks.SUSHIGETA_fish.defaultBlockState()
							.setValue(SushiGeta_full.H_FACING, state.getValue(H_FACING))
							.setValue(SushiGeta_full.DOWN, state.getValue(DOWN))
							.setValue(SushiGeta_full.STAGE_1_4, Integer.valueOf(i - 1)), 3); }
				...
				if (hItem == Items_Teatime.SUSHI_B) {
					...
					worldIn.setBlock(pos, Dish_Blocks.SUSHIGETA_beef.defaultBlockState()
							.setValue(SushiGeta_full.H_FACING, state.getValue(H_FACING))
							.setValue(SushiGeta_full.DOWN, state.getValue(DOWN))
							.setValue(SushiGeta_full.STAGE_1_4, Integer.valueOf(i - 1)), 3); }
				...
				if (hItem == Items_Teatime.SUSHI_T) {
					...
					worldIn.setBlock(pos, Dish_Blocks.SUSHIGETA_tamago.defaultBlockState()
							.setValue(SushiGeta_full.H_FACING, state.getValue(H_FACING))
							.setValue(SushiGeta_full.DOWN, state.getValue(DOWN))
							.setValue(SushiGeta_full.STAGE_1_4, Integer.valueOf(i - 1)), 3); }
	...
	public void appendHoverText(ItemStack stack, @Nullable IBlockReader worldIn, List<ITextComponent> blockTip, ITooltipFlag tipFlag) {
		blockTip.add(new TranslationTextComponent("tips.block_food_sushigeta_kara").withStyle(TextFormatting.GRAY));
	}
}

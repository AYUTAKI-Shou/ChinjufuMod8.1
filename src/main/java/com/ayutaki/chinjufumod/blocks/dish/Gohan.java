...
public class Gohan extends BaseFood_Stage3WP {
	/* Collision */
	private static final VoxelShape AABB_BOX = Block.makeCuboidShape(6.5D, 0.0D, 6.5D, 9.5D, 2.0D, 9.5D);
	private static final VoxelShape AABB_DOWN = Block.makeCuboidShape(6.5D, -8.0D, 6.5D, 9.5D, 0.1D, 9.5D);

	public Gohan(Block.Properties props) {
		super(props);
	}
			...
			if (hStack.isEmpty()) {
				...
				if (!worldIn.isRemote) {
					if (this == Dish_Blocks.GOHAN) { 
						...
						playerIn.addPotionEffect(new EffectInstance(Effects.HASTE, 150, 0));
						playerIn.addPotionEffect(new EffectInstance(Effects.SATURATION, 2, 0)); }
				}

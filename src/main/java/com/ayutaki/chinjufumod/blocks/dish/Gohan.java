...
public class Gohan extends BaseStage3_FDP {
		...
		if (i == 3) { CMEvents.textIsEmpty(worldIn, pos, playerIn); }
			...
			if (hStack.isEmpty()) {
				...
				if (!worldIn.isRemote) {
					...
					else { //this != Dish_Blocks.GOHAN
						((EntityLivingBase) playerIn).addPotionEffect(new PotionEffect(MobEffects.HASTE, 150, 0));
						((EntityLivingBase) playerIn).addPotionEffect(new PotionEffect(MobEffects.SATURATION, 2, 0)); }
				}
	...
	public List<ItemStack> getDrops(IBlockAccess worldIn, BlockPos pos, IBlockState state, int fortune) {
		...
		else { stack.add(new ItemStack(Items_Teatime.Item_DISH, 1, 3)); }
		return stack;
	}
	...
	private Item cloneItem() {
		if (this == Dish_Blocks.GOHAN_TAKE) { return Items_Teatime.GOHAN_TAKE; }
		if (this == Dish_Blocks.GOHAN_KURI) { return Items_Teatime.GOHAN_KURI; }
		if (this == Dish_Blocks.SEKIHAN) { return Items_Teatime.SEKIHAN; }
		else { return Items_Teatime.GOHAN; }
	}
}

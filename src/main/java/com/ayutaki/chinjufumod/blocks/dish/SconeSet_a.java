...
public class SconeSet_a extends BaseStage4_Face {
		...
		if (hStack.isEmpty()) {
			...
			if (i == 4) {
				worldIn.setBlockState(pos, Dish_Blocks.SCONESET_kara.getDefaultState()
						.withProperty(SconeSet_kara.H_FACING, state.getValue(H_FACING))); }
			else { //i != 4
	...
	private Item takeItem(IBlockState state) {
		...
		if (i == 1) { return Items_Teatime.SCONE; }
		if (i == 2) { return Items_Teatime.SCONE; }
		if (i == 3) { return Items_Teatime.CAKE; }

...
public class YakiZakana_set extends BaseFood_Stage5Water {
	...
	public YakiZakana_set(BlockBehaviour.Properties props) {
		super(props);
	}
		...
		if (i == 5) { CMEvents.textIsEmpty(worldIn, pos, playerIn); }
			...
			if (hStack.isEmpty()) {
				...
				if (!worldIn.isClientSide) {
					boolean GOHAN = (this == Dish_Blocks.YAKIZAKANATEI.get() || this == Dish_Blocks.YAKIJYAKETEI.get());
	...
	public void tick(BlockState state, ServerLevel worldIn, BlockPos pos, Random rand) {
		...
		if (waterIn(state, worldIn, pos)) {
			...
			if (i == 5) {
				...
				worldIn.setBlock(pos, Dish_Blocks.TAMAGOYAKITEI.get().defaultBlockState()

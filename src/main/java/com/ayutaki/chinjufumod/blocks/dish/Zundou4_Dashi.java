...
public class Zundou4_Dashi extends BaseZundou_4Stage {
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		if (hItem == Items_Teatime.PASTA && k == 4) { //shouyu_don
			/** Collect with an Item **/
			CMEvents.changeBowl_seBucket(worldIn, pos, playerIn, hand, Items_Teatime.PASTA, 5); //tsuyu_don
		...
		if (hItem != Items_Teatime.PASTA || k != 4) { CMEvents.textNotHave(worldIn, pos, playerIn); }

...
public class Udon extends BaseStage4_FDP {
		...
		if (hItem != Items.COOKED_BEEF && hItem != Items.EGG) {
			if (hStack.isEmpty()) {
				...
				if (i != 4) {
					...
					/** add Potion Effect. **/
					if (!worldIn.isRemote) {
						if (su_udon) { ((EntityLivingBase) playerIn).addPotionEffect(new PotionEffect(MobEffects.SATURATION, 2, 0)); }
	...
	private Item cloneItem() {
		if (this == Dish_Blocks.UDON_SU) { return Items_Teatime.UDON_SU; }
		if (this == Dish_Blocks.UDON_NIKU) { return Items_Teatime.UDON_NIKU; }
		else { return Items_Teatime.UDON_TSUKIMI; }
	}
}

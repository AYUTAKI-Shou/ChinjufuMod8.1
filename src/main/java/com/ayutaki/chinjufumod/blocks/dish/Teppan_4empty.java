...
public class Teppan_4empty extends Abstract_Teppan {
	...
	public Teppan_4empty(Block.Properties props) {
		super(props);
		/** Default state **/
		...
		if (waterOUT(state)) {
			if (i < 3 && cookingIn(worldIn, pos)) {
				worldIn.setBlockState(pos, this.toClick().getDefaultState()
						.with(Teppan_4emptyC.H_FACING, state.get(H_FACING))
						.with(Teppan_4emptyC.DOWN, state.get(DOWN))
						.with(Teppan_4emptyC.WATERLOGGED, state.get(WATERLOGGED))

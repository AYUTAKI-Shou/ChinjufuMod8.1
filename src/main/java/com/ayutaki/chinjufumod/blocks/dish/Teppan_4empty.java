...
public class Teppan_4empty extends Abstract_Teppan {
	...
	private int takeMeta() {
		if (this == Dish_Blocks.OKONOMIYAKI_nama) { return 0; }
		if (this == Dish_Blocks.OKONOMIS_nama) { return 1; }
		else { return 2; }
	}
	...
	public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
		...
		if (cookingIn(worldIn, pos) && i < 3) {
			worldIn.scheduleUpdate(pos, this, COOK_TIME);
			worldIn.setBlockState(pos, this.toClick().getDefaultState()
					.withProperty(Teppan_4emptyC.H_FACING, state.getValue(H_FACING))
					.withProperty(Teppan_4emptyC.DOWN, state.getValue(DOWN))
	...
	public List<ItemStack> getDrops(IBlockAccess worldIn, BlockPos pos, IBlockState state, int fortune) {
		...
		else { stack.add(new ItemStack(Items_Teatime.FRYPAN_NAMA_4, 1, cloneMeta())); }
		return stack;
	}
	...
	private int cloneMeta() {
		if (this == Dish_Blocks.OKONOMIYAKI_nama) { return 2; }
		if (this == Dish_Blocks.OKONOMIS_nama) { return 3; }
		else { return 4; }
	}
}

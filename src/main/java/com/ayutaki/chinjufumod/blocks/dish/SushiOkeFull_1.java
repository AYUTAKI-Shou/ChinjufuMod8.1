...
public class SushiOkeFull_1 extends Abstract_FoodStage9Water {
	/* Collision */
	private static final VoxelShape AABB_BOX = Block.makeCuboidShape(0.0D, 0.0D, 0.0D, 16.0D, 2.5D, 16.0D);
	private static final VoxelShape AABB_DOWN = Block.makeCuboidShape(0.0D, -8.0D, 0.0D, 16.0D, -0.1D, 16.0D);

	private static final VoxelShape AABB9_BOX = Block.makeCuboidShape(1.0D, 0.0D, 1.0D, 15.0D, 2.5D, 15.0D);
	private static final VoxelShape AABB9_DOWN = Block.makeCuboidShape(1.0D, -8.0D, 1.0D, 15.0D, 0.1D, 15.0D);

	public SushiOkeFull_1(Block.Properties props) {
		super(props);
	}
			...
			if (hStack.isEmpty()) {
				...
				if (i == 8) {
					worldIn.setBlockState(pos, Dish_Blocks.SUSHIOKE_FULL_9.getDefaultState()
							.with(SushiOkeFull_9.H_FACING, state.get(H_FACING))
							.with(SushiOkeFull_9.DOWN, state.get(DOWN))
							.with(SushiOkeFull_9.STAGE_1_9, Integer.valueOf(1))); }
				if (i < 8) {
	...
	public VoxelShape getShape(BlockState state, IBlockReader worldIn, BlockPos pos, ISelectionContext context) {
		...
		else { return notDown? AABB_BOX : AABB_DOWN; }
	}

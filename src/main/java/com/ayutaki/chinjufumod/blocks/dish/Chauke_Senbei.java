...
public class Chauke_Senbei extends BaseChauke {
	/* Collision */
	private static final VoxelShape AABB_BOX = Block.box(4.5D, 0.0D, 4.5D, 11.5D, 1.5D, 11.5D);
	private static final VoxelShape AABB_DOWN = Block.box(4.5D, -8.0D, 4.5D, 11.5D, 0.1D, 11.5D);

	public Chauke_Senbei(BlockBehaviour.Properties props) {
		super(props);
	}
	...
	public InteractionResult useItemOn(ItemStack stack, BlockState state, Level worldIn, BlockPos pos, Player playerIn, InteractionHand hand, BlockHitResult hit) {
		...
		if (i == 5) { CMEvents.textIsEmpty(worldIn, pos, playerIn); }
			...
			if (hStack.isEmpty()) {
				CMEvents.emptyTake1_SnowB(worldIn, pos, playerIn, Items_Teatime.SENBEI.get());

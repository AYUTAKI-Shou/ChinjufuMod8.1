...
public class Chauke_Senbei extends BaseChauke {
	/* Collision */
	private static final VoxelShape AABB_BOX = Block.makeCuboidShape(4.5D, 0.0D, 4.5D, 11.5D, 1.5D, 11.5D);
	private static final VoxelShape AABB_DOWN = Block.makeCuboidShape(4.5D, -8.0D, 4.5D, 11.5D, 0.1D, 11.5D);
	
	public Chauke_Senbei(Block.Properties props) {
		super(props);
	}
	...
	public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity playerIn, Hand hand, BlockRayTraceResult hit) {
		...
		if (i == 5) { CMEvents.textIsEmpty(worldIn, pos, playerIn); }
			...
			if (hStack.isEmpty()) {
				CMEvents.emptyTake1_SnowB(worldIn, pos, playerIn, Items_Teatime.SENBEI);

...
public class Kanten extends BaseFood_Stage4WP {
	...
	public Kanten(AbstractBlock.Properties props) {
		super(props);
	}
			...
			if (hStack.isEmpty()) {
				...
				if (!worldIn.isClientSide) {
					...
					if (i == 1) {
						...
						else { playerIn.addEffect(new EffectInstance(this.takeEffect(), 1300 + MATCHA, 0));} }
		
					if (i == 2) {
						if (MILK) { playerIn.curePotionEffects(new ItemStack(Items.MILK_BUCKET)); }
						else { playerIn.addEffect(new EffectInstance(this.takeEffect(), 1440 + MATCHA, 0)); } }
		
					if (i == 3) {
						if (MILK) { playerIn.curePotionEffects(new ItemStack(Items.MILK_BUCKET)); }
						else { playerIn.addEffect(new EffectInstance(this.takeEffect(), 1580 + MATCHA, 0)); } }
				}
	...
	private Effect takeEffect() {
		if (this == Dish_Blocks.KANTEN_APPLE) { return Effects.DAMAGE_RESISTANCE; }
		if (this == Dish_Blocks.KANTEN_CHERRY) { return Effects.DAMAGE_BOOST; }

...
public class NabeGohan extends BaseNabe {
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		if (hItem == Items_Teatime.Item_DISH && k == 3) {
			...
			CMEvents.changeDish_seSnowB(worldIn, pos, playerIn, hand, Items_Teatime.GOHAN, 0);
		...
		if (hItem == Items_Teatime.Item_SARA) {
			/** Collect with an Item **/
			CMEvents.changeDish_seSnowB(worldIn, pos, playerIn, hand, Items_Teatime.RICE, 0);
			if (i == 1) { CMEvents.addEXP(1, worldIn, pos, playerIn); }
		...
		if (hItem == Items_Teatime.Item_DISH && k == 6) {
			if (i != 4) {
				/** Collect with an Item **/
				CMEvents.changeDish_seSnowB(worldIn, pos, playerIn, hand, Items_Teatime.DONBURI_MESHI, 0);
				if (i == 1) { CMEvents.addEXP(1, worldIn, pos, playerIn); }
				
				if (i != 3) { worldIn.setBlockState(pos, state.withProperty(STAGE_1_4, Integer.valueOf(i + 2)), 3); }
		...
		if (hItem == Items.BOWL) {
			if (i == 1) {
				/** Collect with an Item **/
				CMEvents.changeDish_seSnowB(worldIn, pos, playerIn, hand, Items_Teatime.MUSHIGOME, 0);
		...
		if ((hItem != Items_Teatime.Item_DISH && k != 3) && (hItem != Items_Teatime.Item_DISH && k != 6) && 
				hItem != Items_Teatime.Item_SARA && hItem != Items.BOWL) {

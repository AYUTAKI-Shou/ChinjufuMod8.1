...
public class NabeGohan extends BaseNabe {
	...
	public NabeGohan(Block.Properties props) {
		super(props);
	}
		...
		if (hItem == Items_Teatime.DONBURI) {
			if (i == 4) { CMEvents.textNotHave(worldIn, pos, playerIn); }
				...
				if (i == 3) { worldIn.setBlockState(pos, Dish_Blocks.NABE_kara.getDefaultState()
					...
					worldIn.setBlockState(pos, state.with(BaseNabe.STAGE_1_4, Integer.valueOf(i + 2))); }
			}
		}
	...
	public void addInformation(ItemStack stack, @Nullable IBlockReader worldIn, List<ITextComponent> blockTip, ITooltipFlag tipFlag) {
		blockTip.add(new TranslationTextComponent("tips.block_food_nabegohan_1").applyTextStyle(TextFormatting.GRAY));
	}
}

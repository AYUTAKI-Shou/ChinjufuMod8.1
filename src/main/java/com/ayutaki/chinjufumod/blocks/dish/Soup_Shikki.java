...
public class Soup_Shikki extends BaseFood_Stage3Water {
	/* Collision */
	private static final VoxelShape AABB_BOX = Block.box(6.5D, 0.0D, 6.5D, 9.5D, 2.0D, 9.5D);
	private static final VoxelShape AABB_DOWN = Block.box(6.5D, -8.0D, 6.5D, 9.5D, 0.1D, 9.5D);

	public Soup_Shikki(BlockBehaviour.Properties props) {
		super(props);
	}
		...
		if (i == 3) { CMEvents.textIsEmpty(worldIn, pos, playerIn); }
			...
			if (hStack.isEmpty()) {
				...
				if (!worldIn.isClientSide) {
					boolean miso = (this == Dish_Blocks.MISOSOUP.get());
					int SAT = miso? 1 : 2;
					...
					playerIn.addEffect(new MobEffectInstance(MobEffects.SATURATION, SAT, 0)); }

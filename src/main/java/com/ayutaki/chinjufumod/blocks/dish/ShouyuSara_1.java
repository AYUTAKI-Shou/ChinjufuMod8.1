...
public class ShouyuSara_1 extends Abstract_FoodStage9Water {
	/* Collision */
	private static final VoxelShape AABB_SOUTH = Block.makeCuboidShape(6.0D, 0.0D, 9.0D, 10.0D, 0.5D, 13.0D);
	private static final VoxelShape AABB_WEST = Block.makeCuboidShape(3.0D, 0.0D, 6.0D, 7.0D, 0.5D, 10.0D);
	private static final VoxelShape AABB_NORTH = Block.makeCuboidShape(6.0D, 0.0D, 3.0D, 10.0D, 0.5D, 7.0D);
	private static final VoxelShape AABB_EAST = Block.makeCuboidShape(9.0D, 0.0D, 6.0D, 13.0D, 0.5D, 10.0D);

	private static final VoxelShape DOWN_SOUTH = Block.makeCuboidShape(6.0D, -8.0D, 9.0D, 10.0D, 0.1D, 13.0D);
	private static final VoxelShape DOWN_WEST = Block.makeCuboidShape(3.0D, -8.0D, 6.0D, 7.0D, 0.1D, 10.0D);
	private static final VoxelShape DOWN_NORTH = Block.makeCuboidShape(6.0D, -8.0D, 3.0D, 10.0D, 0.1D, 7.0D);
	private static final VoxelShape DOWN_EAST = Block.makeCuboidShape(9.0D, -8.0D, 6.0D, 13.0D, 0.1D, 10.0D);

	public ShouyuSara_1(Block.Properties props) {
		super(props);
	}
	...
	public void addInformation(ItemStack stack, @Nullable IBlockReader worldIn, List<ITextComponent> blockTip, ITooltipFlag tipFlag) {
		blockTip.add(new TranslationTextComponent("tips.block_food_shouyusara_1").applyTextStyle(TextFormatting.GRAY));
	}
}

...
public class KuriIga_Fall extends Abstract_Falling implements IWaterLoggable {
	/* Property */
	public static final BooleanProperty WATERLOGGED = BlockStateProperties.WATERLOGGED;
	/* Collision */
	private static final VoxelShape AABB_BOX = Block.makeCuboidShape(6.0D, -1.0D, 6.0D, 10.0D, 3.5D, 10.0D);
	
	public KuriIga_Fall(Block.Properties props) {
		super(props);
		/** Default state **/
	...
	public void onBlockAdded(BlockState state, World worldIn, BlockPos pos, BlockState oldState, boolean isMoving) {
		if (waterIn(state)) { worldIn.getPendingBlockTicks().scheduleTick(pos, Wood_Blocks.KURIIGA_FALL, 200); }
	...
	public void addInformation(ItemStack stack, @Nullable IBlockReader worldIn, List<ITextComponent> blockTip, ITooltipFlag tipFlag) {
		blockTip.add(new TranslationTextComponent("tips.block_chestnuts").applyTextStyle(TextFormatting.GRAY));
	}
}

...
public class Takenoko extends Regi_addState {
	/* Property */
	public static final PropertyInteger STAGE_0_1 = PropertyInteger.create("stage", 0, 1);
	/* Collision */
	private static final AxisAlignedBB AABB_BOX = new AxisAlignedBB(0.3D, 0.0D, 0.3D, 0.7D, 0.45D, 0.7D);
	...
	private ItemStack cloneStack() {
		return new ItemStack(Items_Seasonal.TAKENOKO, 1, 0);
	}

	@Override
	@SideOnly(Side.CLIENT)
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> blockTip, ITooltipFlag advanced) {
		blockTip.add(I18n.format("tips.block_takenoko.name"));
		blockTip.add(I18n.format("tips.block_takenoko2.name"));
	}
}

...
public class Leaf_Kaede extends Abstract_Leaf {

	public static final PropertyBool CHECK_DECAY = PropertyBool.create("check_decay");

	public Leaf_Kaede(String name) {
		super(name);
	...
	protected ItemStack getSilkTouchDrop(IBlockState state) {
		return new ItemStack(Items_Seasonal.KAEDE_leaf, 1, 0);
	}

	public NonNullList<ItemStack> onSheared(ItemStack item, net.minecraft.world.IBlockAccess worldIn, BlockPos pos, int fortune) {
		return NonNullList.withSize(1, new ItemStack(Items_Seasonal.KAEDE_leaf, 1, 0));
	}
	...
	public java.util.List<ItemStack> getDrops(IBlockAccess worldIn, BlockPos pos, IBlockState state, int fortune) {
		...
		if (rand.nextInt(chance) == 0) { stack.add(new ItemStack(Items_Seasonal.KAEDE_nae, 1, 0)); }
		if (rand.nextInt(chance) == 1) { stack.add(new ItemStack(Items_Seasonal.KAEDE_nae, 1, 0)); }
		if (rand.nextInt(chance) == 2) { stack.add(new ItemStack(Items.STICK, 1, 0)); }
		return stack;
	}

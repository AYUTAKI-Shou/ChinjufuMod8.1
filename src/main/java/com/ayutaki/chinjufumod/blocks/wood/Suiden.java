...
public class Suiden extends Regi_Name {
	/* Property */
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		boolean HOE = (hItem == Items.DIAMOND_HOE || hItem == Items.GOLDEN_HOE || hItem == Items.IRON_HOE || 
				hItem == Items.STONE_HOE || hItem == Items.WOODEN_HOE);
		...
		/** 田植え **/
		if (wet != 2 && hItem == Items_Teatime.SEEDS_RICE) { return true; }

		if (wet == 2 && hItem == Items_Teatime.SEEDS_RICE && worldIn.getBlockState(pos.up()).getMaterial().isReplaceable()) {
		...
		/** 土壌調整 consume1_seSnowP(worldIn, **/
		if ((wet == 4 || wet == 3) && worldIn.isAirBlock(pos.up()) && (hItem == Items_Teatime.INEWARA || hItem == Items_Seasonal.WARAHAI ||
				(hItem == Items_Seasonal.S_CARPET && k == 1) || (hItem == Items_Seasonal.S_CARPET && k == 3) || (hItem == Items.DYE && k == 15))) {

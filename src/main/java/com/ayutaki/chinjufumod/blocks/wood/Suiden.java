...
public class Suiden extends Abstract_Harvest implements IWaterLoggable {
	/* Property */
	...
	private static final VoxelShape AABB_COL = Block.makeCuboidShape(0.0D, 0.0D, 0.0D, 16.0D, 13.0D, 16.0D);
	private static final VoxelShape AABB_BOX = Block.makeCuboidShape(0.0D, 0.0D, 0.0D, 16.0D, 15.0D, 16.0D);

	public Suiden(Block.Properties props) {
		super(props);
		/** Default state **/
	...
	public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity playerIn, Hand hand, BlockRayTraceResult hit) {
		...
		/** 米を植える **/
		if (hItem == Items_Teatime.SEEDS_RICE) { 
			if (wet == 2) {
		...
		/** 土壌調整 **/
		if (hItem == Items_Teatime.INEWARA || hItem == Items_Seasonal.WARAHAI || hItem == Items_Seasonal
				.OCHIBA_carpet || hItem == Items_Seasonal.KAEDE_carpet || hItem == Items.BONE_MEAL) {
		...
		/** 撮影用チート
		if (hItem == Items_Seasonal.ICHOH_carpet) {
			CMEvents.consume1_seSnowP(worldIn, pos, playerIn, hand);
	...
	public void addInformation(ItemStack stack, @Nullable IBlockReader worldIn, List<ITextComponent> blockTip, ITooltipFlag tipFlag) {
		blockTip.add(new TranslationTextComponent("tips.block_suiden").applyTextStyle(TextFormatting.GRAY));
	}
}

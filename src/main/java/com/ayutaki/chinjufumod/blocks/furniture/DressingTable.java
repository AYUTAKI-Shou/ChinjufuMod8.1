...
public class DressingTable extends Abstract_WaterLogged {
	/* Property */
	public static final DirectionProperty H_FACING = BlockStateProperties.HORIZONTAL_FACING;
	public static final EnumProperty<DoubleBlockHalf> HALF = BlockStateProperties.DOUBLE_BLOCK_HALF;

	/* Collision */
	private static final VoxelShape BASE = Block.makeCuboidShape(0.0D, 0.0D, 0.0D, 16.0D, 12.0D, 16.0D);

	private static final VoxelShape BOT_SOUTH = VoxelShapes.or(BASE, Block.makeCuboidShape(1.0D, 12.0D, 0.0D, 15.0D, 16.0D, 2.0D));
	private static final VoxelShape BOT_WEST = VoxelShapes.or(BASE, Block.makeCuboidShape(14.0D, 12.0D, 1.0D, 16.0D, 16.0D, 15.0D));
	private static final VoxelShape BOT_NORTH = VoxelShapes.or(BASE, Block.makeCuboidShape(1.0D, 12.0D, 14.0D, 15.0D, 16.0D, 16.0D));
	private static final VoxelShape BOT_EAST = VoxelShapes.or(BASE, Block.makeCuboidShape(0.0D, 12.0D, 1.0D, 2.0D, 16.0D, 15.0D));

	private static final VoxelShape TOP_SOUTH = Block.makeCuboidShape(1.0D, 0.0D, 0.0D, 15.0D, 16.0D, 2.0D);
	private static final VoxelShape TOP_WEST = Block.makeCuboidShape(14.0D, 0.0D, 1.0D, 16.0D, 16.0D, 15.0D);
	private static final VoxelShape TOP_NORTH = Block.makeCuboidShape(1.0D, 0.0D, 14.0D, 15.0D, 16.0D, 16.0D);
	private static final VoxelShape TOP_EAST = Block.makeCuboidShape(0.0D, 0.0D, 1.0D, 2.0D, 16.0D, 15.0D);

	public DressingTable(Block.Properties props) {
		super(props);
		/** Default state **/
	...
	public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity playerIn, Hand hand, BlockRayTraceResult hit) {
		...
		if (half != DoubleBlockHalf.UPPER) { return ActionResultType.PASS; }

		else {
			if (playerIn != null) {
				worldIn.playSound(null, pos, SoundEvents.ITEM_ARMOR_EQUIP_LEATHER, SoundCategory.PLAYERS, 1.0F, 1.0F);

				if(!worldIn.isRemote) {
					ChinjufuMod.CHANNEL.send(PacketDistributor.ALL.noArg(), new OpenUI_MirrorScreen(playerIn.getUniqueID())); } }
	...
	public BlockState updatePostPlacement(BlockState state, Direction facing, BlockState newState, IWorld worldIn, BlockPos pos, BlockPos newPos) {
		...
		if (facing.getAxis() == Direction.Axis.Y && half == DoubleBlockHalf.LOWER == (facing == Direction.UP)) {
			return newState.getBlock() == this && newState.get(HALF) != half ? state
					.with(H_FACING, newState.get(H_FACING)) : Blocks.AIR.getDefaultState();
		}
		else {

...
public class DressingTable extends Abstract_WaterLogged {
	...
	private static final VoxelShape BOT_SOUTH = VoxelShapes.or(BASE, Block.box(1.0D, 12.0D, 0.0D, 15.0D, 16.0D, 2.0D));
	private static final VoxelShape BOT_WEST = VoxelShapes.or(BASE, Block.box(14.0D, 12.0D, 1.0D, 16.0D, 16.0D, 15.0D));
	private static final VoxelShape BOT_NORTH = VoxelShapes.or(BASE, Block.box(1.0D, 12.0D, 14.0D, 15.0D, 16.0D, 16.0D));
	private static final VoxelShape BOT_EAST = VoxelShapes.or(BASE, Block.box(0.0D, 12.0D, 1.0D, 2.0D, 16.0D, 15.0D));
	...
	public DressingTable(AbstractBlock.Properties props) {
		super(props);
	...
	public ActionResultType use(BlockState state, World worldIn, BlockPos pos, PlayerEntity playerIn, Hand hand, BlockRayTraceResult hit) {
		...
		if (half != DoubleBlockHalf.UPPER) { return ActionResultType.PASS; }
			...
			if (playerIn != null) {
				worldIn.playSound(null, pos, SoundEvents.ARMOR_EQUIP_LEATHER, SoundCategory.PLAYERS, 1.0F, 1.0F);

				if(!worldIn.isClientSide()) {
					ChinjufuMod.CHANNEL.send(PacketDistributor.ALL.noArg(), new OpenUI_MirrorScreen(playerIn.getUUID())); } }

...
public class DeskCloth extends BaseStage4_FaceWater {
	/* Collision */
	private static final VoxelShape SOUTH_AABB = Block.makeCuboidShape(-1.5D, 9.0D, 15.0D, 17.5D, 16.0D, 16.0D);
	private static final VoxelShape EAST_AABB = Block.makeCuboidShape(15.0D, 9.0D, -1.5D, 16.0D, 16.0D, 17.5D);
	private static final VoxelShape WEST_AABB = Block.makeCuboidShape(0.0D, 9.0D, -1.5D, 1.0D, 16.0D, 17.5D);
	private static final VoxelShape NORTH_AABB = Block.makeCuboidShape(-1.5D, 9.0D, 0.0D, 17.5D, 16.0D, 1.0D);
	
	public DeskCloth(Block.Properties props) {
		super(props);
	}
	...
	public BlockState updatePostPlacement(BlockState state, Direction facing, BlockState newState, IWorld worldIn, BlockPos pos, BlockPos newPos) {
		...
		return this.canNotStay(state, worldIn, pos)? Blocks.AIR.getDefaultState() : super.updatePostPlacement(state, facing, newState, worldIn, pos, newPos);
	}

	public boolean canNotStay(BlockState state, IWorldReader worldIn, BlockPos pos) {
		Direction direction = state.get(H_FACING);

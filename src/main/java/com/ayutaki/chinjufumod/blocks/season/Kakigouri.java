			...
			if (hStack.isEmpty()) {
				...
				if (!worldIn.isClientSide) {
					...
					if (i == 1) {
						...
						else { playerIn.addEffect(new MobEffectInstance(takeEffect(), 600, 0)); } }
		
					if (i == 2) {
						if (PLANE) { playerIn.addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SPEED, 500, 0)); }
						else { playerIn.addEffect(new MobEffectInstance(takeEffect(), 780, 0)); } }
		
					if (i == 3) {
						if (PLANE) { playerIn.addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SPEED, 600, 0)); }
						else { playerIn.addEffect(new MobEffectInstance(takeEffect(), 900, 0)); } }
				}

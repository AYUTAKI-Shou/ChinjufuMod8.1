...
public class SnowMan_Bot extends Base_SnowManBot {

	public SnowMan_Bot(String name) {
		super(name);
	}
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		if (hItem == Item.getItemFromBlock(Blocks.SNOW_LAYER)) {
			...
			worldIn.setBlockState(pos, this.takeBlock().getDefaultState()
					...
					.withProperty(BaseStage4_Face.STAGE_1_4, Integer.valueOf(i)), 3);
		}
		
		if (hItem != Item.getItemFromBlock(Blocks.SNOW_LAYER)) { CMEvents.textNotHave(worldIn, pos, playerIn); }
		/** 'true' to not put anything on top. **/
		return true;
	}
	
	private Block takeBlock() {
		if (this == Seasonal_Blocks.SNOWMAN_BOT2) { return Seasonal_Blocks.SNOWMAN_BOT2D; }
		if (this == Seasonal_Blocks.SNOWMAN_BOT3) { return Seasonal_Blocks.SNOWMAN_BOT3D; }
		if (this == Seasonal_Blocks.SNOWMAN_BOT4) { return Seasonal_Blocks.SNOWMAN_BOT4D; }
		if (this == Seasonal_Blocks.SNOWMAN_BOT5) { return Seasonal_Blocks.SNOWMAN_BOT5D; }
		else { return Seasonal_Blocks.SNOWMAN_BOT1D; }
	}
	...
	public List<ItemStack> getDrops(IBlockAccess worldIn, BlockPos pos, IBlockState state, int fortune) {
		...
		if (this == Seasonal_Blocks.SNOWMAN_BOT1) {

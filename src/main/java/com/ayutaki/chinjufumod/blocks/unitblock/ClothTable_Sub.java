...
public class ClothTable_Sub extends BaseClothTable {
	...
	public ClothTable_Sub(Block.Properties props) {
		super(props);
	}
		...
		if (hStack.isEmpty()) {
			if (playerIn.isSneaking()) {
				...
				worldIn.setBlockState(pos, takeMain().getDefaultState()
						.with(ClothTable.STAGE_0_15, state.get(STAGE_0_15))
						.with(ClothTable.WATERLOGGED, state.get(WATERLOGGED)), 3); }
			
			else { // !Crouching
				CMEvents.emptyTakeN_ClothB(worldIn, pos, playerIn, takeItem(state), 1);
				worldIn.setBlockState(pos, takeBlock().getDefaultState()
						.with(CafeTable.WHICH, Boolean.valueOf(true))
	...
	private Block takeBlock() {
		if (this == Unit_Blocks.CLOTHTABLE_oaksub) { return Unit_Blocks.CAFETABLE; }
		if (this == Unit_Blocks.CLOTHTABLE_sprucesub) { return Unit_Blocks.CAFETABLE_spruce; }
		if (this == Unit_Blocks.CLOTHTABLE_birchsub) { return Unit_Blocks.CAFETABLE_birch; }
		if (this == Unit_Blocks.CLOTHTABLE_junglesub) { return Unit_Blocks.CAFETABLE_jungle; }
		if (this == Unit_Blocks.CLOTHTABLE_acaciasub) { return Unit_Blocks.CAFETABLE_acacia; }
		if (this == Unit_Blocks.CLOTHTABLE_darkoaksub) { return Unit_Blocks.CAFETABLE_darkoak; }
		if (this == Unit_Blocks.CLOTHTABLE_sakurasub) { return Unit_Blocks.CAFETABLE_sakura; }
		if (this == Unit_Blocks.CLOTHTABLE_kaedesub) { return Unit_Blocks.CAFETABLE_kaede; }

...
public class TaruY_WineCiderMead extends BaseTaru_Yoh {
	...
	public TaruY_WineCiderMead(Block.Properties props) {
		super(props);
	}
		...
		if (i >= 4) {
			if (hItem == Items_Teatime.SAKEBOTTLE) {
				...
				worldIn.setBlockState(pos, Hakkou_Blocks.COCOA_TARU.getDefaultState()
						.with(BaseTaru_Yoh.AXIS, state.get(AXIS))
						.with(BaseTaru_Yoh.STAGE_1_6, Integer.valueOf(6)));
			}
	...
	public void addInformation(ItemStack stack, @Nullable IBlockReader worldIn, List<ITextComponent> blockTip, ITooltipFlag tipFlag) {
		if (this == Hakkou_Blocks.RINGOSHU_TARU) { blockTip.add(new TranslationTextComponent("tips.block_taru_ringoshu").applyTextStyle(TextFormatting.GRAY)); }
		if (this == Hakkou_Blocks.BUDOUSHU_TARU) { blockTip.add(new TranslationTextComponent("tips.block_taru_budoushu").applyTextStyle(TextFormatting.GRAY)); }
		if (this == Hakkou_Blocks.HACHIMITSU_TARU) { blockTip.add(new TranslationTextComponent("tips.block_taru_hachimitsushu").applyTextStyle(TextFormatting.GRAY)); }
	}
}

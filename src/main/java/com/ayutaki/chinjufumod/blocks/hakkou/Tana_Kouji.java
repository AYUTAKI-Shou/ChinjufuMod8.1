...
public class Tana_Kouji extends Base_Stage05 {
	...
	public Tana_Kouji(AbstractBlock.Properties props) {
		super(props);
	}
	...
	public ActionResultType use(BlockState state, World worldIn, BlockPos pos, PlayerEntity playerIn, Hand hand, BlockRayTraceResult hit) {
		...
		if (i == 5) {
			if (hStack.isEmpty()) {
				CMEvents.emptyTakeN_SnowB(worldIn, pos, playerIn, Items_Teatime.KOMEKOUJI, 4);
	...
	public void appendHoverText(ItemStack stack, @Nullable IBlockReader worldIn, List<ITextComponent> blockTip, ITooltipFlag tipFlag) {
		blockTip.add(new TranslationTextComponent("tips.block_taru_kouji").withStyle(TextFormatting.GRAY));
	}
}

...
public class Base_Bottle extends Abstract_WaterLogged {
	...
	/* Collision */
	private static final VoxelShape AABB_BOX = Block.makeCuboidShape(5.0D, 0.0D, 5.0D, 11.0D, 16.0D, 11.0D);
	private static final VoxelShape AABB_DOWN = Block.makeCuboidShape(5.0D, -8.0D, 5.0D, 11.0D, 8.0D, 11.0D);

	public Base_Bottle(Block.Properties props) {
		super(props);
		/** Default state **/
	...
	public BlockState updatePostPlacement(BlockState state, Direction facing, BlockState newState, IWorld worldIn, BlockPos pos, BlockPos newPos) {
		...
		if (waterIn(state, worldIn, pos) && i != 1 && i != 5) { worldIn.getPendingBlockTicks().scheduleTick(pos, this, 60); }

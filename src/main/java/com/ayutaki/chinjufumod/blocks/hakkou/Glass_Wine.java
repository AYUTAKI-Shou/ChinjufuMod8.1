...
public class Glass_Wine extends Base_Glass {
	/* Collision */
	private static final VoxelShape AABB_BOX = Block.makeCuboidShape(6.8D, 0.0D, 6.8D, 9.2D, 5.0D, 9.2D);
	private static final VoxelShape AABB_DOWN = Block.makeCuboidShape(6.8D, -8.0D, 6.8D, 9.2D, 0.1D, 9.2D);

	public Glass_Wine(Block.Properties props) {
		super(props);
	}
	...
	private void takeEffects(PlayerEntity playerIn, BlockState state) {
		boolean base = (this == Hakkou_Blocks.WINEGLASS);
		int i = state.get(STAGE_0_2);

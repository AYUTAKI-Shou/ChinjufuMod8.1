...
public class Glass_Wine extends Base_Glass {
	/* Collision */
	private static final VoxelShape AABB_BOX = Block.box(6.8D, 0.0D, 6.8D, 9.2D, 5.0D, 9.2D);
	private static final VoxelShape AABB_DOWN = Block.box(6.8D, -8.0D, 6.8D, 9.2D, 0.1D, 9.2D);
	
	public Glass_Wine(BlockBehaviour.Properties props) {
		super(props);
	}
	...
	private void takeEffects(Player playerIn, BlockState state) {
		boolean base = (this == Hakkou_Blocks.WINEGLASS.get());
		int i = state.getValue(STAGE_0_2);

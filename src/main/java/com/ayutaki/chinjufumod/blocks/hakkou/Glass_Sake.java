...
public class Glass_Sake extends Base_Glass {
	...
	public Glass_Sake(BlockBehaviour.Properties props) {
		super(props);
	}
	...
	private void takeEffects(Player playerIn, BlockState state) {
		...
		else { //!= Hakkou_Blocks.AMAZAKEGLASS
			boolean base = (this == Hakkou_Blocks.SAKEGLASS.get());
			boolean aged = (this == Hakkou_Blocks.JUKUSAKEGLASS.get());
			int i = state.getValue(STAGE_0_2);
			
			int eTIME = (i == 0)? 1200 : 1500;
			double eTIME2 = aged? 1 : 1.15;
			int eLV = aged? 2 : (base? 1 : 0);
			
			playerIn.addEffect(new MobEffectInstance(MobEffects.DAMAGE_BOOST, eTIME, eLV));

...
public class Taru_Miso extends BaseTaru_Stage05 {
	...
	public Taru_Miso(BlockBehaviour.Properties props) {
		super(props);
	}
	...
	public InteractionResult use(BlockState state, Level worldIn, BlockPos pos, Player playerIn, InteractionHand hand, BlockHitResult hit) {
		...
		if (i == 5) {
			if (hStack.isEmpty()) {
				CMEvents.emptyTakeN_SnowB(worldIn, pos, playerIn, Items_Teatime.MISO.get(), 4);
				worldIn.setBlock(pos, Hakkou_Blocks.HAKKOU_TARU.get().defaultBlockState()
						.setValue(Taru_Hakkou.STAGE_0_5, Integer.valueOf(2)), 3); }
	...
	public void appendHoverText(ItemStack stack, @Nullable BlockGetter worldIn, List<Component> blockTip, TooltipFlag tipFlag) {
		blockTip.add(new TranslatableComponent("tips.block_taru_miso").withStyle(ChatFormatting.GRAY));
	}
}

...
public class Kido extends Abstract_WaterLogged {
	...
	private static final VoxelShape FRAME_SOUTH = Block.makeCuboidShape(0.0D, 0.0D, 6.75D, 16.0D, 0.01D, 9.25D);
	private static final VoxelShape FRAME_WEST = Block.makeCuboidShape(6.75D, 0.0D, 0.0D, 9.25D, 0.01D, 16.0D);
	private static final VoxelShape FRAME_NORTH = Block.makeCuboidShape(0.0D, 0.0D, 6.75D, 16.0D, 0.01D, 9.25D);
	private static final VoxelShape FRAME_EAST = Block.makeCuboidShape(6.75D, 0.0D, 0.0D, 9.25D, 0.01D, 16.0D);

	private static final VoxelShape CLOSE_SOUTH = Block.makeCuboidShape(0.0D, 0.0D, 6.75D, 16.0D, 16.0D, 9.25D);
	private static final VoxelShape CLOSE_WEST = Block.makeCuboidShape(6.75D, 0.0D, 0.0D, 9.25D, 16.0D, 16.0D);
	private static final VoxelShape CLOSE_NORTH = Block.makeCuboidShape(0.0D, 0.0D, 6.75D, 16.0D, 16.0D, 9.25D);
	private static final VoxelShape CLOSE_EAST = Block.makeCuboidShape(6.75D, 0.0D, 0.0D, 9.25D, 16.0D, 16.0D);

	private static final VoxelShape OPENBR_SOUTH = VoxelShapes.or(FRAME_SOUTH, Block.makeCuboidShape(13.5D, 0.0D, 6.75D, 16.0D, 16.0D, 22.75D));
	private static final VoxelShape OPENBR_WEST = VoxelShapes.or(FRAME_WEST, Block.makeCuboidShape(-6.75D, 0.0D, 13.5D, 9.25D, 16.0D, 16.0D));
	private static final VoxelShape OPENBR_NORTH = VoxelShapes.or(FRAME_NORTH, Block.makeCuboidShape(0.0D, 0.0D, -6.75D, 2.5D, 16.0D, 9.25D));
	private static final VoxelShape OPENBR_EAST = VoxelShapes.or(FRAME_EAST, Block.makeCuboidShape(6.75D, 0.0D, 0.0D, 22.75D, 16.0D, 2.5D));

	private static final VoxelShape OPENBL_SOUTH = VoxelShapes.or(FRAME_SOUTH, Block.makeCuboidShape(0.0D, 0.0D, 6.75D, 2.5D, 16.0D, 22.75D));
	private static final VoxelShape OPENBL_WEST = VoxelShapes.or(FRAME_WEST, Block.makeCuboidShape(-6.75D, 0.0D, 0.0D, 9.25D, 16.0D, 2.5D));
	private static final VoxelShape OPENBL_NORTH = VoxelShapes.or(FRAME_NORTH, Block.makeCuboidShape(13.5D, 0.0D, -6.75D, 16.0D, 16.0D, 9.25D));
	private static final VoxelShape OPENBL_EAST = VoxelShapes.or(FRAME_EAST, Block.makeCuboidShape(6.75D, 0.0D, 13.5D, 22.75D, 16.0D, 16.0D));

	private static final VoxelShape OPENR_SOUTH = Block.makeCuboidShape(13.5D, 0.0D, 6.75D, 16.0D, 16.0D, 22.75D);
	private static final VoxelShape OPENR_WEST = Block.makeCuboidShape(-6.75D, 0.0D, 13.5D, 9.25D, 16.0D, 16.0D);
	private static final VoxelShape OPENR_NORTH = Block.makeCuboidShape(0.0D, 0.0D, -6.75D, 2.5D, 16.0D, 9.25D);
	private static final VoxelShape OPENR_EAST = Block.makeCuboidShape(6.75D, 0.0D, 0.0D, 22.75D, 16.0D, 2.5D);

	private static final VoxelShape OPENL_SOUTH = Block.makeCuboidShape(0.0D, 0.0D, 6.75D, 2.5D, 16.0D, 22.75D);
	private static final VoxelShape OPENL_WEST = Block.makeCuboidShape(-6.75D, 0.0D, 0.0D, 9.25D, 16.0D, 2.5D);
	private static final VoxelShape OPENL_NORTH = Block.makeCuboidShape(13.5D, 0.0D, -6.75D, 16.0D, 16.0D, 9.25D);
	private static final VoxelShape OPENL_EAST = Block.makeCuboidShape(6.75D, 0.0D, 13.5D, 22.75D, 16.0D, 16.0D);

	public Kido(Block.Properties props) {
		super(props);
		/** Default state **/
	...
	/* Play Sound */
	private void moveSound(World worldIn, BlockPos pos, boolean open) {
		if (open) { worldIn.playSound(null, pos, SoundEvents.BLOCK_WOODEN_DOOR_CLOSE, SoundCategory.BLOCKS, 0.8F, 1.0F); }
		else { worldIn.playSound(null, pos, SoundEvents.BLOCK_WOODEN_DOOR_OPEN, SoundCategory.BLOCKS, 0.8F, 1.0F); }
	}
	...
	public BlockState getStateForPlacement(BlockItemUseContext context) {
		...
		if (pos.getY() < 255 && worldIn.getBlockState(pos.up()).isReplaceable(context)) {
			...
			DoorHingeSide SNEAK = playerIn.isSneaking()? DoorHingeSide.RIGHT : DoorHingeSide.LEFT;
			return this.getDefaultState().with(H_FACING, context.getPlacementHorizontalFacing()).with(WATERLOGGED, fluid.getFluid() == Fluids.WATER)
					.with(HINGE, SNEAK).with(POWERED, Boolean.valueOf(flag))
	...
	public BlockState updatePostPlacement(BlockState state, Direction facing, BlockState newState, IWorld worldIn, BlockPos pos, BlockPos newPos) {
		...
		if (facing.getAxis() == Direction.Axis.Y && half == DoubleBlockHalf.LOWER == (facing == Direction.UP)) {
			return newState.getBlock() == this && newState.get(HALF) != half ? state
					.with(H_FACING, newState.get(H_FACING)).with(OPEN, newState.get(OPEN))
					.with(HINGE, newState.get(HINGE)).with(POWERED, newState.get(POWERED)) : Blocks.AIR.getDefaultState();
		}
		else {

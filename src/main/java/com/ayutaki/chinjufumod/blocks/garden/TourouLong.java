...
public class TourouLong extends Abstract_WaterLogged {
	...
	/* Collision */
	private static final VoxelShape AABB_BOT = Block.makeCuboidShape(2.0D, 0.0D, 2.0D, 14.0D, 16.0D, 14.0D);
	private static final VoxelShape AABB_TOP = Block.makeCuboidShape(2.0D, 0.0D, 2.0D, 14.0D, 14.0D, 14.0D);

	public TourouLong(Block.Properties props) {
		super(props);
		/** Default state **/
	...
	public void tick(BlockState state, ServerWorld worldIn, BlockPos pos, Random rand) {
		if (state.get(LIT) && state.get(HALF) == DoubleBlockHalf.UPPER && state.get(WATERLOGGED)) {
			worldIn.getPendingBlockTicks().scheduleTick(pos, this, 10);
			CMEvents.soundFireExting(worldIn, pos);
			worldIn.setBlockState(pos, state.with(LIT, Boolean.valueOf(false))); }

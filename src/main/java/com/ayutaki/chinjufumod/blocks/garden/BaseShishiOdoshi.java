...
public class BaseShishiOdoshi extends Abstract_WaterLogged {
	...
	/* Collision */
	private static final VoxelShape AABB_SOUTH = Block.makeCuboidShape(2.0D, 0.0D, 2.0D, 15.0D, 10.0D, 14.0D);
	private static final VoxelShape AABB_WEST = Block.makeCuboidShape(2.0D, 0.0D, 2.0D, 14.0D, 10.0D, 15.0D);
	private static final VoxelShape AABB_NORTH = Block.makeCuboidShape(1.0D, 0.0D, 2.0D, 14.0D, 10.0D, 14.0D);
	private static final VoxelShape AABB_EAST = Block.makeCuboidShape(2.0D, 0.0D, 1.0D, 14.0D, 10.0D, 14.0D);

	public BaseShishiOdoshi(Block.Properties props) {
		super(props);
		/** Default state **/
	...
	public BlockState updatePostPlacement(BlockState state, Direction facing, BlockState newState, IWorld worldIn, BlockPos pos, BlockPos newPos) {
		...
		if (facing.getAxis() == Direction.Axis.Y && half == DoubleBlockHalf.LOWER == (facing == Direction.UP)) {
			return (newState.getBlock() == Garden_Blocks.SHISHIODOSHI || newState.getBlock() == Garden_Blocks.SHISHIODOSHI2) &&
	...
	public void addInformation(ItemStack stack, @Nullable IBlockReader worldIn, List<ITextComponent> blockTip, ITooltipFlag tipFlag) {
		blockTip.add(new TranslationTextComponent("tips.block_shishiodoshi").applyTextStyle(TextFormatting.GRAY));
	}
}

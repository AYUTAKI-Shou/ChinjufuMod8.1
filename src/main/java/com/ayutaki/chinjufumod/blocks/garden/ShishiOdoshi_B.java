...
public class ShishiOdoshi_B extends Base_ShishiOdoshi {
	...
	public ShishiOdoshi_B(String name) {
		super(name);
	}
	...
	public void observedNeighborChange(IBlockState state, World worldIn, BlockPos pos, Block blockIn, BlockPos fromPos) {
		int i = ((Integer)state.getValue(STAGE_1_4)).intValue();
		if (i != 1) { worldIn.scheduleUpdate(pos, this, this.tickRate(worldIn)); }
		super.observedNeighborChange(state, worldIn, pos, blockIn, pos);
	}
	
	@Override
	public void onBlockAdded(World worldIn, BlockPos pos, IBlockState state) {
		worldIn.scheduleUpdate(pos, Garden_Blocks.SHISHIODOSHIB, this.tickRate(worldIn));
	}

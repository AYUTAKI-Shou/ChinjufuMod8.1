...
public class Takeakari extends Abstract_WaterLogged {
	...
	/* Collision */
	private static final VoxelShape AABB_SOUTH = Block.makeCuboidShape(6.5D, 0.0D, 1.5D, 9.5D, 8.0D, 4.5D);
	private static final VoxelShape AABB_WEST = Block.makeCuboidShape(11.5D, 0.0D, 6.5D, 14.5D, 8.0D, 9.5D);
	private static final VoxelShape AABB_NORTH = Block.makeCuboidShape(6.5D, 0.0D, 11.5D, 9.5D, 8.0D, 14.5D);
	private static final VoxelShape AABB_EAST = Block.makeCuboidShape(1.5D, 0.0D, 6.5D, 4.5D, 8.0D, 9.5D);

	public Takeakari(Block.Properties props) {
		super(props);
		/** Default state **/
	...
	public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity playerIn, Hand hand, BlockRayTraceResult hit) {
		...
		if (state.get(LIT)) {
			...
			if (state.get(WATERLOGGED)) { CMEvents.textIsWaterlogged(worldIn, pos, playerIn); }
				...
				if (hItem != Items.FLINT_AND_STEEL && hItem != Items_Teatime.MATCH) { CMEvents.textNotHave(worldIn, pos, playerIn); } }
		}
	...
	public void addInformation(ItemStack stack, @Nullable IBlockReader worldIn, List<ITextComponent> blockTip, ITooltipFlag tipFlag) {
		blockTip.add(new TranslationTextComponent("tips.block_takeakari").applyTextStyle(TextFormatting.GRAY));
	}
}

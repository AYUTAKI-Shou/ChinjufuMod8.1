...
public class Chouzubachi extends Abstract_WaterLogged {
	...
	/* Collision */
	private static final VoxelShape AABB_BOX = Block.makeCuboidShape(1.0D, 0.0D, 1.0D, 15.0D, 10.0D, 15.0D);

	public Chouzubachi(Block.Properties props) {
		super(props);
		/** Default state **/
	...
	public void randomTick(BlockState state, ServerWorld worldIn, BlockPos pos, Random rand) {
		...
		if (!state.get(WATERLOGGED) && worldIn.isRainingAt(pos.up()) && i != 3) {
	...
	public void addInformation(ItemStack stack, @Nullable IBlockReader worldIn, List<ITextComponent> blockTip, ITooltipFlag tipFlag) {
		blockTip.add(new TranslationTextComponent("tips.block_chouzubachi").applyTextStyle(TextFormatting.GRAY));
	}
}

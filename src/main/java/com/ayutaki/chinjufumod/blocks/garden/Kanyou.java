...
public class Kanyou extends Abstract_WaterLogged {
	...
	/* Collision */
	private static final VoxelShape AABB_TOP = Block.makeCuboidShape(2.0D, 0.0D, 2.0D, 14.0D, 16.0D, 14.0D);
	private static final VoxelShape AABB_BOT = VoxelShapes.or(Block.makeCuboidShape(7.0D, 5.0D, 7.0D, 9.0D, 16.0D, 9.0D),
	...
	private static final VoxelShape AABB_TOP2 = Block.makeCuboidShape(2.0D, 0.0D, 2.0D, 14.0D, 14.0D, 14.0D);
	private static final VoxelShape AABB_BOT2 = VoxelShapes.or(Block.makeCuboidShape(7.0D, 0.0D, 7.0D, 9.0D, 14.0D, 9.0D),
			Block.makeCuboidShape(2.0D, 14.0D, 2.0D, 14.0D, 16.0D, 14.0D));
	
	public Kanyou(Block.Properties props) {
		super(props);
	...
	public BlockState getStateForPlacement(BlockItemUseContext context) {
		...
		if (pos.getY() < 255 && worldIn.getBlockState(pos.up()).isReplaceable(context)) {
			...
			return this.getDefaultState().with(STAGE_1_2, Integer.valueOf(SNEAK))
	...
	public void addInformation(ItemStack stack, @Nullable IBlockReader worldIn, List<ITextComponent> blockTip, ITooltipFlag tipFlag) {
		blockTip.add(new TranslationTextComponent("tips.block_kanyou").applyTextStyle(TextFormatting.GRAY));
	}
}

...
public class IronFence extends Abstract_WaterLogged {
	...
	/* Collision */
	private static final VoxelShape BOT_SOUTH = Block.makeCuboidShape(0.0D, 0.0D, 7.5D, 16.0D, 16.0D, 8.5D);
	private static final VoxelShape BOT_WEST = Block.makeCuboidShape(7.5D, 0.0D, 0.0D, 8.5D, 16.0D, 16.0D);
	private static final VoxelShape BOT_NORTH = Block.makeCuboidShape(0.0D, 0.0D, 7.5D, 16.0D, 16.0D, 8.5D);
	private static final VoxelShape BOT_EAST = Block.makeCuboidShape(7.5D, 0.0D, 0.0D, 8.5D, 16.0D, 16.0D);

	private static final VoxelShape TOP_SOUTH = Block.makeCuboidShape(0.0D, 0.0D, 7.5D, 16.0D, 27.0D, 8.5D);
	private static final VoxelShape TOP_WEST = Block.makeCuboidShape(7.5D, 0.0D, 0.0D, 8.5D, 27.0D, 16.0D);
	private static final VoxelShape TOP_NORTH = Block.makeCuboidShape(0.0D, 0.0D, 7.5D, 27.0D, 16.0D, 8.5D);
	private static final VoxelShape TOP_EAST = Block.makeCuboidShape(7.5D, 0.0D, 0.0D, 8.5D, 27.0D, 16.0D);

	public IronFence(Block.Properties props) {
		super(props);
	...
	public BlockState updatePostPlacement(BlockState state, Direction facing, BlockState newState, IWorld worldIn, BlockPos pos, BlockPos newPos) {
		...
		if (facing.getAxis() != Direction.Axis.Y || half == DoubleBlockHalf.LOWER != (facing == Direction.UP) || newState.getBlock() == this && newState.get(HALF) != half) {

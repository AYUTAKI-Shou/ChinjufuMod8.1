...
public class Itabei extends Abstract_WaterLogged {
	...
	public Itabei(Block.Properties props) {
		super(props);
		/** Default state **/
	...
	public BlockState getStateForPlacement(BlockItemUseContext context) {
		...
		if (pos.getY() < 255 && worldIn.getBlockState(pos.up()).isReplaceable(context)) {
			...
			return this.getDefaultState().with(H_FACING, context.getPlacementHorizontalFacing())
					...
					.with(HALF, DoubleBlockHalf.LOWER).with(CHECK, SNEAK); }
	...
	public BlockState updatePostPlacement(BlockState state, Direction facing, BlockState newState, IWorld worldIn, BlockPos pos, BlockPos newPos) {
		...
		if (facing.getAxis() == Direction.Axis.Y && half == DoubleBlockHalf.LOWER == (facing == Direction.UP)) {
			return newState.getBlock() == this && newState.get(HALF) != half ? state
					.with(H_FACING, newState.get(H_FACING)).with(CHECK, newState.get(CHECK)) : Blocks.AIR.getDefaultState();
		}
		else {

...
public class Itabei extends Regi_addState {
	/* Property */
	...
	/* Collision */
	private static final AxisAlignedBB SN_AABB = new AxisAlignedBB(0.0D * cw, 0.0D * cw, 6.0D * cw, 16.0D * cw, 16.0D * cw, 10.0D * cw);
	private static final AxisAlignedBB EW_AABB = new AxisAlignedBB(6.0D * cw, 0.0D * cw, 0.0D * cw, 10.0D * cw, 16.0D * cw, 16.0D * cw);

	private static final AxisAlignedBB ES_AABB = new AxisAlignedBB(6.0D * cw, 0.0D * cw, 6.0D * cw, 16.0D * cw, 16.0D * cw, 16.0D * cw);
	private static final AxisAlignedBB NE_AABB = new AxisAlignedBB(6.0D * cw, 0.0D * cw, 0.0D * cw, 16.0D * cw, 16.0D * cw, 10.0D * cw);
	private static final AxisAlignedBB SW_AABB = new AxisAlignedBB(0.0D * cw, 0.0D * cw, 6.0D * cw, 10.0D * cw, 16.0D * cw, 16.0D * cw);
	private static final AxisAlignedBB WN_AABB = new AxisAlignedBB(0.0D * cw, 0.0D * cw, 0.0D * cw, 10.0D * cw, 16.0D * cw, 10.0D * cw);
	...
	public AxisAlignedBB getBoundingBox(IBlockState state, IBlockAccess source, BlockPos pos) {
		...
		switch (direction) {
		case NORTH:
		default: return flag? SN_AABB : NE_AABB;
		case SOUTH: return flag? SN_AABB : SW_AABB;
		case EAST: return flag? EW_AABB : ES_AABB;
		case WEST: return flag? EW_AABB : WN_AABB;
		}
	}
	...
	private ItemStack cloneStack(IBlockState state) {
		if (state.getValue(HALF) != HalfState.UPPER) {
			...
			else { return new ItemStack(Items_Seasonal.SITABEI_item, 1, 2); }
		}

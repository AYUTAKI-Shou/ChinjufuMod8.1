...
public class CurtainTall extends Abstract_WaterLogged {
	...
	private static final VoxelShape OPENR_NORTH = Block.makeCuboidShape(14.0D, 0.0D, 0.0D, 16.0D, 16.0D, 2.1D);
	private static final VoxelShape OPENR_EAST = Block.makeCuboidShape(13.9D, 0.0D, 14.0D, 16.0D, 16.0D, 16.0D);
	private static final VoxelShape OPENR_SOUTH = Block.makeCuboidShape(0.0D, 0.0D, 13.9D, 2.0D, 16.0D, 16.0D);
	private static final VoxelShape OPENR_WEST = Block.makeCuboidShape(0.0D, 0.0D, 0.0D, 2.1D, 16.0D, 2.0D);

	private static final VoxelShape OPENL_NORTH = Block.makeCuboidShape(0.0D, 0.0D, 0.0D, 2.0D, 16.0D, 2.1D);
	private static final VoxelShape OPENL_EAST = Block.makeCuboidShape(13.9D, 0.0D, 0.0D, 16.0D, 16.0D, 2.0D);
	private static final VoxelShape OPENL_SOUTH = Block.makeCuboidShape(14.0D, 0.0D, 13.9D, 16.0D, 16.0D, 16.0D);
	private static final VoxelShape OPENL_WEST = Block.makeCuboidShape(0.0D, 0.0D, 14.0D, 2.1D, 16.0D, 16.0D);

	public CurtainTall(Block.Properties props) {
		super(props);
		/** Default state **/
	...
	public BlockState getStateForPlacement(BlockItemUseContext context) {
		...
		if (worldIn.getBlockState(pos.down()).isReplaceable(context)) {
			...
			return this.getDefaultState().with(H_FACING, context.getPlacementHorizontalFacing()).with(WATERLOGGED, fluid.getFluid() == Fluids.WATER)
					.with(HINGE, SNEAK).with(OPEN, Boolean.valueOf(false))
					.with(HALF, DoubleBlockHalf.UPPER); }
	...
	public void onBlockPlacedBy(World worldIn, BlockPos pos, BlockState state, LivingEntity placer, ItemStack stack) {
		...
		worldIn.setBlockState(pos.down(), this.getDefaultState().with(HALF, DoubleBlockHalf.LOWER).with(H_FACING, state.get(H_FACING))
				.with(WATERLOGGED, Boolean.valueOf(fluidDown.getFluid() == Fluids.WATER)), 3);
	}
	...
	public BlockState updatePostPlacement(BlockState state, Direction facing, BlockState newState, IWorld worldIn, BlockPos pos, BlockPos newPos) {
		...
		if (facing.getAxis() == Direction.Axis.Y && half == DoubleBlockHalf.LOWER == (facing == Direction.UP)) {
			return newState.getBlock() == this && newState.get(HALF) != half ? state
					...
					.with(HINGE, newState.get(HINGE)) : Blocks.AIR.getDefaultState();
		}
		else {

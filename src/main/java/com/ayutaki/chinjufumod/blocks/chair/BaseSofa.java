...
public class BaseSofa extends Abstract_WaterLogged {
	...
	private BlockState getConnectState(BlockState state, IWorld worldIn, BlockPos pos, Direction dir) {
		...
		if (left && right) { return state.with(TYPE, TypeLR.BOTH); }

		else if (left) { return state.with(TYPE, TypeLR.RIGHT); }

		else if (right) { return state.with(TYPE, TypeLR.LEFT); }

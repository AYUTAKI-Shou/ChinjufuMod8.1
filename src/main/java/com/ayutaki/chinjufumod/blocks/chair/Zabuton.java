...
public class Zabuton extends Abstract_WaterLogged {
	...
	/* Collision */
	private static final VoxelShape AABB_BOX = Block.makeCuboidShape(1.0D, 0.0D, 1.0D, 15.0D, 3.0D, 15.0D);
	private static final VoxelShape DOWN_BOX = Block.makeCuboidShape(1.0D, -8.0D, 1.0D, 15.0D, 0.01D, 15.0D);
	private static final VoxelShape DOWN_COLL = Block.makeCuboidShape(1.0D, -8.0D, 1.0D, 15.0D, -5.0D, 15.0D);
	
	private static final VoxelShape AABB_WARA = Block.makeCuboidShape(0.0D, 0.0D, 0.0D, 16.0D, 3.0D, 16.0D);
	private static final VoxelShape DOWN_WARA = Block.makeCuboidShape(0.0D, -8.0D, 0.0D, 16.0D, 0.01D, 16.0D);
	
	public Zabuton(Block.Properties props) {
		super(props);
		/** Default state **/
	...
	public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity playerIn, Hand hand, BlockRayTraceResult hit) {
		...
		if (hItem instanceof Abstract_Hake && this != JPChair_Blocks.WARAZABUTON) { 
	...
	public void addInformation(ItemStack stack, @Nullable IBlockReader worldIn, List<ITextComponent> blockTip, ITooltipFlag tipFlag) {
		blockTip.add(new TranslationTextComponent("tips.block_mzabuton").applyTextStyle(TextFormatting.GRAY));
	}
}

...
public class Base_Slab_JP extends Regi_addState {
	...
	public Base_Slab_JP(String name) {
		super(name);
	...
	public IBlockState getStateFromMeta(int meta) {
		return this.getDefaultState().withProperty(CRA, Boolean.valueOf((meta & 1) != 0))
				...
				.withProperty(HALF, (meta & 4) == 0 ? SlabHalf.BOTTOM : SlabHalf.TOP);
	}
	...
	public int getMetaFromState(IBlockState state) {
		...
		if (state.getValue(DOUBLE)) { i |= 2; }
		if (state.getValue(HALF) != SlabHalf.TOP) { i |= 4; }
	...
	public boolean isTopSolid(IBlockState state) {
		return (state.getValue(DOUBLE) || state.getValue(HALF) == SlabHalf.TOP);
	}
	...
	public BlockFaceShape getBlockFaceShape(IBlockAccess worldIn, IBlockState state, BlockPos pos, EnumFacing face) {
		...
		if (state.getValue(DOUBLE) || (face == EnumFacing.UP && state.getValue(HALF) == SlabHalf.TOP)) {
	...
	public boolean isOpaqueCube(IBlockState state) {
		return state.getValue(DOUBLE)? true : false;
	}

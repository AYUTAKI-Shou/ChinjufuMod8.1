...
public class Base_Report extends BaseStage4_Face {
	...
	protected void flesh16_seWool(IBlockState state, World worldIn, BlockPos pos, EntityPlayer playerIn, EnumHand hand) {
		CMEvents.consumeN_Hand(16, playerIn, hand);
		CMEvents.soundClothPlace(worldIn, pos); }
	
	protected void take_5WorkOrder(World worldIn, BlockPos pos, EntityPlayer playerIn) {
		worldIn.playSound(null, pos, SoundEvents.BLOCK_NOTE_BELL, SoundCategory.BLOCKS, 1.0F, 1.0F);
		CMEvents.emptyTake_NItem(worldIn, pos, playerIn, Items_Chinjufu.WORK_ORDER, 5, 0); }

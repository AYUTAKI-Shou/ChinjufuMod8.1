...
public class AlumiBlock extends Abstract_WaterLogged {
	...
	/* Collision */
	private static final VoxelShape AABB_BOTTOM = Block.makeCuboidShape(0.5D, 0.0D, 0.5D, 15.5D, 8.0D, 15.5D);
	private static final VoxelShape AABB_BOX = Block.makeCuboidShape(0.5D, 0.0D, 0.5D, 15.5D, 16.0D, 15.5D);

	public AlumiBlock(Block.Properties props) {
		super(props);
		/** Default state **/
	...
	public BlockState getStateForPlacement(BlockItemUseContext context) {
		...
		if (state.getBlock() == this) {
			...
			return hitFace != Direction.DOWN && (hitFace == Direction.UP || context.getHitVec().y - (double)pos.getY() <= 0.5D) ? blockState2 : blockState2;
		}
	}

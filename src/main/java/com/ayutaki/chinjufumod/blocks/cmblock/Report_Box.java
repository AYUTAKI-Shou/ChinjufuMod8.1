...
public class Report_Box extends Abstract_Harvest {
	...
	public Report_Box(AbstractBlock.Properties props) {
		super(props);
	...
	public BlockState updateShape(BlockState state, Direction facing, BlockState newState, IWorld worldIn, BlockPos pos, BlockPos newPos) {
		worldIn.getBlockTicks().scheduleTick(pos, Chinjufu_Blocks.REPORT_BOX, 120);
	...
	public void onPlace(BlockState state, World worldIn, BlockPos pos, BlockState oldState, boolean isMoving) {
		int i = state.getValue(STAGE_1_15);
		boolean repo = (i == 5 || i == 7 || i == 9 || i == 11 || i == 13 || i == 15);
		if (repo) { worldIn.getBlockTicks().scheduleTick(pos, Chinjufu_Blocks.REPORT_BOX, 120); }
	}
	...
	public void tick(BlockState state, ServerWorld worldIn, BlockPos pos, Random rand) {
		...
		if (repo) {
			...
			if (i == 15) { worldIn.setBlock(pos, state.setValue(STAGE_1_15, Integer.valueOf(1)), 3); }
			else { worldIn.setBlock(pos, state.setValue(STAGE_1_15, Integer.valueOf(i + 1)), 3); }
		}
	...
	public void appendHoverText(ItemStack stack, @Nullable IBlockReader worldIn, List<ITextComponent> blockTip, ITooltipFlag tipFlag) {
		blockTip.add(new TranslationTextComponent("tips.block_report_box").withStyle(TextFormatting.GRAY));
	}
}

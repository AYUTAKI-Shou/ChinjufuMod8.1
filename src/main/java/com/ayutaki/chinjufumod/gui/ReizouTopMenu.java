...
public class ReizouTopMenu extends Container {
	private final ReizouTop_TileEntity chestSlots;
	
	public ReizouTopMenu(InventoryPlayer playerInventory, ReizouTop_TileEntity tileEntity, EntityPlayer player) {
		this.chestSlots = tileEntity;
		int containerRows = 3;
				...
				this.addSlotToContainer(new Slot(tileEntity, k + j * 9, 8 + k * 18, 18 + j * 18) {
					...
					public ItemStack onTake(EntityPlayer playerIn, ItemStack stack) {
						...
						if(!player.world.isRemote && this.inventory instanceof AbstractReizouTileEntity)
							((AbstractReizouTileEntity)tileEntity).expFromGUI(playerIn);
						return stack;
					}
				});
			}

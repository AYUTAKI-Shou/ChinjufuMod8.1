...
public class ReizouTopMenu extends AbstractContainerMenu {
	private final Container chestSlots;
	private final int containerRows;
	
	// net.minecraft.world.inventory.Shulker
	public ReizouTopMenu(int windowId, Inventory inventory) {
		this(MenuTypes_CM.REIZOUTOP_MENU.get(), windowId, inventory, new SimpleContainer(27));
	}
	
	public ReizouTopMenu(MenuType<?> type, int windowId, final Inventory inventory, final Container tileSlots) {
		super(type, windowId);
			...
			for(int k = 0; k < 9; ++k) {
				this.addSlot(new Slot(tileSlots, k + j * 9, 8 + k * 18, 18 + j * 18) {
					...
					public void onTake(Player player, ItemStack stack) {
						...
						if(player instanceof ServerPlayer serverPlayer && this.container instanceof AbstractReizouTileEntity reizou) {
							reizou.expFromGUI(serverPlayer); }
					}
				});
			}
		}

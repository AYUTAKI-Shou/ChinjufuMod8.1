...
public class TansuMenu extends Container {
	private final IInventory chestSlots;
	
	public TansuMenu(int windowId, PlayerInventory inventory, PacketBuffer data) {
		this(MenuTypes_CM.TANSU_MENU.get(), windowId, inventory, getContainer(inventory, data));
	}
	
	public TansuMenu(ContainerType<?> type, int windowId, final PlayerInventory inventory, final IInventory tileSlots) {
		super(type, windowId);
	...
	private static Tansu_TileEntity getContainer(final PlayerInventory inventory, final PacketBuffer data) {

...
public class Uchiwa_ST extends IR_Seasonal {

	public Uchiwa_ST(String name) {
		super(name);
		/** Have sub items. **/
		setHasSubtypes(true);
	}
	...
	public EnumActionResult onItemUse(EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		if (playerIn.canPlayerEdit(pos, facing, hStack) && this.int2Block(k).canPlaceBlockAt(worldIn, pos)) {
			...
			worldIn.setBlockState(pos, this.int2Block(k).getDefaultState()
					.withProperty(Uchiwa.H_FACING, direction)
	...
	private Block int2Block(int k) {
		...
		else { return Seasonal_Blocks.UCHIWA_black; }
	}
	
	@Override
	@SideOnly(Side.CLIENT)
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> itemTip, ITooltipFlag advanced) {
		itemTip.add(I18n.format("tips.block_uchiwa.name"));
	}
}

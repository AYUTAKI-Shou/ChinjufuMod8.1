...
public class WindowTall_CT extends IR_Chinjufu {

	public WindowTall_CT(String name) {
		super(name);
		/** Have sub items. **/
		setHasSubtypes(true);
	}
	...
	public EnumActionResult onItemUse(EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		if (playerIn.canPlayerEdit(pos, facing, hStack) && this.int2Block(k).canPlaceBlockAt(worldIn, pos)) {
			...
			if (playerIn.isSneaking()) {
				placeWindow_right(worldIn, pos, direction, this.int2Block(k));
			...
			/** デフォルトは left。右取手, 左蝶番で開くのが hinge=left **/
				...
				placeWindow_left(worldIn, pos, direction, this.int2Block(k));
				CMEvents.ItemBlock_Wood(worldIn, pos, playerIn, hand);
				return EnumActionResult.SUCCESS;
			}
		}
	...
	private Block int2Block(int k) {
		...
		else { return Window_Blocks.WINDOWTALL_ichoh; }
	}

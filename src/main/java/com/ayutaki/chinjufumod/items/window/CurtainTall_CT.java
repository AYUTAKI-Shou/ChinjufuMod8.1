...
public class CurtainTall_CT extends IR_Chinjufu {

	public CurtainTall_CT(String name) {
		super(name);
		/** Have sub items. **/
		setHasSubtypes(true);
	}
	...
	public EnumActionResult onItemUse(EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		if (facing != EnumFacing.UP && facing != EnumFacing.DOWN) {
			...
			IBlockState putState = this.int2Block(k).getDefaultState().withProperty(CurtainTall.H_FACING, facing.getOpposite())
	...
	private Block int2Block(int k) {
		...
		else { return Window_Blocks.CURTAINTALL_black; }
	}

...
public class MeasureCup_TT extends BucketItem {

	private final Fluid content;
	private final Block block;

	@SuppressWarnings("deprecation")
	public MeasureCup_TT(Fluid containedFluidIn, Block blockIn, Item.Properties props) {
		super(containedFluidIn, props.tab(ItemGroups_CM.TEATIME));
		this.block = blockIn;
		this.content = containedFluidIn;
	}

	private void MeasureCup_toFull(World worldIn, BlockPos pos, PlayerEntity playerIn, Hand hand) {
		CMEvents.take1Item(playerIn, hand, Items_Teatime.KEIRYO_CUP_full);
	...
	public ActionResult<ItemStack> use(World worldIn, PlayerEntity playerIn, Hand hand) {
		...
		if (raytraceresult.getType() == RayTraceResult.Type.MISS) { return ActionResult.pass(hStack); }

		if (raytraceresult.getType() != RayTraceResult.Type.BLOCK) { return ActionResult.pass(hStack); }
				...
				if (worldIn.mayInteract(playerIn, pos) && playerIn.mayUseItemAt(pos1, direction, hStack)) {
					...
					if (this.content == Fluids.EMPTY) {
						...
						if (stateIn1.getBlock() == Blocks.CAULDRON) {
							...
							if (cauldron == 0) { return ActionResult.pass(hStack); }
	...
	@Override
	public void checkExtraContent(World worldIn, ItemStack hStack, BlockPos pos) { }
		...
		if (!context.canPlace()) { return ActionResultType.FAIL; }
			...
			if (blockcontext == null) { return ActionResultType.FAIL; }
				...
				if (stateIn == null) { return ActionResultType.FAIL; }
					...
					ItemStack hStack = blockcontext.getItemInHand();
					BlockState stateIn1 = worldIn.getBlockState(pos1);
					Block block = stateIn1.getBlock();
					if (block == stateIn.getBlock()) {
						stateIn1 = this.updateBlockStateFromTag(pos1, worldIn, hStack, stateIn1);
						this.updateCustomBlockEntityTag(pos1, worldIn, playerIn, hStack, stateIn1);
						block.setPlacedBy(worldIn, pos1, stateIn1, playerIn, hStack);
					...
					if (playerIn == null || !playerIn.abilities.instabuild) { hStack.shrink(1); }
	...
	public void appendHoverText(ItemStack stack, @Nullable World worldIn, List<ITextComponent> itemTip, ITooltipFlag tipFlag) {
		itemTip.add(new TranslationTextComponent("tips.block_measurecup").withStyle(TextFormatting.GRAY));

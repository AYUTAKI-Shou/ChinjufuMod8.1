...
public class KineTsuki_TT extends IG_Teatime {

	public KineTsuki_TT(Properties props) {
		super(props);
	}
	...
	public ActionResultType onItemUse(ItemUseContext context) {
		...
		if (!playerIn.getCooldownTracker().hasCooldown(this)) {
			if (block == Kitchen_Blocks.USU_TSUKI) {
				...
				if (hitFace != Direction.UP) { iworld.playSound(playerIn, pos, SoundEvents_CM.TOUCH_BLOCK, SoundCategory.BLOCKS, 1.0F, 0.8F); }
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> itemTip, ITooltipFlag tipFlag) {
		itemTip.add(new TranslationTextComponent("tips.block_usutsuki").applyTextStyle(TextFormatting.GRAY));
	}
}

...
public class Zaisu_DT extends IR_Wadeco {

	public Zaisu_DT(String name) {
		super(name);
		/** Have sub items. **/
		setHasSubtypes(true);
	}
	...
	public EnumActionResult onItemUse(EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		if (!hStack.isEmpty() && playerIn.canPlayerEdit(pos, facing, hStack) && worldIn.mayPlace(int2Block(k), pos, false, facing, (Entity)null)) {
			...
			worldIn.setBlockState(pos, this.int2Block(k).getDefaultState().withProperty(Zaisu.H_FACING, direction), 10);
	...
	private Block int2Block(int k) {
		...
		else { return JPDeco_Blocks.ZAISU_black; }
	}
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> itemTip, ITooltipFlag advanced) {
		itemTip.add(I18n.format("tips.block_zaisu.name"));
	}
}

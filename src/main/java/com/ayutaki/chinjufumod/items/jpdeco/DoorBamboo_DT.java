...
public class DoorBamboo_DT extends IR_Wadeco {

	public DoorBamboo_DT(String name) {
		super(name);
		/** Have sub items. **/
		setHasSubtypes(true);
	}
	...
	public String getUnlocalizedName(ItemStack stack) {
		...
		case 0:
		default:
			return "item." + "block_bamboo_door";
		case 1:
			return "item." + "block_bamboo_y_door";
		case 2:
			return "item." + "block_bamboo_k_door";
		}
	}
	...
	public EnumActionResult onItemUse(EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		if (facing != EnumFacing.UP) { return EnumActionResult.FAIL; }
			...
			if (playerIn.canPlayerEdit(pos, facing, hStack) && this.int2Block(k).canPlaceBlockAt(worldIn, pos)) {
				...
				placeDoor(worldIn, pos, enumfacing, this.int2Block(k), flag);
	...
	private Block int2Block(int k) {
		if (k == 0) { return Door_Blocks.TAKEDOOR; }
		if (k == 1) { return Door_Blocks.TAKEDOOR_Y; }
		else { return Door_Blocks.TAKEDOOR_K; }
	}

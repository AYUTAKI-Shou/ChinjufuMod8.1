package com.ayutaki.chinjufumod.items.color;

import com.ayutaki.chinjufumod.blocks.base.BaseFacingWater;
import com.ayutaki.chinjufumod.blocks.base.BaseStage2_FaceWater;
import com.ayutaki.chinjufumod.blocks.chair.BaseSofa;
import com.ayutaki.chinjufumod.blocks.chair.CafeChair;
import com.ayutaki.chinjufumod.blocks.chair.Sofa;
import com.ayutaki.chinjufumod.blocks.chair.Zabuton;
import com.ayutaki.chinjufumod.blocks.chair.Zaisu;
import com.ayutaki.chinjufumod.blocks.furniture.Candle;
import com.ayutaki.chinjufumod.blocks.furniture.DeskCloth;
import com.ayutaki.chinjufumod.blocks.harbor.Truss_Amp;
import com.ayutaki.chinjufumod.blocks.harbor.Truss_Cable;
import com.ayutaki.chinjufumod.blocks.jpblock.Base_Full_JP;
import com.ayutaki.chinjufumod.blocks.jpblock.Base_Slab_JP;
import com.ayutaki.chinjufumod.blocks.jpblock.Full_Kawara;
import com.ayutaki.chinjufumod.blocks.jpblock.Full_Namako;
import com.ayutaki.chinjufumod.blocks.jpblock.Full_Namako_B;
import com.ayutaki.chinjufumod.blocks.jpblock.Full_Plaster;
import com.ayutaki.chinjufumod.blocks.jpblock.Slab_Kawara;
import com.ayutaki.chinjufumod.blocks.jpblock.Slab_Namako;
import com.ayutaki.chinjufumod.blocks.jpblock.Slab_Namako_B;
import com.ayutaki.chinjufumod.blocks.jpblock.Slab_Plaster;
import com.ayutaki.chinjufumod.blocks.jpblock.Stairs_Kawara;
import com.ayutaki.chinjufumod.blocks.jpblock.Stairs_Namako;
import com.ayutaki.chinjufumod.blocks.jpblock.Stairs_Namako_B;
import com.ayutaki.chinjufumod.blocks.jpblock.Stairs_Plaster;
import com.ayutaki.chinjufumod.blocks.jpblock.Wall_Kawara;
import com.ayutaki.chinjufumod.blocks.jpblock.Wall_Namako;
import com.ayutaki.chinjufumod.blocks.jpblock.Wall_NamakoB;
import com.ayutaki.chinjufumod.blocks.jpblock.Wall_Plaster;
import com.ayutaki.chinjufumod.blocks.jpblock.Wall_Sama;
import com.ayutaki.chinjufumod.blocks.jpdeco.Andon;
import com.ayutaki.chinjufumod.blocks.jpdeco.BaseTatami;
import com.ayutaki.chinjufumod.blocks.jpdeco.Futon;
import com.ayutaki.chinjufumod.blocks.jpdeco.Tatami;
import com.ayutaki.chinjufumod.blocks.jpdeco.Tatami_Y;
import com.ayutaki.chinjufumod.blocks.kamoislab.Base_KamoiPlaster;
import com.ayutaki.chinjufumod.blocks.kamoislab.KamoiPlaster_Acacia;
import com.ayutaki.chinjufumod.blocks.kamoislab.KamoiPlaster_Birch;
import com.ayutaki.chinjufumod.blocks.kamoislab.KamoiPlaster_DarkOak;
import com.ayutaki.chinjufumod.blocks.kamoislab.KamoiPlaster_Ichoh;
import com.ayutaki.chinjufumod.blocks.kamoislab.KamoiPlaster_Jungle;
import com.ayutaki.chinjufumod.blocks.kamoislab.KamoiPlaster_Kaede;
import com.ayutaki.chinjufumod.blocks.kamoislab.KamoiPlaster_Oak;
import com.ayutaki.chinjufumod.blocks.kamoislab.KamoiPlaster_Sakura;
import com.ayutaki.chinjufumod.blocks.kamoislab.KamoiPlaster_Spruce;
import com.ayutaki.chinjufumod.blocks.ranma.Noren;
import com.ayutaki.chinjufumod.blocks.slidedoor.BaseSlidedoor;
import com.ayutaki.chinjufumod.blocks.slidedoor.Fusuma;
import com.ayutaki.chinjufumod.blocks.slidedoor.Fusuma_B;
import com.ayutaki.chinjufumod.blocks.unitblock.BaseClothTable;
import com.ayutaki.chinjufumod.blocks.unitblock.Wagasa;
import com.ayutaki.chinjufumod.blocks.wallpane.WallPane_Clay;
import com.ayutaki.chinjufumod.blocks.wallpane.WallPane_Glass;
import com.ayutaki.chinjufumod.blocks.wallpane.WallPane_Namako;
import com.ayutaki.chinjufumod.blocks.wallpane.WallPane_Namako_B;
import com.ayutaki.chinjufumod.blocks.wallpane.WallPane_Plaster;
import com.ayutaki.chinjufumod.blocks.window.Curtain;
import com.ayutaki.chinjufumod.blocks.window.CurtainTall;
import com.ayutaki.chinjufumod.handler.CMEvents;
import com.ayutaki.chinjufumod.registry.Chair_Blocks;
import com.ayutaki.chinjufumod.registry.Furniture_Blocks;
import com.ayutaki.chinjufumod.registry.Harbor_Blocks;
import com.ayutaki.chinjufumod.registry.JPChair_Blocks;
import com.ayutaki.chinjufumod.registry.JPDeco_Blocks;
import com.ayutaki.chinjufumod.registry.JP_Blocks;
import com.ayutaki.chinjufumod.registry.KamoiPlaster_Blocks;
import com.ayutaki.chinjufumod.registry.Ranma_Blocks;
import com.ayutaki.chinjufumod.registry.Slidedoor_Blocks;
import com.ayutaki.chinjufumod.registry.Unit_Blocks;
import com.ayutaki.chinjufumod.registry.WallPanel_Blocks;
import com.ayutaki.chinjufumod.registry.Window_Blocks;

import net.minecraft.block.Block;
import net.minecraft.block.BlockState;
import net.minecraft.block.Blocks;
import net.minecraft.block.CarpetBlock;
import net.minecraft.block.CauldronBlock;
import net.minecraft.block.ConcretePowderBlock;
import net.minecraft.block.GlazedTerracottaBlock;
import net.minecraft.block.PaneBlock;
import net.minecraft.block.StainedGlassBlock;
import net.minecraft.block.StainedGlassPaneBlock;
import net.minecraft.block.StairsBlock;
import net.minecraft.entity.player.PlayerEntity;
import net.minecraft.item.ItemStack;
import net.minecraft.item.ItemUseContext;
import net.minecraft.state.properties.DoubleBlockHalf;
import net.minecraft.state.properties.SlabType;
import net.minecraft.util.ActionResultType;
import net.minecraft.util.math.BlockPos;
import net.minecraft.world.IWorld;

public class Hake_Pink extends Abstract_Hake {

	public Hake_Pink(Properties props) {
		super(props);
	}

	/* FlintAndSteel */
	public ActionResultType onItemUse(ItemUseContext context) {
		PlayerEntity playerIn = context.getPlayer();
		IWorld iworld = context.getWorld();
		BlockPos pos = context.getPos();
		BlockState state = iworld.getBlockState(pos);
		Block block = state.getBlock();
		ItemStack hStack = context.getItem();

		/** other Blocks **/
		if (block == Blocks.WHITE_WOOL || block == Blocks.ORANGE_WOOL || block == Blocks.MAGENTA_WOOL ||
				block == Blocks.LIGHT_BLUE_WOOL || block == Blocks.YELLOW_WOOL || block == Blocks.LIME_WOOL ||
				block == Blocks.GRAY_WOOL || block == Blocks.LIGHT_GRAY_WOOL || block == Blocks.CYAN_WOOL ||
				block == Blocks.PURPLE_WOOL || block == Blocks.BLUE_WOOL || block == Blocks.BROWN_WOOL ||
				block == Blocks.GREEN_WOOL || block == Blocks.RED_WOOL || block == Blocks.BLACK_WOOL) {

			iworld.setBlockState(pos, Blocks.PINK_WOOL.getDefaultState(), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(2, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		if (block == Blocks.WHITE_CONCRETE || block == Blocks.ORANGE_CONCRETE || block == Blocks.MAGENTA_CONCRETE ||
				block == Blocks.LIGHT_BLUE_CONCRETE || block == Blocks.YELLOW_CONCRETE || block == Blocks.LIME_CONCRETE ||
				block == Blocks.GRAY_CONCRETE || block == Blocks.LIGHT_GRAY_CONCRETE || block == Blocks.CYAN_CONCRETE ||
				block == Blocks.PURPLE_CONCRETE || block == Blocks.BLUE_CONCRETE || block == Blocks.BROWN_CONCRETE ||
				block == Blocks.GREEN_CONCRETE || block == Blocks.RED_CONCRETE || block == Blocks.BLACK_CONCRETE) {

			iworld.setBlockState(pos, Blocks.PINK_CONCRETE.getDefaultState(), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(2, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		if (block == Blocks.WHITE_TERRACOTTA || block == Blocks.ORANGE_TERRACOTTA || block == Blocks.MAGENTA_TERRACOTTA ||
				block == Blocks.LIGHT_BLUE_TERRACOTTA || block == Blocks.YELLOW_TERRACOTTA || block == Blocks.LIME_TERRACOTTA ||
				block == Blocks.GRAY_TERRACOTTA || block == Blocks.LIGHT_GRAY_TERRACOTTA || block == Blocks.CYAN_TERRACOTTA ||
				block == Blocks.PURPLE_TERRACOTTA || block == Blocks.BLUE_TERRACOTTA || block == Blocks.BROWN_TERRACOTTA ||
				block == Blocks.GREEN_TERRACOTTA || block == Blocks.RED_TERRACOTTA || block == Blocks.BLACK_TERRACOTTA ||
				block == Blocks.TERRACOTTA) {

			iworld.setBlockState(pos, Blocks.PINK_TERRACOTTA.getDefaultState(), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(2, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		if (block instanceof ConcretePowderBlock && block != Blocks.PINK_CONCRETE_POWDER) {

			iworld.setBlockState(pos, Blocks.PINK_CONCRETE_POWDER.getDefaultState(), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(2, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		if ((block instanceof StainedGlassBlock && block != Blocks.PINK_STAINED_GLASS) || block == Blocks.GLASS) {

			iworld.setBlockState(pos, Blocks.PINK_STAINED_GLASS.getDefaultState(), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(2, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		/** テラコッタ **/
		if (block instanceof GlazedTerracottaBlock && block != Blocks.PINK_GLAZED_TERRACOTTA) {

			iworld.setBlockState(pos, Blocks.PINK_GLAZED_TERRACOTTA.getDefaultState()
					.with(GlazedTerracottaBlock.HORIZONTAL_FACING, state.get(GlazedTerracottaBlock.HORIZONTAL_FACING)), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(2, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		/** 板ガラス **/
		if ((block instanceof StainedGlassPaneBlock && block != Blocks.PINK_STAINED_GLASS_PANE) || block == Blocks.GLASS_PANE) {

			iworld.setBlockState(pos, Blocks.PINK_STAINED_GLASS_PANE.getDefaultState()
					.with(PaneBlock.NORTH, state.get(PaneBlock.NORTH))
					.with(PaneBlock.EAST, state.get(PaneBlock.EAST))
					.with(PaneBlock.SOUTH, state.get(PaneBlock.SOUTH))
					.with(PaneBlock.WEST, state.get(PaneBlock.WEST))
					.with(PaneBlock.WATERLOGGED, state.get(PaneBlock.WATERLOGGED)), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		/**カーペット **/
		if (block instanceof CarpetBlock && block != Blocks.PINK_CARPET) {

			iworld.setBlockState(pos, Blocks.PINK_CARPET.getDefaultState(), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}


		/** 瓦, 漆喰, なまこ壁 **/
		if (block instanceof Full_Kawara && block != JP_Blocks.KAWARA_pink) {

			iworld.setBlockState(pos, JP_Blocks.KAWARA_pink.getDefaultState()
					.with(Base_Full_JP.CRACK, state.get(Base_Full_JP.CRACK)), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(2, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		if (block instanceof Full_Plaster && block != JP_Blocks.SHIKKUI_pink && block != JP_Blocks.DIRTWALL) {

			iworld.setBlockState(pos, JP_Blocks.SHIKKUI_pink.getDefaultState()
					.with(Base_Full_JP.CRACK, state.get(Base_Full_JP.CRACK)), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(2, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		if (block instanceof Full_Namako && block != JP_Blocks.NAMAKO_pink) {

			iworld.setBlockState(pos, JP_Blocks.NAMAKO_pink.getDefaultState()
					.with(Base_Full_JP.CRACK, state.get(Base_Full_JP.CRACK)), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(2, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		if (block instanceof Full_Namako_B && block != JP_Blocks.NAMAKOB_pink) {

			iworld.setBlockState(pos, JP_Blocks.NAMAKOB_pink.getDefaultState()
					.with(Base_Full_JP.CRACK, state.get(Base_Full_JP.CRACK)), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(2, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		/** 階段 **/
		if (block instanceof Stairs_Kawara && block != JP_Blocks.KAWARA_ST_pink) {

			iworld.setBlockState(pos, JP_Blocks.KAWARA_ST_pink.getDefaultState()
					.with(StairsBlock.FACING, state.get(StairsBlock.FACING))
					.with(StairsBlock.HALF, state.get(StairsBlock.HALF))
					.with(StairsBlock.SHAPE, state.get(StairsBlock.SHAPE))
					.with(StairsBlock.WATERLOGGED, state.get(StairsBlock.WATERLOGGED)), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(2, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		if (block instanceof Stairs_Plaster && block != JP_Blocks.SHIKKUI_ST_pink && block != JP_Blocks.DIRTWALL_stairs) {

			iworld.setBlockState(pos, JP_Blocks.SHIKKUI_ST_pink.getDefaultState()
					.with(StairsBlock.FACING, state.get(StairsBlock.FACING))
					.with(StairsBlock.HALF, state.get(StairsBlock.HALF))
					.with(StairsBlock.SHAPE, state.get(StairsBlock.SHAPE))
					.with(StairsBlock.WATERLOGGED, state.get(StairsBlock.WATERLOGGED)), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(2, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		if (block instanceof Stairs_Namako && block != JP_Blocks.NAMAKO_ST_pink) {

			iworld.setBlockState(pos, JP_Blocks.NAMAKO_ST_pink.getDefaultState()
					.with(StairsBlock.FACING, state.get(StairsBlock.FACING))
					.with(StairsBlock.HALF, state.get(StairsBlock.HALF))
					.with(StairsBlock.SHAPE, state.get(StairsBlock.SHAPE))
					.with(StairsBlock.WATERLOGGED, state.get(StairsBlock.WATERLOGGED)), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(2, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		if (block instanceof Stairs_Namako_B && block != JP_Blocks.NAMAKOB_ST_pink) {

			iworld.setBlockState(pos, JP_Blocks.NAMAKOB_ST_pink.getDefaultState()
					.with(StairsBlock.FACING, state.get(StairsBlock.FACING))
					.with(StairsBlock.HALF, state.get(StairsBlock.HALF))
					.with(StairsBlock.SHAPE, state.get(StairsBlock.SHAPE))
					.with(StairsBlock.WATERLOGGED, state.get(StairsBlock.WATERLOGGED)), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(2, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		/** ハーフブロック **/
		if (block instanceof Slab_Kawara && block != JP_Blocks.KAWARA_SH_pink) {

			iworld.setBlockState(pos, JP_Blocks.KAWARA_SH_pink.getDefaultState()
					.with(Base_Slab_JP.TYPE, state.get(Base_Slab_JP.TYPE))
					.with(Base_Slab_JP.CRACK, state.get(Base_Slab_JP.CRACK))
					.with(Base_Slab_JP.WATERLOGGED, state.get(Base_Slab_JP.WATERLOGGED)), 3);

			if (state.get(Base_Slab_JP.TYPE) == SlabType.DOUBLE) {
				if (playerIn != null) { Abstract_Hake.consumeAndBreak(2, hStack, playerIn, context); }
			}

			if (state.get(Base_Slab_JP.TYPE) != SlabType.DOUBLE) {
				if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }
			}
			return ActionResultType.SUCCESS;
		}

		if (block instanceof Slab_Plaster && block != JP_Blocks.SHIKKUI_SH_pink && block != JP_Blocks.DIRTWALL_SH) {

			iworld.setBlockState(pos, JP_Blocks.SHIKKUI_SH_pink.getDefaultState()
					.with(Base_Slab_JP.TYPE, state.get(Base_Slab_JP.TYPE))
					.with(Base_Slab_JP.CRACK, state.get(Base_Slab_JP.CRACK))
					.with(Base_Slab_JP.WATERLOGGED, state.get(Base_Slab_JP.WATERLOGGED)), 3);

			if (state.get(Base_Slab_JP.TYPE) == SlabType.DOUBLE) {
				if (playerIn != null) { Abstract_Hake.consumeAndBreak(2, hStack, playerIn, context); }
			}

			if (state.get(Base_Slab_JP.TYPE) != SlabType.DOUBLE) {
				if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }
			}
			return ActionResultType.SUCCESS;
		}

		if (block instanceof Slab_Namako && block != JP_Blocks.NAMAKO_SH_pink) {

			iworld.setBlockState(pos, JP_Blocks.NAMAKO_SH_pink.getDefaultState()
					.with(Base_Slab_JP.TYPE, state.get(Base_Slab_JP.TYPE))
					.with(Base_Slab_JP.CRACK, state.get(Base_Slab_JP.CRACK))
					.with(Base_Slab_JP.WATERLOGGED, state.get(Base_Slab_JP.WATERLOGGED)), 3);

			if (state.get(Base_Slab_JP.TYPE) == SlabType.DOUBLE) {
				if (playerIn != null) { Abstract_Hake.consumeAndBreak(2, hStack, playerIn, context); }
			}

			if (state.get(Base_Slab_JP.TYPE) != SlabType.DOUBLE) {
				if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }
			}
			return ActionResultType.SUCCESS;
		}

		if (block instanceof Slab_Namako_B && block != JP_Blocks.NAMAKOB_SH_pink) {

			iworld.setBlockState(pos, JP_Blocks.NAMAKOB_SH_pink.getDefaultState()
					.with(Base_Slab_JP.TYPE, state.get(Base_Slab_JP.TYPE))
					.with(Base_Slab_JP.CRACK, state.get(Base_Slab_JP.CRACK))
					.with(Base_Slab_JP.WATERLOGGED, state.get(Base_Slab_JP.WATERLOGGED)), 3);

			if (state.get(Base_Slab_JP.TYPE) == SlabType.DOUBLE) {
				if (playerIn != null) { Abstract_Hake.consumeAndBreak(2, hStack, playerIn, context); }
			}

			if (state.get(Base_Slab_JP.TYPE) != SlabType.DOUBLE) {
				if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }
			}
			return ActionResultType.SUCCESS;
		}

		///////////////////////
		/** トラス **/
		if (block instanceof Truss_Cable && block != Harbor_Blocks.TRUSS_pink) {

			iworld.setBlockState(pos, Harbor_Blocks.TRUSS_pink.getDefaultState()
					.with(Truss_Cable.H_FACING, state.get(Truss_Cable.H_FACING))
					.with(Truss_Cable.WATERLOGGED, state.get(Truss_Cable.WATERLOGGED))
					.with(Truss_Cable.POWER, state.get(Truss_Cable.POWER)), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		if (block instanceof Truss_Amp && block != Harbor_Blocks.AMP_pink) {

			iworld.setBlockState(pos, Harbor_Blocks.AMP_pink.getDefaultState()
					.with(Truss_Amp.FACING, state.get(Truss_Amp.FACING))
					.with(Truss_Amp.WATERLOGGED, state.get(Truss_Amp.WATERLOGGED))
					.with(Truss_Amp.POWERED, state.get(Truss_Amp.POWERED)), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		/** 椅子, 座布団, キャンドル **/
		if (block instanceof CafeChair && block != Chair_Blocks.CAFECHAIR_pink) {

			iworld.setBlockState(pos, Chair_Blocks.CAFECHAIR_pink.getDefaultState(), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		if (block instanceof Zabuton && block != JPChair_Blocks.ZABUTON_pink && block != JPChair_Blocks.WARAZABUTON) {

			iworld.setBlockState(pos, JPChair_Blocks.ZABUTON_pink.getDefaultState()
					.with(Zabuton.DOWN, state.get(Zabuton.DOWN))
					.with(Zabuton.WATERLOGGED, state.get(Zabuton.WATERLOGGED)), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		if (block instanceof Candle && block != Furniture_Blocks.CANDLE_pink) {

			iworld.setBlockState(pos, Furniture_Blocks.CANDLE_pink.getDefaultState()
					.with(Candle.LIT, state.get(Candle.LIT))
					.with(Candle.DOWN, state.get(Candle.DOWN))
					.with(Candle.WATERLOGGED, state.get(Candle.WATERLOGGED)), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		/** 行灯 **/
		if (block instanceof Andon && block != JPDeco_Blocks.ANDON_pink) {

			iworld.setBlockState(pos, JPDeco_Blocks.ANDON_pink.getDefaultState()
					.with(Andon.LIT, state.get(Andon.LIT))
					.with(Andon.H_FACING, state.get(Andon.H_FACING))
					.with(Andon.WATERLOGGED, state.get(Andon.WATERLOGGED)), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		/** ソファ **/
		if (block instanceof Sofa && block != Chair_Blocks.SOFA_pink) {

			iworld.setBlockState(pos, Chair_Blocks.SOFA_pink.getDefaultState()
					.with(BaseSofa.TYPE, state.get(BaseSofa.TYPE))
					.with(BaseSofa.H_FACING, state.get(BaseSofa.H_FACING))
					.with(BaseSofa.WATERLOGGED, state.get(BaseSofa.WATERLOGGED)), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(2, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		/** 座椅子 **/
		if (block instanceof Zaisu && block != JPChair_Blocks.ZAISU_pink) {

			iworld.setBlockState(pos, JPChair_Blocks.ZAISU_pink.getDefaultState()
					.with(BaseFacingWater.H_FACING, state.get(BaseFacingWater.H_FACING))
					.with(BaseFacingWater.WATERLOGGED, state.get(BaseFacingWater.WATERLOGGED)), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		/** のれん **/
		if (block instanceof Noren && block != Ranma_Blocks.NOREN_pink) {

			iworld.setBlockState(pos, Ranma_Blocks.NOREN_pink.getDefaultState()
					.with(Noren.TYPE, state.get(Noren.TYPE))
					.with(Noren.H_FACING, state.get(Noren.H_FACING))
					.with(Noren.WATERLOGGED, state.get(Noren.WATERLOGGED)), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		/** 布団 **/
		if (block instanceof Futon && block != JPDeco_Blocks.FUTON_pink) {

			iworld.setBlockState(pos, JPDeco_Blocks.FUTON_pink.getDefaultState()
					.with(BaseStage2_FaceWater.STAGE_1_2, state.get(BaseStage2_FaceWater.STAGE_1_2))
					.with(BaseStage2_FaceWater.H_FACING, state.get(BaseStage2_FaceWater.H_FACING))
					.with(BaseStage2_FaceWater.WATERLOGGED, state.get(BaseStage2_FaceWater.WATERLOGGED)), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		/** 畳 **/
		if (block instanceof Tatami && block != JPDeco_Blocks.TATAMI_H_pink && block != JPDeco_Blocks.TATAMI_H) {

			iworld.setBlockState(pos, JPDeco_Blocks.TATAMI_H_pink.getDefaultState()
					.with(BaseTatami.TYPE, state.get(BaseTatami.TYPE))
					.with(BaseTatami.H_FACING, state.get(BaseTatami.H_FACING))
					.with(BaseTatami.WATERLOGGED, state.get(BaseTatami.WATERLOGGED)), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		if (block instanceof Tatami_Y && block != JPDeco_Blocks.TATAMI_HY_pink && block != JPDeco_Blocks.TATAMI_HY) {

			iworld.setBlockState(pos, JPDeco_Blocks.TATAMI_HY_pink.getDefaultState()
					.with(BaseTatami.TYPE, state.get(BaseTatami.TYPE))
					.with(BaseTatami.H_FACING, state.get(BaseTatami.H_FACING))
					.with(BaseTatami.WATERLOGGED, state.get(BaseTatami.WATERLOGGED)), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		/** 傘 **/
		if (block instanceof Wagasa && block != Unit_Blocks.KASA_pink) {

			iworld.setBlockState(pos, Unit_Blocks.KASA_pink.getDefaultState()
					.with(Wagasa.NORTH, state.get(Wagasa.NORTH))
					.with(Wagasa.EAST, state.get(Wagasa.EAST))
					.with(Wagasa.SOUTH, state.get(Wagasa.SOUTH))
					.with(Wagasa.WEST, state.get(Wagasa.WEST))
					.with(Wagasa.WHICH, Boolean.valueOf(false))
					.with(Wagasa.WATERLOGGED, state.get(Wagasa.WATERLOGGED)), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}


		///////////////////////
		/** ふすま **/
		if (block instanceof Fusuma && block != Slidedoor_Blocks.FUSUMA_pink) {

			if (state.get(BaseSlidedoor.HALF) == DoubleBlockHalf.LOWER) {
				iworld.setBlockState(pos, Slidedoor_Blocks.FUSUMA_pink.getDefaultState()
						.with(BaseSlidedoor.H_FACING, state.get(BaseSlidedoor.H_FACING))
						.with(BaseSlidedoor.OPEN, state.get(BaseSlidedoor.OPEN))
						.with(BaseSlidedoor.HINGE, state.get(BaseSlidedoor.HINGE))
						.with(BaseSlidedoor.POWERED, state.get(BaseSlidedoor.POWERED))
						.with(BaseSlidedoor.HALF, DoubleBlockHalf.LOWER)
						.with(BaseSlidedoor.WATERLOGGED, state.get(BaseSlidedoor.WATERLOGGED)), 3);
				iworld.setBlockState(pos.up(), Slidedoor_Blocks.FUSUMA_pink.getDefaultState()
						.with(BaseSlidedoor.H_FACING, state.get(BaseSlidedoor.H_FACING))
						.with(BaseSlidedoor.OPEN, state.get(BaseSlidedoor.OPEN))
						.with(BaseSlidedoor.HINGE, state.get(BaseSlidedoor.HINGE))
						.with(BaseSlidedoor.POWERED, state.get(BaseSlidedoor.POWERED))
						.with(BaseSlidedoor.HALF, DoubleBlockHalf.UPPER)
						.with(BaseSlidedoor.WATERLOGGED, state.get(BaseSlidedoor.WATERLOGGED)), 3); }

			if (state.get(BaseSlidedoor.HALF) == DoubleBlockHalf.UPPER) {
				iworld.setBlockState(pos, Slidedoor_Blocks.FUSUMA_pink.getDefaultState()
						.with(BaseSlidedoor.H_FACING, state.get(BaseSlidedoor.H_FACING))
						.with(BaseSlidedoor.OPEN, state.get(BaseSlidedoor.OPEN))
						.with(BaseSlidedoor.HINGE, state.get(BaseSlidedoor.HINGE))
						.with(BaseSlidedoor.POWERED, state.get(BaseSlidedoor.POWERED))
						.with(BaseSlidedoor.HALF, DoubleBlockHalf.UPPER)
						.with(BaseSlidedoor.WATERLOGGED, state.get(BaseSlidedoor.WATERLOGGED)), 3);
				iworld.setBlockState(pos.down(), Slidedoor_Blocks.FUSUMA_pink.getDefaultState()
						.with(BaseSlidedoor.H_FACING, state.get(BaseSlidedoor.H_FACING))
						.with(BaseSlidedoor.OPEN, state.get(BaseSlidedoor.OPEN))
						.with(BaseSlidedoor.HINGE, state.get(BaseSlidedoor.HINGE))
						.with(BaseSlidedoor.POWERED, state.get(BaseSlidedoor.POWERED))
						.with(BaseSlidedoor.HALF, DoubleBlockHalf.LOWER)
						.with(BaseSlidedoor.WATERLOGGED, state.get(BaseSlidedoor.WATERLOGGED)), 3); }

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		if (block instanceof Fusuma_B && block != Slidedoor_Blocks.FUSUMAB_pink) {

			if (state.get(BaseSlidedoor.HALF) == DoubleBlockHalf.LOWER) {
				iworld.setBlockState(pos, Slidedoor_Blocks.FUSUMAB_pink.getDefaultState()
						.with(BaseSlidedoor.H_FACING, state.get(BaseSlidedoor.H_FACING))
						.with(BaseSlidedoor.OPEN, state.get(BaseSlidedoor.OPEN))
						.with(BaseSlidedoor.HINGE, state.get(BaseSlidedoor.HINGE))
						.with(BaseSlidedoor.POWERED, state.get(BaseSlidedoor.POWERED))
						.with(BaseSlidedoor.HALF, DoubleBlockHalf.LOWER)
						.with(BaseSlidedoor.WATERLOGGED, state.get(BaseSlidedoor.WATERLOGGED)), 3);
				iworld.setBlockState(pos.up(), Slidedoor_Blocks.FUSUMAB_pink.getDefaultState()
						.with(BaseSlidedoor.H_FACING, state.get(BaseSlidedoor.H_FACING))
						.with(BaseSlidedoor.OPEN, state.get(BaseSlidedoor.OPEN))
						.with(BaseSlidedoor.HINGE, state.get(BaseSlidedoor.HINGE))
						.with(BaseSlidedoor.POWERED, state.get(BaseSlidedoor.POWERED))
						.with(BaseSlidedoor.HALF, DoubleBlockHalf.UPPER)
						.with(BaseSlidedoor.WATERLOGGED, state.get(BaseSlidedoor.WATERLOGGED)), 3); }

			if (state.get(BaseSlidedoor.HALF) == DoubleBlockHalf.UPPER) {
				iworld.setBlockState(pos, Slidedoor_Blocks.FUSUMAB_pink.getDefaultState()
						.with(BaseSlidedoor.H_FACING, state.get(BaseSlidedoor.H_FACING))
						.with(BaseSlidedoor.OPEN, state.get(BaseSlidedoor.OPEN))
						.with(BaseSlidedoor.HINGE, state.get(BaseSlidedoor.HINGE))
						.with(BaseSlidedoor.POWERED, state.get(BaseSlidedoor.POWERED))
						.with(BaseSlidedoor.HALF, DoubleBlockHalf.UPPER)
						.with(BaseSlidedoor.WATERLOGGED, state.get(BaseSlidedoor.WATERLOGGED)), 3);
				iworld.setBlockState(pos.down(), Slidedoor_Blocks.FUSUMAB_pink.getDefaultState()
						.with(BaseSlidedoor.H_FACING, state.get(BaseSlidedoor.H_FACING))
						.with(BaseSlidedoor.OPEN, state.get(BaseSlidedoor.OPEN))
						.with(BaseSlidedoor.HINGE, state.get(BaseSlidedoor.HINGE))
						.with(BaseSlidedoor.POWERED, state.get(BaseSlidedoor.POWERED))
						.with(BaseSlidedoor.HALF, DoubleBlockHalf.LOWER)
						.with(BaseSlidedoor.WATERLOGGED, state.get(BaseSlidedoor.WATERLOGGED)), 3); }

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		///////////////////////
		/** 鴨居 **/
		if (block instanceof KamoiPlaster_Oak && block != KamoiPlaster_Blocks.KAMOI_pink_oak) {

			iworld.setBlockState(pos, KamoiPlaster_Blocks.KAMOI_pink_oak.getDefaultState()
					.with(Base_KamoiPlaster.H_FACING, state.get(Base_KamoiPlaster.H_FACING))
					.with(Base_KamoiPlaster.STAGE_1_4, state.get(Base_KamoiPlaster.STAGE_1_4)), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		if (block instanceof KamoiPlaster_Spruce && block != KamoiPlaster_Blocks.KAMOI_pink_spru) {

			iworld.setBlockState(pos, KamoiPlaster_Blocks.KAMOI_pink_spru.getDefaultState()
					.with(Base_KamoiPlaster.H_FACING, state.get(Base_KamoiPlaster.H_FACING))
					.with(Base_KamoiPlaster.STAGE_1_4, state.get(Base_KamoiPlaster.STAGE_1_4)), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		if (block instanceof KamoiPlaster_Birch && block != KamoiPlaster_Blocks.KAMOI_pink_bir) {

			iworld.setBlockState(pos, KamoiPlaster_Blocks.KAMOI_pink_bir.getDefaultState()
					.with(Base_KamoiPlaster.H_FACING, state.get(Base_KamoiPlaster.H_FACING))
					.with(Base_KamoiPlaster.STAGE_1_4, state.get(Base_KamoiPlaster.STAGE_1_4)), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		if (block instanceof KamoiPlaster_Jungle && block != KamoiPlaster_Blocks.KAMOI_pink_jun) {

			iworld.setBlockState(pos, KamoiPlaster_Blocks.KAMOI_pink_jun.getDefaultState()
					.with(Base_KamoiPlaster.H_FACING, state.get(Base_KamoiPlaster.H_FACING))
					.with(Base_KamoiPlaster.STAGE_1_4, state.get(Base_KamoiPlaster.STAGE_1_4)), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		if (block instanceof KamoiPlaster_Acacia && block != KamoiPlaster_Blocks.KAMOI_pink_aca) {

			iworld.setBlockState(pos, KamoiPlaster_Blocks.KAMOI_pink_aca.getDefaultState()
					.with(Base_KamoiPlaster.H_FACING, state.get(Base_KamoiPlaster.H_FACING))
					.with(Base_KamoiPlaster.STAGE_1_4, state.get(Base_KamoiPlaster.STAGE_1_4)), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		if (block instanceof KamoiPlaster_DarkOak && block != KamoiPlaster_Blocks.KAMOI_pink_doak) {

			iworld.setBlockState(pos, KamoiPlaster_Blocks.KAMOI_pink_doak.getDefaultState()
					.with(Base_KamoiPlaster.H_FACING, state.get(Base_KamoiPlaster.H_FACING))
					.with(Base_KamoiPlaster.STAGE_1_4, state.get(Base_KamoiPlaster.STAGE_1_4)), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		if (block instanceof KamoiPlaster_Sakura && block != KamoiPlaster_Blocks.KAMOI_pink_sakura) {

			iworld.setBlockState(pos, KamoiPlaster_Blocks.KAMOI_pink_sakura.getDefaultState()
					.with(Base_KamoiPlaster.H_FACING, state.get(Base_KamoiPlaster.H_FACING))
					.with(Base_KamoiPlaster.STAGE_1_4, state.get(Base_KamoiPlaster.STAGE_1_4)), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		if (block instanceof KamoiPlaster_Kaede && block != KamoiPlaster_Blocks.KAMOI_pink_kaede) {

			iworld.setBlockState(pos, KamoiPlaster_Blocks.KAMOI_pink_kaede.getDefaultState()
					.with(Base_KamoiPlaster.H_FACING, state.get(Base_KamoiPlaster.H_FACING))
					.with(Base_KamoiPlaster.STAGE_1_4, state.get(Base_KamoiPlaster.STAGE_1_4)), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		if (block instanceof KamoiPlaster_Ichoh && block != KamoiPlaster_Blocks.KAMOI_pink_ichoh) {

			iworld.setBlockState(pos, KamoiPlaster_Blocks.KAMOI_pink_ichoh.getDefaultState()
					.with(Base_KamoiPlaster.H_FACING, state.get(Base_KamoiPlaster.H_FACING))
					.with(Base_KamoiPlaster.STAGE_1_4, state.get(Base_KamoiPlaster.STAGE_1_4)), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}


		///////////////////////
		/** 粘土 **/
		if (block instanceof WallPane_Clay && block != WallPanel_Blocks.WP_CLAY_pink) {

			iworld.setBlockState(pos, WallPanel_Blocks.WP_CLAY_pink.getDefaultState()
					.with(BaseFacingWater.H_FACING, state.get(BaseFacingWater.H_FACING))
					.with(BaseFacingWater.WATERLOGGED, state.get(BaseFacingWater.WATERLOGGED)), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		/** ガラス **/
		if (block instanceof WallPane_Glass && block != WallPanel_Blocks.WP_GLASS_pink) {

			iworld.setBlockState(pos, WallPanel_Blocks.WP_GLASS_pink.getDefaultState()
					.with(BaseFacingWater.H_FACING, state.get(BaseFacingWater.H_FACING))
					.with(BaseFacingWater.WATERLOGGED, state.get(BaseFacingWater.WATERLOGGED)), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		/** しっくい **/
		if (block instanceof WallPane_Plaster && block != WallPanel_Blocks.WP_PLASTER_pink) {

			iworld.setBlockState(pos, WallPanel_Blocks.WP_PLASTER_pink.getDefaultState()
					.with(BaseStage2_FaceWater.H_FACING, state.get(BaseStage2_FaceWater.H_FACING))
					.with(BaseStage2_FaceWater.STAGE_1_2, state.get(BaseStage2_FaceWater.STAGE_1_2))
					.with(BaseStage2_FaceWater.WATERLOGGED, state.get(BaseStage2_FaceWater.WATERLOGGED)), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		/** なまこ **/
		if (block instanceof WallPane_Namako && block != WallPanel_Blocks.WP_NAMAKO_pink) {

			iworld.setBlockState(pos, WallPanel_Blocks.WP_NAMAKO_pink.getDefaultState()
					.with(BaseStage2_FaceWater.H_FACING, state.get(BaseStage2_FaceWater.H_FACING))
					.with(BaseStage2_FaceWater.STAGE_1_2, state.get(BaseStage2_FaceWater.STAGE_1_2))
					.with(BaseStage2_FaceWater.WATERLOGGED, state.get(BaseStage2_FaceWater.WATERLOGGED)), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		if (block instanceof WallPane_Namako_B && block != WallPanel_Blocks.WP_NAMAKOB_pink) {

			iworld.setBlockState(pos, WallPanel_Blocks.WP_NAMAKOB_pink.getDefaultState()
					.with(BaseStage2_FaceWater.H_FACING, state.get(BaseStage2_FaceWater.H_FACING))
					.with(BaseStage2_FaceWater.STAGE_1_2, state.get(BaseStage2_FaceWater.STAGE_1_2))
					.with(BaseStage2_FaceWater.WATERLOGGED, state.get(BaseStage2_FaceWater.WATERLOGGED)), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}

		///6.1.2////////////////////
		if (block instanceof Wall_Plaster && block != JP_Blocks.SHIKKUI_WALL_pink && block != JP_Blocks.DIRTWALL_WALL) {
			
			iworld.setBlockState(pos, JP_Blocks.SHIKKUI_WALL_pink.getDefaultState()
					.with(Wall_Plaster.NORTH, state.get(Wall_Plaster.NORTH))
					.with(Wall_Plaster.SOUTH, state.get(Wall_Plaster.SOUTH))
					.with(Wall_Plaster.EAST, state.get(Wall_Plaster.EAST))
					.with(Wall_Plaster.WEST, state.get(Wall_Plaster.WEST))
					.with(Wall_Plaster.CRACK, state.get(Wall_Plaster.CRACK))
					.with(Wall_Plaster.WATERLOGGED, state.get(Wall_Plaster.WATERLOGGED)), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}
		
		if (block instanceof Wall_Sama && block != JP_Blocks.SHIKKUI_SAMA_pink && block != JP_Blocks.DIRTWALL_SAMA) {
			
			iworld.setBlockState(pos, JP_Blocks.SHIKKUI_SAMA_pink.getDefaultState()
					.with(Wall_Sama.H_FACING, state.get(Wall_Sama.H_FACING))
					.with(Wall_Sama.STAGE_1_4, state.get(Wall_Sama.STAGE_1_4))
					.with(Wall_Sama.WATERLOGGED, state.get(Wall_Sama.WATERLOGGED)), 3);
			
			if (state.get(Wall_Sama.STAGE_1_4) <= 2) {
				if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }
			}
			
			if (state.get(Wall_Sama.STAGE_1_4) >= 3) {
				if (playerIn != null) { Abstract_Hake.consumeAndBreak(2, hStack, playerIn, context); }
			}

			return ActionResultType.SUCCESS;
		}

		if (block instanceof Wall_Kawara && block != JP_Blocks.KAWARA_WALL_pink) {
			
			iworld.setBlockState(pos, JP_Blocks.KAWARA_WALL_pink.getDefaultState()
					.with(Wall_Kawara.NORTH, state.get(Wall_Kawara.NORTH))
					.with(Wall_Kawara.SOUTH, state.get(Wall_Kawara.SOUTH))
					.with(Wall_Kawara.EAST, state.get(Wall_Kawara.EAST))
					.with(Wall_Kawara.WEST, state.get(Wall_Kawara.WEST))
					.with(Wall_Kawara.WATERLOGGED, state.get(Wall_Kawara.WATERLOGGED)), 3);
			
			if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}
		
		if (block instanceof Wall_Namako && block != JP_Blocks.NAMAKO_WALL_pink) {
			
			iworld.setBlockState(pos, JP_Blocks.NAMAKO_WALL_pink.getDefaultState()
					.with(Wall_Namako.NORTH, state.get(Wall_Namako.NORTH))
					.with(Wall_Namako.SOUTH, state.get(Wall_Namako.SOUTH))
					.with(Wall_Namako.EAST, state.get(Wall_Namako.EAST))
					.with(Wall_Namako.WEST, state.get(Wall_Namako.WEST))
					.with(Wall_Namako.CRACK, state.get(Wall_Namako.CRACK))
					.with(Wall_Namako.WATERLOGGED, state.get(Wall_Namako.WATERLOGGED)), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}
		
		if (block instanceof Wall_NamakoB && block != JP_Blocks.NAMAKOB_WALL_pink) {
			
			iworld.setBlockState(pos, JP_Blocks.NAMAKOB_WALL_pink.getDefaultState()
					.with(Wall_NamakoB.NORTH, state.get(Wall_NamakoB.NORTH))
					.with(Wall_NamakoB.SOUTH, state.get(Wall_NamakoB.SOUTH))
					.with(Wall_NamakoB.EAST, state.get(Wall_NamakoB.EAST))
					.with(Wall_NamakoB.WEST, state.get(Wall_NamakoB.WEST))
					.with(Wall_NamakoB.CRACK, state.get(Wall_NamakoB.CRACK))
					.with(Wall_NamakoB.WATERLOGGED, state.get(Wall_NamakoB.WATERLOGGED)), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}
		
		///6.2.4////////////////////
		if (block instanceof Curtain && block != Window_Blocks.CURTAIN_pink) {
			
			iworld.setBlockState(pos, Window_Blocks.CURTAIN_pink.getDefaultState()
					.with(Curtain.H_FACING, state.get(Wall_Sama.H_FACING))
					.with(Curtain.STAGE_1_4, state.get(Wall_Sama.STAGE_1_4))
					.with(Curtain.WATERLOGGED, state.get(Wall_Sama.WATERLOGGED)), 3);

			if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}
		
		if (block instanceof CurtainTall && block != Window_Blocks.CURTAINTALL_pink) {

			if (state.get(CurtainTall.HALF) == DoubleBlockHalf.LOWER) {
				iworld.setBlockState(pos, Window_Blocks.CURTAINTALL_pink.getDefaultState()
						.with(CurtainTall.H_FACING, state.get(CurtainTall.H_FACING))
						.with(CurtainTall.OPEN, state.get(CurtainTall.OPEN))
						.with(CurtainTall.HINGE, state.get(CurtainTall.HINGE))
						.with(CurtainTall.HALF, DoubleBlockHalf.LOWER)
						.with(CurtainTall.WATERLOGGED, state.get(CurtainTall.WATERLOGGED)), 3);
				iworld.setBlockState(pos.up(), Window_Blocks.CURTAINTALL_pink.getDefaultState()
						.with(CurtainTall.H_FACING, state.get(CurtainTall.H_FACING))
						.with(CurtainTall.OPEN, state.get(CurtainTall.OPEN))
						.with(CurtainTall.HINGE, state.get(CurtainTall.HINGE))
						.with(CurtainTall.HALF, DoubleBlockHalf.UPPER)
						.with(CurtainTall.WATERLOGGED, state.get(CurtainTall.WATERLOGGED)), 3); }
			
			if (state.get(CurtainTall.HALF) == DoubleBlockHalf.UPPER) {
				iworld.setBlockState(pos, Window_Blocks.CURTAINTALL_pink.getDefaultState()
						.with(CurtainTall.H_FACING, state.get(CurtainTall.H_FACING))
						.with(CurtainTall.OPEN, state.get(CurtainTall.OPEN))
						.with(CurtainTall.HINGE, state.get(CurtainTall.HINGE))
						.with(CurtainTall.HALF, DoubleBlockHalf.UPPER)
						.with(CurtainTall.WATERLOGGED, state.get(CurtainTall.WATERLOGGED)), 3);
				iworld.setBlockState(pos.down(), Window_Blocks.CURTAINTALL_pink.getDefaultState()
						.with(CurtainTall.H_FACING, state.get(CurtainTall.H_FACING))
						.with(CurtainTall.OPEN, state.get(CurtainTall.OPEN))
						.with(CurtainTall.HINGE, state.get(CurtainTall.HINGE))
						.with(CurtainTall.HALF, DoubleBlockHalf.LOWER)
						.with(CurtainTall.WATERLOGGED, state.get(CurtainTall.WATERLOGGED)), 3); }
			
			if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }
			return ActionResultType.SUCCESS;
		}
		
		if (block instanceof DeskCloth) {
			int color = state.get(DeskCloth.STAGE_1_4);
			if (block != Furniture_Blocks.DESKCLOTH_47 || color != 3) {

				iworld.setBlockState(pos, Furniture_Blocks.DESKCLOTH_47.getDefaultState()
						.with(DeskCloth.H_FACING, state.get(DeskCloth.H_FACING))
						.with(DeskCloth.STAGE_1_4, Integer.valueOf(3))
						.with(DeskCloth.WATERLOGGED, state.get(DeskCloth.WATERLOGGED)), 3);
				
				if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }				
				return ActionResultType.SUCCESS; }
		}
		
		if (block instanceof BaseClothTable) {
			int color = state.get(BaseClothTable.STAGE_0_15);
			if (color != 6) {

				iworld.setBlockState(pos, state.with(BaseClothTable.STAGE_0_15, Integer.valueOf(6)), 3);//PINK
				
				if (playerIn != null) { Abstract_Hake.consumeAndBreak(1, hStack, playerIn, context); }				
				return ActionResultType.SUCCESS; }
		}
		
		/* Wash the brush. MIZUOKE work is in the block. */
		if (block == Blocks.CAULDRON) {
			int level = state.get(CauldronBlock.LEVEL);
			if (level != 0) {
				iworld.setBlockState(pos, state.with(CauldronBlock.LEVEL, Integer.valueOf(level - 1)), 3);
				CMEvents.washHAKE_Cauldron(iworld, pos, hStack, playerIn);
				return ActionResultType.SUCCESS;
			}
		}
		return ActionResultType.FAIL;
	}
}

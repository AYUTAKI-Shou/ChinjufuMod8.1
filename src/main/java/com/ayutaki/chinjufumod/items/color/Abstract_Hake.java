...
public abstract class Abstract_Hake extends IG_Wadeco {
	...
	public Abstract_Hake(Properties props) {
		super(props.maxStackSize(1).maxDamage(128));
	} //127
	...
	public static void consumeAndBreak(int damage, ItemStack hStack, PlayerEntity playerIn, ItemUseContext context) {
		...
		hStack.damageItem(mode? 0 : damage, playerIn, user -> {
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> itemTip, ITooltipFlag tipFlag) {
		itemTip.add(new TranslationTextComponent("tips.item_hake_color").applyTextStyle(TextFormatting.GRAY));
	}
}

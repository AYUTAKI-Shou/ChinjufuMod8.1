...
public class Device_Radar extends Abstract_Device {
	...
	protected boolean waterCheck(LivingEntity entityLiving) {
		return !entityLiving.isEyeInFluid(FluidTags.WATER) &&
				 !entityLiving.isEyeInFluid(FluidTags.LAVA);
	}
	...
	public void appendHoverText(ItemStack stack, @Nullable World worldIn, List<ITextComponent> itemTip, ITooltipFlag tipFlag) {
		itemTip.add(new TranslationTextComponent("tips.item_device_radar").withStyle(TextFormatting.GRAY));
		itemTip.add(new TranslationTextComponent("tips.item_device_radar2").withStyle(TextFormatting.RED));
	}
}

...
public class Device_Radar extends Abstract_Device {

	public Device_Radar(Item.Properties props) {
		super(props);
	}

	protected boolean GISOU(Player playerIn) {
		return ShipTypes_CM.typeDestroyer(playerIn) || ShipTypes_CM.typeCruiser(playerIn) || 
				ShipTypes_CM.typeBattleship(playerIn) || ShipTypes_CM.typeCarrier(playerIn);
	}
	
	protected float pitchSE() { return 1.25F; }

	protected boolean waterCheck(LivingEntity entityLiving) {
		return !entityLiving.isEyeInFluidType(NeoForgeMod.WATER_TYPE.value()) &&
				!entityLiving.isEyeInFluidType(NeoForgeMod.LAVA_TYPE.value());
	}
	...
	public InteractionResult useOn(UseOnContext context) {
		...
		if (this.GISOU(playerIn)) {
			if (playerIn.isEyeInFluidType(NeoForgeMod.WATER_TYPE.value())) {
				...
				if (playerIn.getCooldowns().isOnCooldown(hStack)) { return InteractionResult.PASS; }
				
				else {
					/** Rador Click UP_DOWN **/
					int ms = 80; //128/340 376ms, *1.5/7
					...
					case UP :
						...
						this.coolDown(worldIn, playerIn, hStack);
	...
	public InteractionResult use(Level worldIn, Player playerIn, InteractionHand hand) {
		...
		if (this.GISOU(playerIn)) {
			if (playerIn.isEyeInFluidType(NeoForgeMod.WATER_TYPE.value())) {
				...
				if (playerIn.getCooldowns().isOnCooldown(hStack)) { return InteractionResult.FAIL; }
	...
	public void appendHoverText(ItemStack stack, Item.TooltipContext context, List<Component> itemTip, TooltipFlag tipFlag) {
		itemTip.add(Component.translatable("tips.item_device_radar").withStyle(ChatFormatting.GRAY));
		itemTip.add(Component.translatable("tips.item_device_radar2").withStyle(ChatFormatting.RED));
	}
}

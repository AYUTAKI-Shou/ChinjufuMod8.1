...
public class Device_Sonar extends Abstract_Device {
	...
	private void text_Tips2(World worldIn, PlayerEntity playerIn) {
		...
		playerIn.displayClientMessage(new TranslationTextComponent("tips.item_device_sonar2"), true);
	}
				...
				if (raytraceresult.getType() != RayTraceResult.Type.BLOCK) { 
					...
					if (state.getBlock() instanceof FlowingFluidBlock) {
						...
						boolean WATER_UP = (state.getMaterial() == Material.WATER && 
								((Integer)state.getValue(FlowingFluidBlock.LEVEL)).intValue() == 0 && blockResult.getDirection() == Direction.UP);
						if (WATER_UP) {
							if (playerIn.getCooldowns().isOnCooldown(this)) { return ActionResult.fail(hStack); }
	...
	public void appendHoverText(ItemStack stack, @Nullable World worldIn, List<ITextComponent> itemTip, ITooltipFlag tipFlag) {
		itemTip.add(new TranslationTextComponent("tips.item_device_sonar").withStyle(TextFormatting.GRAY));
		itemTip.add(new TranslationTextComponent("tips.item_device_sonar2").withStyle(TextFormatting.GRAY));
		itemTip.add(new TranslationTextComponent("tips.item_device_sonar3").withStyle(TextFormatting.BLUE));
	}
}

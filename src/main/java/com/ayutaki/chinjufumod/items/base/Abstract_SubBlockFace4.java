...
public abstract class Abstract_SubBlockFace4 extends Item_Regi {

	public Abstract_SubBlockFace4(String name) {
		super(name);
		/** Have sub items. **/
		setHasSubtypes(true);
	}
	...
	public EnumActionResult onItemUse(EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		if (!hStack.isEmpty() && playerIn.canPlayerEdit(pos, facing, hStack) && worldIn.mayPlace(this.int2Block(k), pos, false, facing, (Entity)null)) {
			...
			IBlockState putSTATE = this.int2Block(k).getDefaultState().withProperty(BaseStage4_Face.H_FACING, direction)

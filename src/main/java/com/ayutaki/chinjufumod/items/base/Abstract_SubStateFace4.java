...
public abstract class Abstract_SubStateFace4 extends ItemBlock_Regi {

	public Abstract_SubStateFace4(String name, Block putBlock) {
		super(name, putBlock);
		/** Have sub items. **/
		setHasSubtypes(true);
	}
	...
	public EnumActionResult onItemUse(EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		if (!hStack.isEmpty() && playerIn.canPlayerEdit(pos, facing, hStack) && worldIn.mayPlace(this.takeBlock(), pos, false, facing, (Entity)null)) {
			...
			IBlockState putSTATE = this.takeBlock().getDefaultState().withProperty(BaseStage4_Face.H_FACING, direction)

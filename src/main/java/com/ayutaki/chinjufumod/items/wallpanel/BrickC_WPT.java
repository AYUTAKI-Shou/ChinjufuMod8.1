...
public class BrickC_WPT extends ItemBlock_Regi {

	public BrickC_WPT(String name) {
	...
	public EnumActionResult onItemUse(EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		if (!hStack.isEmpty() && playerIn.canPlayerEdit(pos, facing, hStack) && worldIn.mayPlace(WallBrick_Blocks.ROCK, pos, false, facing, (Entity)null)) {
			...
			IBlockState putSTATE = WallBrick_Blocks.ROCK.getDefaultState().withProperty(Bricks_CM.STAGE_1_14, Integer.valueOf(k));

...
public class Dish_Kanten extends BaseDishAlways {
	...
	protected void addEffect(LivingEntity entityLiving) {
		...
		if (this == Items_Teatime.KANTEN_CITRUS) {
			entityLiving.addPotionEffect(new EffectInstance(Effects.FIRE_RESISTANCE, 3600, 0)); }
		
		if (this == Items_Teatime.KANTEN_GRAPE) {
			entityLiving.addPotionEffect(new EffectInstance(Effects.NIGHT_VISION, 3600, 0)); }
		...
		if (this == Items_Teatime.YOKAN_MATCHA) {
			entityLiving.addPotionEffect(new EffectInstance(Effects.HASTE, 3800, 0)); }
	}

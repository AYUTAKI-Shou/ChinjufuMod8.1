...
public abstract class TTab_DishEat extends Food_Teatime {

	public TTab_DishEat(String name, int amount, float saturation, boolean isWolfFood) {
	...
	public ItemStack onItemUseFinish(ItemStack stack, World worldIn, EntityLivingBase entityLiving) {
		...
		if (playerIn == null || !playerIn.capabilities.isCreativeMode) {
			...
			if (stack.isEmpty()) { return this.remainStack(); }
			else if (!playerIn.inventory.addItemStackToInventory(this.remainStack())) { playerIn.dropItem(this.remainStack(), false); }
			
			if (this == Items_Teatime.PASTASEAFOOD) {
				ItemStack shell = new ItemStack(Items_NoTab.HAMAGURI_KARA, 3, 0);

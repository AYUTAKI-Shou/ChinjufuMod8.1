...
public class CurryNabe_TT extends IBR_Teatime {

	public CurryNabe_TT(String name) {
		super(name, Dish_Blocks.ZUNDOU_CURRY);
		/** Have sub items. **/
		setHasSubtypes(true);
	}
	...
	public EnumActionResult onItemUse(EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		if (!hStack.isEmpty() && playerIn.canPlayerEdit(pos, facing, hStack) && worldIn.mayPlace(int2Block(k), pos, false, facing, (Entity)null)) {
			...
			IBlockState putSTATE = this.int2Block(k).getDefaultState()
					.withProperty(Zundou4_Curry.H_FACING, direction)
					.withProperty(Zundou4_Curry.STAGE_1_4, Integer.valueOf(1));
	...
	private Block int2Block(int k) {
		if (k == 0) { return Dish_Blocks.ZUNDOU_CURRY; }
		if (k == 1) { return Dish_Blocks.ZUNDOU_CURRY_C; }
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> itemTip, ITooltipFlag advanced) {
		itemTip.add(I18n.format("tips.block_food_cunabe_1.name"));
	}
}

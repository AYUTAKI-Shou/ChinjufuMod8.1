...
public class Teppan_TT extends IR_Teatime {

	public Teppan_TT(String name) {
		super(name);
		/** Have sub items. **/
		setHasSubtypes(true);
	}
	...
	public EnumActionResult onItemUse(EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		if (k == 1 && playerIn.canPlayerEdit(pos, facing, hStack) && this.int2Block(k).canPlaceBlockAt(worldIn, pos)) {
			
			worldIn.setBlockState(pos, this.int2Block(k).getDefaultState()
					.withProperty(Base_CookPanStage4.H_FACING, direction)
					.withProperty(Base_CookPanStage4.STAGE_1_4, Integer.valueOf(1)), 2);
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> itemTip, ITooltipFlag advanced) {
		...
		if (k == 1) { itemTip.add(I18n.format("tips.block_food_frypan_osauce.name")); }
		if (k != 1) { itemTip.add(I18n.format("tips.take_plate.name")); } 
	}
}

...
public abstract class WT_SubWall extends IBR_Wablock {

	public WT_SubWall(String name, Block putBlock) {
		super(name, putBlock);
		/** Have sub items. **/
		setHasSubtypes(true);
	}
	...
	public EnumActionResult onItemUse(EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		if (!hStack.isEmpty() && playerIn.canPlayerEdit(pos, facing, hStack) && worldIn.mayPlace(this.takeBlock(), pos, false, facing, (Entity)null)) {
			...
			IBlockState putSTATE = this.takeBlock().getDefaultState().withProperty(Base_Wall.STAGE_0_15, Integer.valueOf(k));

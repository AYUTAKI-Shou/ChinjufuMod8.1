...
public class Mizuoke_TT extends BucketItem {
	...
	public Mizuoke_TT(Fluid containedFluidIn, Block blockIn, Item.Properties props) {
		super(containedFluidIn, props.group(ItemGroups_CM.TEATIME));
		this.block = blockIn;
	...
	@Override
	public int getBurnTime(ItemStack hStack) {
		Item hItem = hStack.getItem();

		if (hItem == Items_Teatime.MIZUOKE) { return 100; }
		else { return 0; }
	}

	private void MIZUOKE_toFull(World worldIn, BlockPos pos, PlayerEntity playerIn, Hand hand) {
		CMEvents.take1Item(playerIn, hand, Items_Teatime.MIZUOKE_full);
	...
	public ActionResult<ItemStack> onItemRightClick(World worldIn, PlayerEntity playerIn, Hand hand) {
		...
		if (raytraceresult.getType() == RayTraceResult.Type.MISS) { return ActionResult.resultPass(hStack); }

		if (raytraceresult.getType() != RayTraceResult.Type.BLOCK) { return ActionResult.resultPass(hStack); }
					...
					if (this.containedBlock == Fluids.EMPTY) {
						...
						if (stateIn1.getBlock() == Blocks.CAULDRON) {
							...
							if (cauldron != 3) {
								...
								((CauldronBlock)block).setWaterLevel(worldIn, pos, stateIn1, 3);
							...
							if (this.emptyBucket(playerIn, worldIn, pos2, blockResult)) {
								this.checkExtraContent(worldIn, hStack, pos2);

								if (playerIn instanceof ServerPlayerEntity) {
									CriteriaTriggers.PLACED_BLOCK.trigger((ServerPlayerEntity)playerIn, pos2, hStack); }
								playerIn.addStat(Stats.ITEM_USED.get(this));
								
								CMEvents.mode1Through_Consume(playerIn, hand, Items_Teatime.MIZUOKE);
								return ActionResult.resultSuccess(hStack); }

							else { return ActionResult.resultPass(hStack); }
						}
					}//water
	...
	public boolean itemInteractionForEntity(ItemStack stack, net.minecraft.entity.player.PlayerEntity playerIn, LivingEntity entity, net.minecraft.util.Hand hand) {
		...
		if (stack.getItem() == Items_Teatime.MIZUOKE) {
			...
			if (entity instanceof CowEntity && !mode && !entity.isChild()) {
				...
				CMEvents.take1Item(playerIn, hand, Items_Teatime.MIZUOKE_Milk);
				/* 消費を最後に回す */
				stack.shrink(1);
				return true;
			}
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> itemTip, ITooltipFlag tipFlag) {
		itemTip.add(new TranslationTextComponent("tips.block_mizuoke").applyTextStyle(TextFormatting.GRAY));

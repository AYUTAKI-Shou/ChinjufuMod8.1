...
public class Plate_DT extends IR_Wadeco {

	public Plate_DT(String name) {
		super(name);
		/** Have sub items. **/
		setHasSubtypes(true);
	}
	...
	public String getUnlocalizedName(ItemStack stack) {
		...
		case 0:
		default:
			return "item." + "block_bamboo_plate";
		case 1:
			return "item." + "block_bamboo_y_plate";
		case 2:
			return "item." + "block_bamboo_k_plate";
		}
	}
	...
	public EnumActionResult onItemUse(EntityPlayer playerIn, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		...
		if (!hStack.isEmpty() && playerIn.canPlayerEdit(pos, facing, hStack) && worldIn.mayPlace(int2Block(k), pos, false, facing, (Entity)null)) {
			...
			IBlockState putSTATE = this.int2Block(k).getDefaultState().withProperty(BambooPressurePlate.H_FACING, direction);
	...
	private Block int2Block(int k) {
		if (k == 0) { return JPDeco_Blocks.TAKE_PLATE; }
		if (k == 1) { return JPDeco_Blocks.TAKE_PLATE_Y; }
		else { return JPDeco_Blocks.TAKE_PLATE_K; }
	}
}

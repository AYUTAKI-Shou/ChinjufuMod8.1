...
public class SB_Seiran extends Abstract_Kansaiki {

	public SB_Seiran(String name, int max) {
		super(name, max);
	}
		...
		if (entityLiving instanceof EntityPlayer) {
			...
			if (!fuel.isEmpty() || mode) {
				...
				if (!((double)charge < 0.1D)) {
					...
					if (!worldIn.isRemote) {
						SB_SeiranEntity kansaiki = new SB_SeiranEntity(worldIn, playerIn, stack);
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> itemTip, ITooltipFlag advanced) {
		...
		itemTip.add(TextFormatting.GREEN + I18n.format("tips.item_kk_11f.name"));

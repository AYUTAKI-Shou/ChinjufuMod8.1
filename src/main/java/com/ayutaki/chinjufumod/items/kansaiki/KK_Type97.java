...
public class KK_Type97 extends Abstract_BowKansaiki implements ProjectileItem {
	
	public KK_Type97(Item.Properties props) {
		super(props);
	}
			...
			if (!fuel.isEmpty() || mode) {
				...
				if (!((double)charge < 0.1D)) {
					...
					if (worldIn instanceof ServerLevel server) {
						KK_Type97Entity kansaiki = new KK_Type97Entity(playerIn, server, stack);
	...
	public Projectile asProjectile(Level worldIn, Position pos, ItemStack stack, Direction direct) {
		return new KK_Type97Entity(worldIn, pos.x(), pos.y(), pos.z(), stack);
	}
}

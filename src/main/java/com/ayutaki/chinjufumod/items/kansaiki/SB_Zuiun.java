...
public class SB_Zuiun extends Abstract_BowKansaiki {

	public SB_Zuiun(Properties props) {
		super(props);
	}
		...
		if (entityLiving instanceof PlayerEntity) {
			...
			if (!fuel.isEmpty() || mode) {
				...
				if (!((double)charge < 0.1D)) {
					...
					if (!worldIn.isRemote) {
						SB_ZuiunEntity kansaiki = new SB_ZuiunEntity(playerIn, worldIn, stack);
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> itemTip, ITooltipFlag tipFlag) {
		...
		itemTip.add(new TranslationTextComponent("tips.item_kk_7f").applyTextStyle(TextFormatting.GREEN));
		itemTip.add(new TranslationTextComponent("tips.item_sb").applyTextStyle(TextFormatting.GRAY)); }
}

...
public class SB_Zuiun extends Abstract_BowKansaiki implements ProjectileItem {
	
	public SB_Zuiun(Item.Properties props) {
		super(props);
	}
			...
			if (!fuel.isEmpty() || mode) {
				...
				if (!((double)charge < 0.1D)) {
					...
					if (worldIn instanceof ServerLevel server) {
						SB_ZuiunEntity kansaiki = new SB_ZuiunEntity(playerIn, server, stack);
	...
	public void appendHoverText(ItemStack stack, Item.TooltipContext context, List<Component> itemTip, TooltipFlag tipFlag) {
		...
		itemTip.add(Component.translatable("tips.item_kk_7f").withStyle(ChatFormatting.GREEN));
		itemTip.add(Component.translatable("tips.item_sb").withStyle(ChatFormatting.GRAY)); }

	@Override
	public Projectile asProjectile(Level worldIn, Position pos, ItemStack stack, Direction direct) {
		return new SB_ZuiunEntity(worldIn, pos.x(), pos.y(), pos.z(), stack);
	}
}

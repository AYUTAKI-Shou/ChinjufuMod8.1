...
public class KK_Ryusei extends Abstract_BowKansaiki {
			...
			if (!fuel.isEmpty() || mode) {
				...
				if (!((double)charge < 0.1D)) {
					...
					if (!worldIn.isClientSide) {
						KK_RyuseiEntity kansaiki = new KK_RyuseiEntity(playerIn, worldIn, stack);
	...
	public void appendHoverText(ItemStack stack, @Nullable World worldIn, List<ITextComponent> itemTip, ITooltipFlag tipFlag) {
		...
		itemTip.add(new TranslationTextComponent("tips.item_kk_10f").withStyle(TextFormatting.BLUE)); }
}

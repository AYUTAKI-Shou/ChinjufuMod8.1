...
public class KB_Re2001 extends Abstract_BowKansaiki implements ProjectileItem {
			...
			if (!fuel.isEmpty() || mode) {
				...
				if (!((double)charge < 0.1D)) {
					...
					if (worldIn instanceof ServerLevel server) {
						KB_Re2001Entity kansaiki = new KB_Re2001Entity(playerIn, server, stack);
	...
	public Projectile asProjectile(Level worldIn, Position pos, ItemStack stack, Direction direct) {
		return new KB_Re2001Entity(worldIn, pos.x(), pos.y(), pos.z(), stack);
	}
}

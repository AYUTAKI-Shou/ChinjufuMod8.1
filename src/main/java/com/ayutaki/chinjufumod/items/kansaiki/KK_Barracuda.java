...
public class KK_Barracuda extends Abstract_BowKansaiki {
			...
			if (!fuel.isEmpty() || mode) {
				...
				if (!((double)charge < 0.1D)) {
					...
					if (!worldIn.isClientSide) {
						KK_BarracudaEntity kansaiki = new KK_BarracudaEntity(playerIn, worldIn, stack);
	...
	public void appendHoverText(ItemStack stack, @Nullable World worldIn, List<ITextComponent> itemTip, ITooltipFlag tipFlag) {
		...
		itemTip.add(new TranslationTextComponent("tips.item_kk_7f").withStyle(TextFormatting.BLUE)); }
}

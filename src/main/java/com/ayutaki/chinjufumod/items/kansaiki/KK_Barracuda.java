...
public class KK_Barracuda extends Abstract_Kansaiki {

	public KK_Barracuda(String name, int max) {
		super(name, max);
	}
		...
		if (entityLiving instanceof EntityPlayer) {
			...
			if (!fuel.isEmpty() || mode) {
				...
				if (!((double)charge < 0.1D)) {
					...
					if (!worldIn.isRemote) {
						KK_BarracudaEntity kansaiki = new KK_BarracudaEntity(worldIn, playerIn, stack);
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> itemTip, ITooltipFlag advanced) {
		...
		itemTip.add(TextFormatting.BLUE + I18n.format("tips.item_kk_7f.name")); }
}

...
public class KK_Swordfish extends Abstract_BowKansaiki {

	public KK_Swordfish(Properties props) {
		super(props);
	}
		...
		if (entityLiving instanceof PlayerEntity) {
			...
			if (!fuel.isEmpty() || mode) {
				...
				if (!((double)charge < 0.1D)) {
					...
					if (!worldIn.isRemote) {
						KK_SwordfishEntity kansaiki = new KK_SwordfishEntity(playerIn, worldIn, stack);
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> itemTip, ITooltipFlag tipFlag) {
		...
		itemTip.add(new TranslationTextComponent("tips.item_kk_5f").applyTextStyle(TextFormatting.BLUE)); }
}

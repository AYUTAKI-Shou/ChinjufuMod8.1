...
public class KB_Ju87 extends Abstract_BowKansaiki implements ProjectileItem {
	
	public KB_Ju87(Item.Properties props) {
		super(props);
	}
			...
			if (!fuel.isEmpty() || mode) {
				...
				if (!((double)charge < 0.1D)) {
					...
					if (worldIn instanceof ServerLevel server) {
						KB_Ju87Entity kansaiki = new KB_Ju87Entity(playerIn, server, stack);
	...
	public void appendHoverText(ItemStack stack, Item.TooltipContext context, List<Component> itemTip, TooltipFlag tipFlag) {
		...
		itemTip.add(Component.translatable("tips.item_kk_9f").withStyle(ChatFormatting.RED)); }

	@Override
	public Projectile asProjectile(Level worldIn, Position pos, ItemStack stack, Direction direct) {
		return new KB_Ju87Entity(worldIn, pos.x(), pos.y(), pos.z(), stack);
	}
}

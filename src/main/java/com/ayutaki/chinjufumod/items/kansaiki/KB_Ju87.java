...
public class KB_Ju87 extends Abstract_BowKansaiki {

	public KB_Ju87(Properties props) {
		super(props);
	}
		...
		if (entityLiving instanceof PlayerEntity) {
			...
			if (!fuel.isEmpty() || mode) {
				...
				if (!((double)charge < 0.1D)) {
					...
					if (!worldIn.isRemote) {
						KB_Ju87Entity kansaiki = new KB_Ju87Entity(playerIn, worldIn, stack);
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> itemTip, ITooltipFlag tipFlag) {
		...
		itemTip.add(new TranslationTextComponent("tips.item_kk_9f").applyTextStyle(TextFormatting.RED)); }
}

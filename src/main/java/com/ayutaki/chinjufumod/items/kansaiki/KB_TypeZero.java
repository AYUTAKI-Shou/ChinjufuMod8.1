...
public class KB_TypeZero extends Abstract_Kansaiki {

	public KB_TypeZero(String name, int max) {
		super(name, max);
	}
		...
		if (entityLiving instanceof EntityPlayer) {
			...
			if (!fuel.isEmpty() || mode) {
				...
				if (!((double)charge < 0.1D)) {
					...
					if (!worldIn.isRemote) {
						KB_TypeZeroEntity kansaiki = new KB_TypeZeroEntity(worldIn, playerIn, stack);
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> itemTip, ITooltipFlag advanced) {
		...
		itemTip.add(TextFormatting.RED + I18n.format("tips.item_kk_10f.name")); }
}

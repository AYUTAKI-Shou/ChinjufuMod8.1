...
public class KB_TypeZero extends Abstract_BowKansaiki implements ProjectileItem {
	
	public KB_TypeZero(Item.Properties props) {
		super(props);
	}
			...
			if (!fuel.isEmpty() || mode) {
				...
				if (!((double)charge < 0.1D)) {
					...
					if (worldIn instanceof ServerLevel server) {
						KB_TypeZeroEntity kansaiki = new KB_TypeZeroEntity(playerIn, server, stack);
	...
	public void appendHoverText(ItemStack stack, Item.TooltipContext context, List<Component> itemTip, TooltipFlag tipFlag) {
		...
		itemTip.add(Component.translatable("tips.item_kk_10f").withStyle(ChatFormatting.RED)); }

	@Override
	public Projectile asProjectile(Level worldIn, Position pos, ItemStack stack, Direction direct) {
		return new KB_TypeZeroEntity(worldIn, pos.x(), pos.y(), pos.z(), stack);
	}
}

...
public class KB_F4U extends Abstract_BowKansaiki implements ProjectileItem {
	
	public KB_F4U(Item.Properties props) {
		super(props);
	}
			...
			if (!fuel.isEmpty() || mode) {
				...
				if (!((double)charge < 0.1D)) {
					...
					if (worldIn instanceof ServerLevel server) {
						KB_F4UEntity kansaiki = new KB_F4UEntity(playerIn, server, stack);
	...
	public void appendHoverText(ItemStack stack, Item.TooltipContext context, List<Component> itemTip, TooltipFlag tipFlag) {
		...
		itemTip.add(Component.translatable("tips.item_kk_11f").withStyle(ChatFormatting.RED)); }

	@Override
	public Projectile asProjectile(Level worldIn, Position pos, ItemStack stack, Direction direct) {
		return new KB_F4UEntity(worldIn, pos.x(), pos.y(), pos.z(), stack);
	}
}

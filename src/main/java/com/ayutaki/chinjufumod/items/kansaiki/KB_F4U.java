...
public class KB_F4U extends Abstract_BowKansaiki {
			...
			if (!fuel.isEmpty() || mode) {
				...
				if (!((double)charge < 0.1D)) {
					...
					if (!worldIn.isClientSide) {
						KB_F4UEntity kansaiki = new KB_F4UEntity(playerIn, worldIn, stack);
	...
	public void appendHoverText(ItemStack stack, @Nullable World worldIn, List<ITextComponent> itemTip, ITooltipFlag tipFlag) {
		...
		itemTip.add(new TranslationTextComponent("tips.item_kk_11f").withStyle(TextFormatting.RED)); }
}

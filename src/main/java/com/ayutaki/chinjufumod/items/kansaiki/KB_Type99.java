...
public class KB_Type99 extends Abstract_BowKansaiki {
			...
			if (!fuel.isEmpty() || mode) {
				...
				if (!((double)charge < 0.1D)) {
					...
					if (!worldIn.isClientSide) {
						KB_Type99Entity kansaiki = new KB_Type99Entity(playerIn, worldIn, stack);
	...
	public void appendHoverText(ItemStack stack, @Nullable World worldIn, List<ITextComponent> itemTip, ITooltipFlag tipFlag) {
		...
		itemTip.add(new TranslationTextComponent("tips.item_kk_5f").withStyle(TextFormatting.RED)); }
}

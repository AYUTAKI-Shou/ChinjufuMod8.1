...
public class KB_Type99 extends Abstract_Kansaiki {

	public KB_Type99(String name, int max) {
		super(name, max);
	}
		...
		if (entityLiving instanceof EntityPlayer) {
			...
			if (!fuel.isEmpty() || mode) {
				...
				if (!((double)charge < 0.1D)) {
					...
					if (!worldIn.isRemote) {
						KB_Type99Entity kansaiki = new KB_Type99Entity(worldIn, playerIn, stack);
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> itemTip, ITooltipFlag advanced) {
		...
		itemTip.add(TextFormatting.RED + I18n.format("tips.item_kk_5f.name")); }
}

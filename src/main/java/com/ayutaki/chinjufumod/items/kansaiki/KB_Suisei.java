...
public class KB_Suisei extends Abstract_Kansaiki {

	public KB_Suisei(String name, int max) {
		super(name, max);
	}
		...
		if (entityLiving instanceof EntityPlayer) {
			...
			if (!fuel.isEmpty() || mode) {
				...
				if (!((double)charge < 0.1D)) {
					...
					if (!worldIn.isRemote) {
						KB_SuiseiEntity kansaiki = new KB_SuiseiEntity(worldIn, playerIn, stack);
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> itemTip, ITooltipFlag advanced) {
		...
		itemTip.add(TextFormatting.RED + I18n.format("tips.item_kk_8f.name")); }
}

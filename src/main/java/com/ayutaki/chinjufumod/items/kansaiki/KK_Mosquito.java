...
public class KK_Mosquito extends Abstract_BowKansaiki implements ProjectileItem {
			...
			if (!fuel.isEmpty() || mode) {
				...
				if (!((double)charge < 0.1D)) {
					...
					if (worldIn instanceof ServerLevel server) {
						KK_MosquitoEntity kansaiki = new KK_MosquitoEntity(playerIn, server, stack);
	...
	public void appendHoverText(ItemStack stack, Item.TooltipContext context, List<Component> itemTip, TooltipFlag tipFlag) {
		...
		itemTip.add(Component.translatable("tips.item_kk_8f").withStyle(ChatFormatting.BLUE)); }

	@Override
	public Projectile asProjectile(Level worldIn, Position pos, ItemStack stack, Direction direct) {
		return new KK_MosquitoEntity(worldIn, pos.x(), pos.y(), pos.z(), stack);
	}
}

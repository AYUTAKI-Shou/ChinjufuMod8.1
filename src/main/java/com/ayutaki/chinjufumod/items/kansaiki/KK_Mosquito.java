...
public class KK_Mosquito extends Abstract_Kansaiki {

	public KK_Mosquito(String name, int max) {
		super(name, max);
	}
		...
		if (entityLiving instanceof EntityPlayer) {
			...
			if (!fuel.isEmpty() || mode) {
				...
				if (!((double)charge < 0.1D)) {
					...
					if (!worldIn.isRemote) {
						KK_MosquitoEntity kansaiki = new KK_MosquitoEntity(worldIn, playerIn, stack);
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> itemTip, ITooltipFlag advanced) {
		...
		itemTip.add(TextFormatting.BLUE + I18n.format("tips.item_kk_8f.name")); }
}

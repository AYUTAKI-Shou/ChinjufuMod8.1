...
public class KK_Tenzan extends Abstract_Kansaiki {

	public KK_Tenzan(String name, int max) {
		super(name, max);
	}
		...
		if (entityLiving instanceof EntityPlayer) {
			...
			if (!fuel.isEmpty() || mode) {
				...
				if (!((double)charge < 0.1D)) {
					...
					if (!worldIn.isRemote) {
						KK_TenzanEntity kansaiki = new KK_TenzanEntity(worldIn, playerIn, stack);

...
public class KK_Tenzan extends Abstract_BowKansaiki implements ProjectileItem {
	
	public KK_Tenzan(Item.Properties props) {
		super(props);
	}
			...
			if (!fuel.isEmpty() || mode) {
				...
				if (!((double)charge < 0.1D)) {
					...
					if (worldIn instanceof ServerLevel server) {
						KK_TenzanEntity kansaiki = new KK_TenzanEntity(playerIn, server, stack);
	...
	public Projectile asProjectile(Level worldIn, Position pos, ItemStack stack, Direction direct) {
		return new KK_TenzanEntity(worldIn, pos.x(), pos.y(), pos.z(), stack);
	}
}

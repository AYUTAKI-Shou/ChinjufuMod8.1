...
public class KK_TBF extends Abstract_BowKansaiki implements ProjectileItem {
	
	public KK_TBF(Item.Properties props) {
		super(props);
	}
			...
			if (!fuel.isEmpty() || mode) {
				...
				if (!((double)charge < 0.1D)) {
					...
					if (worldIn instanceof ServerLevel server) {
						KK_TBFEntity kansaiki = new KK_TBFEntity(playerIn, server, stack);
	...
	public void appendHoverText(ItemStack stack, Item.TooltipContext context, List<Component> itemTip, TooltipFlag tipFlag) {
		...
		itemTip.add(Component.translatable("tips.item_kk_11f").withStyle(ChatFormatting.BLUE)); }

	@Override
	public Projectile asProjectile(Level worldIn, Position pos, ItemStack stack, Direction direct) {
		return new KK_TBFEntity(worldIn, pos.x(), pos.y(), pos.z(), stack);
	}
}

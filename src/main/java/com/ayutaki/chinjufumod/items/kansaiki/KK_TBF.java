...
public class KK_TBF extends Abstract_BowKansaiki {

	public KK_TBF(Properties props) {
		super(props);
	}
		...
		if (entityLiving instanceof PlayerEntity) {
			...
			if (!fuel.isEmpty() || mode) {
				...
				if (!((double)charge < 0.1D)) {
					...
					if (!worldIn.isRemote) {
						KK_TBFEntity kansaiki = new KK_TBFEntity(playerIn, worldIn, stack);
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> itemTip, ITooltipFlag tipFlag) {
		...
		itemTip.add(new TranslationTextComponent("tips.item_kk_11f").applyTextStyle(TextFormatting.BLUE)); }
}

...
public abstract class AbstractRensouHou_L extends Base_RensouHou {

	public AbstractRensouHou_L(Item.Properties props) {
		super(props);
	}
	
	/* Called when the player stops using an Item (stops holding the right mouse button). */
	@Override
	public boolean releaseUsing(ItemStack hStack, Level worldIn, LivingEntity entityIn, int timeLeft) { ///for 1.21.4
		if (entityIn instanceof Player playerIn) {
			ItemStack projectile = playerIn.getProjectile(hStack);
			if (!projectile.isEmpty()) {
				int i = this.getUseDuration(hStack, entityIn) - timeLeft;
				i = net.neoforged.neoforge.event.EventHooks.onArrowLoose(hStack, worldIn, playerIn, i, true); //neo
				if (i < 0) return false;
				...
				if (!((double)f < 0.1)) {
					...
					if (!worldIn.isClientSide() && !list.isEmpty()) {
						...
						this.shoot(worldIn, playerIn, playerIn.getUsedItemHand(), hStack, list, (battleship)? 4.0F : 3.0F, 1.0F, true, null); 
	...
	protected static List<ItemStack> draw(ItemStack hStack, ItemStack projectile, LivingEntity playerIn) {
		if (projectile.isEmpty()) { return List.of(); } 
			...
			boolean infinite = projectile.getItem() instanceof Ammo_Large arrow && arrow.isInfinite(projectile, hStack, playerIn);
	...
	protected static ItemStack useAmmo(ItemStack hStack, ItemStack projectile, LivingEntity playerIn, boolean mode) {
		boolean flag = !mode && !playerIn.hasInfiniteMaterials();
		if (!flag) {

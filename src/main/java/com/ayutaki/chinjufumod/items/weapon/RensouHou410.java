...
public class RensouHou410 extends Abstract_RensouHou {

	public RensouHou410(Item.Properties props) {
		super(props);
	}
	...
	public void releaseUsing(ItemStack hStack, Level worldIn, LivingEntity entityLiving, int timeLeft) {
		if (entityLiving instanceof Player) {
			...
			if (!projectile.isEmpty() || mode) {
				...
				if (projectile.isEmpty()) { projectile = new ItemStack(Items_Weapon.AMMUNITION_L.get()); }
				
				float charge = getPowerForTime(i);
				if (!((double)charge < 0.1D)) {
					boolean mode1 = playerIn.getAbilities().instabuild || (projectile.getItem() instanceof Ammo_Large && ((Ammo_Large)projectile.getItem()).isInfinite(projectile, hStack, playerIn));
					
					if (!worldIn.isClientSide) {
						Ammo_Large arrowItem = (Ammo_Large)(projectile.getItem() instanceof Ammo_Large ? projectile.getItem() : Items_Weapon.AMMUNITION_L.get());
						...
						abstractArrow.shootFromRotation(playerIn, playerIn.getXRot(), playerIn.getYRot(), 0.0F, (battleship)? 4.0F : 3.0F, 1.0F);

						/* Damage */
						abstractArrow.setCritArrow(true);
						/** addKOUKEI, fixFIT, shipType **/
						this.setAmmoDamage(hStack, worldIn, abstractArrow, playerIn, 6.0D, 3.0D, battleship);
					...
					worldIn.playSound((Player)null, playerIn.getX(), playerIn.getY(), playerIn.getZ(), SoundEvents_CM.AM_FIRE.get(), 
							SoundSource.PLAYERS, 1.2F, 1.0F / (worldIn.getRandom().nextFloat() * 0.4F + 1.2F) + 0.5F);
					
					Vec3 pos = playerIn.getEyePosition(1.0F).add(0.0D, -0.15D, 0.0D).subtract(playerIn.getLookAngle().multiply(-1.0D, -1.0D, -1.0D));
					worldIn.addParticle((ParticleOptions) ParticleTypes_CM.SHOOTL_PT.get(), pos.x, pos.y, pos.z, 0.0D, 0.0D, 0.0D);
					...
					if (!mode1 && !playerIn.getAbilities().instabuild) {
						...
						/* Drop the Cartridge. */
						if (!worldIn.isClientSide) {
							playerIn.drop(new ItemStack(Items_NoTab.CARTRIDGE_L.get()), false);
							worldIn.playSound(null, playerIn.getX(), playerIn.getY(), playerIn.getZ(), SoundEvents_CM.AM_CARTRIDGE.get(), 
									SoundSource.BLOCKS, 1.2F, 1.0F / (worldIn.getRandom().nextFloat() * 0.4F + 1.2F) + 0.5F); }
					}
	...
	public static final Predicate<ItemStack> AMMOL = (projectile) -> {
		return projectile.getItem() == Items_Weapon.AMMUNITION_L.get();
	};
	...
	public void appendHoverText(ItemStack stack, @Nullable Level worldIn, List<Component> itemTip, TooltipFlag tipFlag) {
		itemTip.add(new TranslatableComponent("tips.item_rensouhou_large").withStyle(ChatFormatting.GRAY));
		itemTip.add(new TranslatableComponent("tips.item_rensouhou410").withStyle(ChatFormatting.DARK_GREEN));
	}
}

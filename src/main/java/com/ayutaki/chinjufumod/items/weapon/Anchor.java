...
public class Anchor extends ItemSword {
	...
	public void onUpdate(ItemStack hStack, World worldIn, Entity entityIn, int itemSlot, boolean isSelected) {
	...
	protected void playEquipSound(EntityPlayer player, ItemStack hStack) {
		if (!hStack.isEmpty()) {
			SoundEvent sound = SoundEvents.ITEM_ARMOR_EQUIP_GENERIC;
			Item hItem = hStack.getItem();

			if (hItem instanceof Anchor) {
				sound = SoundEvents.ITEM_ARMOR_EQIIP_ELYTRA; }
			
			else if (hItem == Items.ELYTRA) {
				sound = SoundEvents.ITEM_ARMOR_EQIIP_ELYTRA; }

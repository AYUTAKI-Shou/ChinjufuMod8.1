...
public class RensouHou127 extends Abstract_RensouHou {
	...
	public RensouHou127(Item.Properties props) {
		super(props);
	}
		...
		if (entityLiving instanceof PlayerEntity) {
			...
			if (!projectile.isEmpty() || mode) {
				...
				if (!((double)charge < 0.1D)) {
					boolean mode1 = playerIn.abilities.isCreativeMode || (projectile.getItem() instanceof Ammo_Small && ((Ammo_Small)projectile.getItem()).isInfinite(projectile, hStack, playerIn));
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> itemTip, ITooltipFlag tipFlag) {
		itemTip.add(new TranslationTextComponent("tips.item_rensouhou_small").applyTextStyle(TextFormatting.GRAY));
		itemTip.add(new TranslationTextComponent("tips.item_rensouhou127").applyTextStyle(TextFormatting.DARK_GREEN));
	}
}

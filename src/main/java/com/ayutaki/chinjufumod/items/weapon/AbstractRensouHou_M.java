...
public abstract class AbstractRensouHou_M extends Base_RensouHou {

	public AbstractRensouHou_M(Item.Properties props) {
		super(props);
	}
			...
			if (!projectile.isEmpty()) {
				...
				if (!((double)f < 0.1)) {
					...
					if (!worldIn.isClientSide() && !list.isEmpty()) {
						this.shoot(worldIn, playerIn, playerIn.getUsedItemHand(), hStack, list, 3.5F, 1.0F, true, null); 
	...
	protected static List<ItemStack> draw(ItemStack hStack, ItemStack projectile, LivingEntity playerIn) {
		if (projectile.isEmpty()) { return List.of(); } 
			...
			boolean infinite = projectile.getItem() instanceof Ammo_Medium arrow && arrow.isInfinite(projectile, hStack, playerIn);

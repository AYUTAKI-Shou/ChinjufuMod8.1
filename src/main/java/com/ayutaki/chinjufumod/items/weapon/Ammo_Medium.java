...
public class Ammo_Medium extends Item implements ProjectileItem {
	...
	public AbstractAmmo_Entity createAmmo(Level worldIn, ItemStack hStack, LivingEntity shooter, @Nullable ItemStack eStack) {
		return new AmmoEntity_Medium(worldIn, shooter, hStack.copyWithCount(1), eStack);
	}

	@Override
	public Projectile asProjectile(Level worldIn, Position pos, ItemStack hStack, Direction facing) {
		AmmoEntity_Medium arrow = new AmmoEntity_Medium(worldIn, pos.x(), pos.y(), pos.z(), hStack.copyWithCount(1), null);

...
public class Shield_CM extends ShieldItem {
	...
	@Override
	public void releaseUsing(ItemStack hStack, Level worldIn, LivingEntity entity, int timeLeft) {
				...
				if (playerIn.getCooldowns().isOnCooldown(this)) { }
					...
					if (listEntity.size() > 1) {//1 == You.
						if (playerIn.isEyeInFluid(FluidTags.WATER)) {
							...
							this.createExplosion(playerIn, playerIn.getX(), playerIn.getY(), playerIn.getZ(), 1.0F, false, Explosion.BlockInteraction.NONE);
	...
	public void appendHoverText(ItemStack stack, @Nullable Level worldIn, List<Component> itemTip, TooltipFlag tipFlag) {
		itemTip.add(new TranslatableComponent("tips.item_shield").withStyle(ChatFormatting.GRAY));
		itemTip.add(new TranslatableComponent("tips.item_shield2").withStyle(ChatFormatting.GRAY));
	}
}

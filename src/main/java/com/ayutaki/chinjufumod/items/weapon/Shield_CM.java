...
public class Shield_CM extends ShieldItem {
	...
	public Shield_CM(Item.Properties props) {
		super(props.group(ItemGroups_CM.CMARMOR));
		/** 336×3 =1008, ×5=1680, ×7=2352, ×9=3024 **/
	}

	/* Called when the player stops using an Item (stops holding the right mouse button). */
	public void onPlayerStoppedUsing(ItemStack hStack, World worldIn, LivingEntity entity, int timeLeft) {
	...
	/* 盾として消耗処理 */
	@Override
	public boolean isShield(ItemStack hStack, LivingEntity entity) {
		return hStack.getItem() instanceof ShieldItem;
	}
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> itemTip, ITooltipFlag tipFlag) {
		itemTip.add(new TranslationTextComponent("tips.item_shield").applyTextStyle(TextFormatting.GRAY));
		itemTip.add(new TranslationTextComponent("tips.item_shield2").applyTextStyle(TextFormatting.GRAY));
	}
}

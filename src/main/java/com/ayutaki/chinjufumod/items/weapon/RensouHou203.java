...
public class RensouHou203 extends Abstract_RensouHou {
	...
	public RensouHou203(Item.Properties props) {
		super(props);
	}
		...
		if (entityLiving instanceof PlayerEntity) {
			...
			if (!projectile.isEmpty() || mode) {
				...
				if (!((double)charge < 0.1D)) {
					boolean mode1 = playerIn.abilities.instabuild || (projectile.getItem() instanceof Ammo_Medium && ((Ammo_Medium)projectile.getItem()).isInfinite(projectile, hStack, playerIn));
					...
					if (!worldIn.isClientSide) {
						...
						/* Move */
						abstractArrow.shootFromRotation(playerIn, playerIn.xRot, playerIn.yRot, 0.0F, 3.5F, 1.0F);
	...
	public void appendHoverText(ItemStack stack, @Nullable World worldIn, List<ITextComponent> itemTip, ITooltipFlag tipFlag) {
		itemTip.add(new TranslationTextComponent("tips.item_rensouhou_medium").withStyle(TextFormatting.GRAY));
		itemTip.add(new TranslationTextComponent("tips.item_rensouhou203").withStyle(TextFormatting.DARK_GREEN));
	}
}

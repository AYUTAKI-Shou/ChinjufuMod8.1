...
public class KoukakuHou100 extends Abstract_RensouHou {
	...
	public KoukakuHou100(String name, int max) {
		super(name, max);
	}
		...
		if (entityLiving instanceof EntityPlayer) {
			...
			if (!projectile.isEmpty() || mode) {
				...
				if (!((double)charge < 0.1D)) {
					boolean mode1 = playerIn.capabilities.isCreativeMode || (projectile.getItem() instanceof Ammo_Small && ((Ammo_Small) projectile.getItem()).isInfinite(projectile, hStack, playerIn));
					...
					if (!worldIn.isRemote) {
						...
						AmmoEntity_Small abstractArrow = (AmmoEntity_Small) arrowItem.createAmmo(worldIn, projectile, playerIn);

						/* Move */
						abstractArrow.shoot(playerIn, playerIn.rotationPitch, playerIn.rotationYaw, 0.0F, 3.0F, 1.0F);
						...
						if (mode1 || playerIn.capabilities.isCreativeMode) {
							abstractArrow.pickupStatus = AmmoEntity_Small.PickupStatus.CREATIVE_ONLY; }
					...
					worldIn.playSound((EntityPlayer)null, playerIn.posX, playerIn.posY, playerIn.posZ,
							SoundEvents_CM.AM_FIRE, SoundCategory.PLAYERS, 0.5F, 1.4F / (itemRand.nextFloat() * 0.4F + 1.2F) + 0.5F);
					...
					if (!mode1 && !playerIn.capabilities.isCreativeMode) {
						...
						if (!worldIn.isRemote) {
							...
							worldIn.playSound((EntityPlayer)null, playerIn.posX, playerIn.posY, playerIn.posZ, SoundEvents_CM.AM_CARTRIDGE, 
									SoundCategory.BLOCKS, 0.5F, 1.4F / (itemRand.nextFloat() * 0.4F + 1.2F) + 0.5F); }
					}
	...
	} /** DON'T use Time Unit, because it interferes with processing.**/

	/* Ammo to be used. */
	protected boolean isArrow(ItemStack hStack) {
		return hStack.getItem() instanceof Ammo_Small;
	}
	
	@Override
	@SideOnly(Side.CLIENT)
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> itemTip, ITooltipFlag advanced) {
		itemTip.add(I18n.format("tips.item_rensouhou_small.name"));
		itemTip.add(TextFormatting.DARK_GREEN + I18n.format("tips.item_koukakuhou100.name"));
	}
}

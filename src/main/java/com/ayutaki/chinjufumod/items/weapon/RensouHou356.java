...
public class RensouHou356 extends Abstract_RensouHou {

	public RensouHou356(Item.Properties props) {
		super(props);
	}
			...
			if (!projectile.isEmpty() || mode) {
				...
				if (!((double)charge < 0.1D)) {
					...
					if (!worldIn.isClientSide) {
						...
						this.setAmmoDamage(hStack, worldIn, abstractArrow, playerIn, 4.5D, 2.0D, battleship);
	...
	public void appendHoverText(ItemStack stack, @Nullable Level worldIn, List<Component> itemTip, TooltipFlag tipFlag) {
		...
		itemTip.add(new TranslatableComponent("tips.item_rensouhou356").withStyle(ChatFormatting.DARK_GREEN));
	}
}

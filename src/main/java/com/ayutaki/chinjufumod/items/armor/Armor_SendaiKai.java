...
public class Armor_SendaiKai extends Base_CruiserItem {
	...
	public Armor_SendaiKai(IArmorMaterial material, EquipmentSlotType slot, Properties props) {
	...
	public void inventoryTick(ItemStack stack, World worldIn, Entity entity, int slot, boolean selected) {
		if (entity instanceof PlayerEntity) {
			...
			if (playerIn.getItemStackFromSlot(EquipmentSlotType.FEET).getItem() instanceof Armor_SendaiKai) {
	...
	public <Armor extends BipedModel<?>> Armor getArmorModel(LivingEntity entityLiving, ItemStack stack, EquipmentSlotType slotType, Armor defModel) {
		return (Armor) (slotType == EquipmentSlotType.LEGS ? new SendaiInner(0.4F) : new SendaiOuter(0.55F));
	}

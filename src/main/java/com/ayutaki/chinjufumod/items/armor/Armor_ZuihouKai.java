...
public class Armor_ZuihouKai extends Base_CarrierItem {
	...
	public Armor_ZuihouKai(ArmorMaterial material, ArmorType slot, Item.Properties props) {
	...
	public void inventoryTick(ItemStack stack, Level worldIn, Entity entity, int slot, boolean selected) {
		if (entity instanceof Player playerIn) {
			if (playerIn.getItemBySlot(EquipmentSlot.FEET).getItem() instanceof Armor_ZuihouKai) {
	...
	public static final class ArmorRender extends Base_ArmorItem.BaseRender {
		...
		public Model getHumanoidArmorModel(ItemStack stack, EquipmentClientInfo.LayerType type, Model original) {
			...
			ModelPart part = models.bakeLayer(leg? ArmorLayer_CM.ZUIHOU_INNER : ArmorLayer_CM.ZUIHOU_OUTER);

...
public class Armor_ZuihouKai extends Base_CarrierItem {

	public Armor_ZuihouKai(IArmorMaterial material, EquipmentSlotType slot, Properties props) {
	...
	public void inventoryTick(ItemStack stack, World worldIn, Entity entity, int slot, boolean selected) {
		if (entity instanceof PlayerEntity) {
			...
			if(playerIn.getItemBySlot(EquipmentSlotType.FEET).getItem() instanceof Armor_ZuihouKai) {
	...
	public <Armor extends BipedModel<?>> Armor getArmorModel(LivingEntity entityLiving, ItemStack stack, EquipmentSlotType slot, Armor defModel) {
		return (Armor) (slot == EquipmentSlotType.LEGS ? new ZuihouInner(0.3F) : new ZuihouOuter(0.55F));
	}

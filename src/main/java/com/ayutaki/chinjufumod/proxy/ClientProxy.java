...
public class ClientProxy extends CommonProxy {
	...
	* Register Entity and Config. */
	...
	public void registerEntityRender() {
		...
		RenderingRegistry.registerEntityRenderingHandler(SB_SeiranEntity.class, renderManager ->
		new RenderSB_Seiran(renderManager, Items_Weapon.SEIRAN, Minecraft.getMinecraft().getRenderItem()));

		RenderingRegistry.registerEntityRenderingHandler(SB_ZuiunEntity.class, renderManager ->
		new RenderSB_Zuiun(renderManager, Items_Weapon.ZUIUN, Minecraft.getMinecraft().getRenderItem()));
	...
	public void registerTileEntityRenderer() {
		ClientRegistry.bindTileEntitySpecialRenderer(BlackBoard_TileEntity.class, new BlackBoard_TERenderer());
		ClientRegistry.bindTileEntitySpecialRenderer(WoodBoard_TileEntity.class, new WoodBoard_TERenderer());
	}
		...
		if (entity != null && mc.effectRenderer != null) {
			...
			if (dx * dx + dy * dy + dz * dz <= 1024D && i <= 1) {
				...
				case MARK_PT:
					...
					break;
				case CHALK_PT:
					particle = new Chalk_Particle(Minecraft.getMinecraft().getTextureManager(), world, x, y, z, vx, vy, vz);
					break;

...
public class ColdRecipeSerializer<T extends AbstractColdRecipe> extends ForgeRegistryEntry<IRecipeSerializer<?>> implements IRecipeSerializer<T> {
	...
	public T fromJson(ResourceLocation name, JsonObject jsonObject) {
		String groupName = JSONUtils.getAsString(jsonObject, "group", "");
		...
		ItemStack output;
		if (jsonObject.get("result").isJsonObject()) output = ShapedRecipe.itemFromJson(JSONUtils.getAsJsonObject(jsonObject, "result"));
		else {
		...
		float exp = JSONUtils.getAsFloat(jsonObject, "experience", 0.0F);
		int time = JSONUtils.getAsInt(jsonObject, "cookingtime", this.defaultCookTime);
		return this.factory.create(name, groupName, inputList, output, exp, time);
	}

	@Override
	public T fromNetwork(ResourceLocation name, PacketBuffer buffer) {
	...
	@Override
	public void toNetwork(PacketBuffer buffer, T iRecipe) {

...
public class Tansu_TileEntity extends LockableLootTileEntity implements IChestLid, ITickableTileEntity {
	...
	public void tick() {
		...
		if (this.openCount > 0 && this.lidOpenness == 0.0F) {
			this.level.setBlock(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(Tansu.OPEN, Boolean.valueOf(true)), 3);
		...
		if (this.openCount == 0 && this.lidOpenness > 0.0F || this.openCount > 0 && this.lidOpenness < 1.0F) {
			...
			if (this.lidOpenness < f2 && f1 >= f2) {
				this.level.setBlock(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(Tansu.OPEN, Boolean.valueOf(false)), 3);
	...
	public static int getOpenCount(World worldIn, LockableTileEntity lTileEntity, int x, int y, int z) {
		...
		for(PlayerEntity playerIn : worldIn.getEntitiesOfClass(PlayerEntity.class, new AxisAlignedBB((double)((float)x - f), (double)((float)y - f), (double)((float)z - f), (double)((float)(x + 1) + f), (double)((float)(y + 1) + f), (double)((float)(z + 1) + f)))) {
			if (playerIn.containerMenu instanceof TansuMenu) {
				IInventory iinventory = ((TansuMenu)playerIn.containerMenu).getContainer();

...
public class BlackBoardText {
	...
	private static final Codec<Component[]> LINES_CODEC = ComponentSerialization.FLAT_CODEC
		.listOf().comapFlatMap(
			function -> Util.fixedSize((List<Component>)function, MAX)
					.map(iText -> new Component[]{iText.get(0), iText.get(1), iText.get(2), iText.get(3), iText.get(4), iText.get(5)}),
			iFilteredText -> List.of(iFilteredText[0], iFilteredText[1], iFilteredText[2], iFilteredText[3], iFilteredText[4], iFilteredText[5]) );
	
	public static final Codec<BlackBoardText> DIRECT_CODEC = RecordCodecBuilder.create(
		instance -> instance.group(
					LINES_CODEC.fieldOf("messages").forGetter(iTxt -> iTxt.messages),
					LINES_CODEC.lenientOptionalFieldOf("filtered_messages").forGetter(BlackBoardText::filteredMessages),
				...
				.apply(instance, BlackBoardText::load)
	);

	public BlackBoardText() {
		this(emptyMessages(), emptyMessages(), DyeColor.WHITE, false);
	}

	public BlackBoardText(Component[] iText, Component[] iFilteredText, DyeColor dye, boolean hasGlow) {
		this.messages = iText;
	...
	private static Component[] emptyMessages() {
		return new Component[]{CommonComponents.EMPTY, CommonComponents.EMPTY, CommonComponents.EMPTY, 
				CommonComponents.EMPTY, CommonComponents.EMPTY, CommonComponents.EMPTY};
	}//Component.literal(""),

	private static BlackBoardText load(Component[] iText, Optional<Component[]> iFilteredText, DyeColor dye, boolean hasGlow) {
		return new BlackBoardText(iText, iFilteredText.orElse(Arrays.copyOf(iText, iText.length)), dye, hasGlow);
	}
	...
	public BlackBoardText setGlowText(boolean hasGlow) {
		return hasGlow == this.hasGlowingText ? this : new BlackBoardText(this.messages, this.filteredMessages, this.color, hasGlow);
	}
	...
	public BlackBoardText setColor(DyeColor dye) {
		return dye == this.getColor() ? this : new BlackBoardText(this.messages, this.filteredMessages, dye, this.hasGlowingText);
	}
	...
	public BlackBoardText setMessage(int i, Component iText) {
		return this.setMessage(i, iText, iText);
	}

	public BlackBoardText setMessage(int i, Component iText, Component iFilteredText) {
		...
		return new BlackBoardText(acomponent, acomponent1, this.color, this.hasGlowingText);
	}

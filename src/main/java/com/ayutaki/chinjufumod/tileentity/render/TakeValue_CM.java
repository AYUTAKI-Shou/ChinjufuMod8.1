...
public class TakeValue_CM {
	...
	public static EnumDyeColor chalkColor(Item item) {
		if (item instanceof Chalk_Orange) { return EnumDyeColor.ORANGE; }
		if (item instanceof Chalk_Magenta) { return EnumDyeColor.MAGENTA; }
		if (item instanceof Chalk_LightBlue) { return EnumDyeColor.LIGHT_BLUE; }
		if (item instanceof Chalk_Yellow) { return EnumDyeColor.YELLOW; }
		if (item instanceof Chalk_Lime) { return EnumDyeColor.LIME; }
		if (item instanceof Chalk_Pink) { return EnumDyeColor.PINK; }
		if (item instanceof Chalk_Gray) { return EnumDyeColor.GRAY; }
		if (item instanceof Chalk_LightGray) { return EnumDyeColor.SILVER; }
		if (item instanceof Chalk_Cyan) { return EnumDyeColor.CYAN; }
		if (item instanceof Chalk_Purple) { return EnumDyeColor.PURPLE; }
		if (item instanceof Chalk_Blue) { return EnumDyeColor.BLUE; }
		if (item instanceof Chalk_Brown) { return EnumDyeColor.BROWN; }
		if (item instanceof Chalk_Green) { return EnumDyeColor.GREEN; }
		if (item instanceof Chalk_Red) { return EnumDyeColor.RED; }
		if (item instanceof Chalk_Black) { return EnumDyeColor.BLACK; }
		else { return EnumDyeColor.WHITE; }
	}

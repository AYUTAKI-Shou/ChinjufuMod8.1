...
public class BlackBoard_TERenderer extends TileEntitySpecialRenderer<BlackBoard_TileEntity> {
	private int MAX = 6;
	
	public void render(BlackBoard_TileEntity tileEntity, double x, double y, double z, float ticks, int dSage, float alpha) {
	...
	public void renderBoardText(BlackBoard_TileEntity tileEntity, int dSage) {
		FontRenderer fontRenderer = this.getFontRenderer();
		float posX = -0.49333334F; //-0.46666667D;
		float posY = 0.5F; //0.33333334F 0.023333333
		float posZ = 0.0315F; //0.046666667
		...
		if (dSage < 0) {
			for (int i = 0; i < MAX; ++i) {
				if (tileEntity.boardText[i] != null) {
					...
					ITextComponent iText = tileEntity.boardText[i];
					List<ITextComponent> list = GuiUtilRenderComponents.splitText(iText, chara, fontRenderer, false, true);
					...
					if (i == tileEntity.lineBeingEdited) {
						s = s + "_";
						fontRenderer.drawString(s, ALIGN, (float)(i * space * spaceFix - 20), txtColor, false); }

...
public class BlackBoard_TERenderer extends TileEntityRenderer<BlackBoard_TileEntity> {
	private int MAX = 6;
	
	public BlackBoard_TERenderer(TileEntityRendererDispatcher rendererDispatcherIn) {
		super(rendererDispatcherIn);
	}

	public void render(BlackBoard_TileEntity tileEntity, float ticks, MatrixStack matrix, IRenderTypeBuffer bufferIn, int light, int overLay) {
		BlockState state = tileEntity.getBlockState();
		matrix.push();
		matrix.translate(0.5D, 0.5D, 0.5D);
		float f4 = -state.get(BlackBoard.H_FACING).getHorizontalAngle();

...
public class OfficeDesk_TileEntity extends TileEntityLockableLoot implements ITickable {

	private NonNullList<ItemStack> chestContents = NonNullList.<ItemStack>withSize(27, ItemStack.EMPTY);

	public int openCount;
	public float lidOpenness;
	public float preOpenness;
	...
	public static void registerFixesChest(DataFixer fixer) {
		fixer.registerWalker(FixTypes.BLOCK_ENTITY, new ItemStackDataLists(OfficeDesk_TileEntity.class, new String[] {"Items"}));
	}
	...
	@Nullable
	protected OfficeDesk_TileEntity getAdjacentChest(EnumFacing side) {
		...
		if (this.isChestAt(pos)) {
			...
			if (tileentity instanceof OfficeDesk_TileEntity) {
				OfficeDesk_TileEntity tileentitychest = (OfficeDesk_TileEntity)tileentity;
	...
	private boolean isChestAt(BlockPos posIn) {
		if (this.world == null) {
			...
			return block instanceof OfficeDesk;
		}
	}
	...
	public void closeInventory(EntityPlayer playerIn) {
		if (!playerIn.isSpectator() && this.getBlockType() instanceof OfficeDesk) {
			--this.openCount;
			this.world.addBlockEvent(this.pos, getBlockType(), 1, this.openCount);
			this.world.notifyNeighborsOfStateChange(this.pos, this.getBlockType(), false);
		}
	...
	public static int calculatePlayersUsing(World worldIn, TileEntityLockable lTileEntity, int x, int y, int z) {
		...
		for(EntityPlayer playerIn : worldIn.getEntitiesWithinAABB(EntityPlayer.class, new AxisAlignedBB((double)((float)x - 5.0F), (double)((float)y - 5.0F), (double)((float)z - 5.0F), (double)((float)(x + 1) + 5.0F), (double)((float)(y + 1) + 5.0F), (double)((float)(z + 1) + 5.0F)))) {
			if (playerIn.openContainer instanceof ContainerChest) {
				IInventory iinventory = ((ContainerChest)playerIn.openContainer).getLowerChestInventory();
	...
	public void update() {
		...
		int k = this.pos.getZ();
		++this.ticksSinceSync;
		this.openCount = calculatePlayersUsingSync(this.world, this, this.ticksSinceSync, i, j, k, this.openCount);

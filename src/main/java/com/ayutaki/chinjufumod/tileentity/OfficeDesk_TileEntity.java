...
public class OfficeDesk_TileEntity extends LockableLootTileEntity implements IChestLid, ITickableTileEntity {
	...
	protected OfficeDesk_TileEntity(TileEntityType<?> typeIn) {
		super(typeIn);
	}
	...
	public void tick() {
		...
		if (this.openCount > 0 && this.lidOpenness == 0.0F) {
			this.level.setBlock(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(OfficeDesk.OPEN, Boolean.valueOf(true)), 3);
		...
		if (this.openCount == 0 && this.lidOpenness > 0.0F || this.openCount > 0 && this.lidOpenness < 1.0F) {
			...
			if (this.lidOpenness < f2 && f1 >= f2) {
				this.level.setBlock(this.worldPosition, this.level.getBlockState(this.worldPosition).setValue(OfficeDesk.OPEN, Boolean.valueOf(false)), 3);
	...
	private net.minecraftforge.items.IItemHandlerModifiable createHandler() {
		...
		IInventory inv = OfficeDesk.getContainer((OfficeDesk) state.getBlock(), state, getLevel(), getBlockPos(), true);

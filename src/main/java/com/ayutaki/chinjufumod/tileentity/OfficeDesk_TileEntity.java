...
public class OfficeDesk_TileEntity extends LockableLootTileEntity implements IChestLid, ITickableTileEntity, IInventory {
	...
	public OfficeDesk_TileEntity(TileEntityType<?> typeIn) {
		super(typeIn);
	}

	public OfficeDesk_TileEntity() {
		this(TileEntity_CM.OFFICEDESK);
	}
	...
	public void tick() {
		...
		if (this.openCount > 0 && this.lidAngle == 0.0F) {
			this.world.setBlockState(this.pos, this.world.getBlockState(this.pos).with(OfficeDesk.OPEN, Boolean.valueOf(true)), 3);
		...
		if (this.openCount == 0 && this.lidAngle > 0.0F || this.openCount > 0 && this.lidAngle < 1.0F) {
			...
			if (this.lidAngle < 0.5F && f1 >= 0.5F) {
				this.world.setBlockState(this.pos, this.world.getBlockState(this.pos).with(OfficeDesk.OPEN, Boolean.valueOf(false)), 3);
	...
	protected void onOpenOrClose() {
		...
		if (block instanceof OfficeDesk) {
	...
	public static int getPlayersUsing(IBlockReader reader, BlockPos pos) {
		...
		if (state.hasTileEntity()) {
			...
			if (tileentity instanceof OfficeDesk_TileEntity) {
				return ((OfficeDesk_TileEntity) tileentity).openCount;
			}
		}
		return 0;
	}

	public static void swapContents(OfficeDesk_TileEntity te, OfficeDesk_TileEntity otherTe) {
	...
	private net.minecraftforge.items.IItemHandlerModifiable createHandler() {
		...
		if (!(state.getBlock() instanceof OfficeDesk)) {
			return new net.minecraftforge.items.wrapper.InvWrapper(this);
		}
		IInventory inv = OfficeDesk.getContainer((OfficeDesk) state.getBlock(), state, getWorld(), getPos(), true);

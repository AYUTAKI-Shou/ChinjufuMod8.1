...
public class NoteTileEntity extends BlockEntity implements Clearable, MenuProvider {
	...
	private final Container bookAccess = new Container() {
		...
		@Override
		public ItemStack removeItem(int pnPage, int count) {
			if (pnPage == 0) {
				ItemStack hStack = NoteTileEntity.this.book.split(count);
				if (NoteTileEntity.this.book.isEmpty()) {
		...
		public void setChanged() {
			NoteTileEntity.this.setChanged();
		}
	...
	public void setBook(ItemStack hStack, @Nullable Player player) {
		this.book = this.resolveBook(hStack, player);
		this.page = 0;
	...
	private ItemStack resolveBook(ItemStack hStack, @Nullable Player playerIn) {
		if (this.level instanceof ServerLevel server) {
			WrittenBookItem.resolveBookComponents(hStack, this.createCommandSourceStack(playerIn, server), playerIn);
		}
		return hStack;
	}

	private CommandSourceStack createCommandSourceStack(@Nullable Player playerIn, ServerLevel worldIn) {
		String s;
		Component component;
		if (playerIn == null) {
			s = "Lectern";
		...
		return new CommandSourceStack(CommandSource.NULL, vec3, Vec2.ZERO, worldIn, 2, s, component, worldIn.getServer(), playerIn);
	}
	...
	protected void loadAdditional(CompoundTag compound, HolderLookup.Provider provider) {
		...
		if (compound.contains("Book", 10)) {
			this.book = this.resolveBook(ItemStack.parse(provider, compound.getCompound("Book")).orElse(ItemStack.EMPTY), null);
		} 
		else { this.book = ItemStack.EMPTY; }

		this.pageCount = getPageCount(this.book);
		this.page = Mth.clamp(compound.getInt("Page"), 0, this.pageCount - 1);
	}
	...
	protected void saveAdditional(CompoundTag compound, HolderLookup.Provider provider) {
		...
		if (!this.getBook().isEmpty()) {
			compound.put("Book", this.getBook().save(provider));
	...
	private static int getPageCount(ItemStack hStack) {
		WrittenBookContent writtenbookcontent = hStack.get(DataComponents.WRITTEN_BOOK_CONTENT);
		if (writtenbookcontent != null) {
			return writtenbookcontent.pages().size();
		} else {
			WritableBookContent writablebookcontent = hStack.get(DataComponents.WRITABLE_BOOK_CONTENT);

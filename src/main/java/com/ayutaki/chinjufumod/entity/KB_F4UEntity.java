...
public class KB_F4UEntity extends AbstractKK_Entity {

	private ItemStack projectile = new ItemStack(Items_Weapon.F4U.get());
	private double baseDamage = 11.0D; /** F4U-7 爆装+11 **/

	public KB_F4UEntity(EntityType<KB_F4UEntity> type, Level worldIn) {
		super(type, worldIn);
	}

	public KB_F4UEntity(LivingEntity entityIn, Level world, ItemStack stack) {
		super(EntityTypes_CM.F4U.get(), entityIn, world, stack);
		this.projectile = stack.copy();
	}

	public KB_F4UEntity(Level wroldIn, double x, double y, double z, ItemStack stack) {
		super(EntityTypes_CM.F4U.get(), x, y, z, wroldIn, stack);
	}

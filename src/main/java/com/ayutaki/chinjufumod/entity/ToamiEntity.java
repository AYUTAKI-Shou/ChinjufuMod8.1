...
public class ToamiEntity extends EntityThrowable {
	...
	public void onUpdate() {
		...
		if (!world.isRemote) {
			...
			if (isReturning()) {
				if (thrower == null && ticksExisted > 300) { dropAndKill(); }
	...
	private void returnAndKill() {
		...
		EntityItem hItem = new EntityItem(world, thrower.posX, thrower.posY, thrower.posZ, hStack);
		...
		case ENTITY: {
			...
			if (!world.isRemote && result.entityHit != null && result.entityHit instanceof EntityLivingBase && result.entityHit != getThrower()) {
				...
				if (result.entityHit instanceof EntitySquid) {
					...
					if (world.getBiome(this.getPosition()) == Biomes.OCEAN) {
						int amount = world.rand.nextInt(2);
						if (world.rand.nextInt(2) == 0) { thrower.entityDropItem(new ItemStack(Items_Teatime.SHIO, amount, 3), 1.0F); } }

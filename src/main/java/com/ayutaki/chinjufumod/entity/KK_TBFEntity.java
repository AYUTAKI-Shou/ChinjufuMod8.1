...
public class KK_TBFEntity extends AbstractKK_Entity {
	...
	private ItemStack projectile = new ItemStack(Items_Weapon.TBF.get());
	private double baseDamage = 11.0D; /** TBF 火力 +2, 雷装 +9 **/
	
	public KK_TBFEntity(EntityType<KK_TBFEntity> type, Level worldIn) {
		super(type, worldIn);
	}

	public KK_TBFEntity(LivingEntity entityIn, Level world, ItemStack stack) {
		super(EntityTypes_CM.TBF.get(), entityIn, world, stack);
		this.projectile = stack.copy();
	}

	public KK_TBFEntity(Level wroldIn, double x, double y, double z, ItemStack stack) {
		super(EntityTypes_CM.TBF.get(), x, y, z, wroldIn, stack);
	}
	...
	protected void onHitEntity_KK(EntityHitResult result) {
		...
		if (!level().isClientSide() && result.getEntity() instanceof LivingEntity && result.getEntity() != thrower) {
			...
			else { this.blueKOUGEKI(result, 12.5F, 7.0F); }
		}
	}

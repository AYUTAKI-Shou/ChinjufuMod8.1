...
public class KB_TypeZeroEntity extends AbstractKK_Entity {

	private ItemStack projectile = new ItemStack(Items_Weapon.TYPEZERO.get());
	private double baseDamage = 10.0D; /** 63型(爆戦) 爆装+5 対空+5 **/
	
	public KB_TypeZeroEntity(EntityType<KB_TypeZeroEntity> type, Level worldIn) {
		super(type, worldIn);
	}

	public KB_TypeZeroEntity(LivingEntity entityIn, Level world, ItemStack stack) {
		super(EntityTypes_CM.TYPEZERO.get(), entityIn, world, stack);
		this.projectile = stack.copy();
	}

	public KB_TypeZeroEntity(Level wroldIn, double x, double y, double z, ItemStack stack) {
		super(EntityTypes_CM.TYPEZERO.get(), x, y, z, wroldIn, stack);
	}

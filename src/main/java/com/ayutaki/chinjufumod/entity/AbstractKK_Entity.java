...
public abstract class AbstractKK_Entity extends ThrowableProjectile implements ItemSupplier {
	...
	protected void dropAndKill() {
		...
		if (thrower != null && !playerIn.getAbilities().instabuild) { hStack.hurt(1, random, null); }
	...
	protected void redBAKUGEKI(EntityHitResult result) {
		...
		result.getEntity().hurt(DamageSource.thrown(this, this.getOwner()), (float)dealDamage * criKANBAKU);
	...
	protected void blueKOUGEKI(EntityHitResult result, float capA, float capB) {
		...
		if (this.isCarrier()) {
			...
			result.getEntity().hurt(DamageSource.thrown(this, this.getOwner()), (total > cap)? cap : total);

...
public class KK_MosquitoEntity extends AbstractKK_Entity {
	...
	private ItemStack projectile = new ItemStack(Items_Weapon.MOSQUITO.get());
	private double baseDamage = 8.0D; /** Mosquito TR Mk.33 雷装+8 **/
	
	public KK_MosquitoEntity(EntityType<KK_MosquitoEntity> type, Level worldIn) {
		super(type, worldIn);
	}

	public KK_MosquitoEntity(LivingEntity entityIn, Level world, ItemStack stack) {
		super(EntityTypes_CM.MOSQUITO.get(), entityIn, world, stack);
		this.projectile = stack.copy();
	}

	public KK_MosquitoEntity(Level wroldIn, double x, double y, double z, ItemStack stack) {
		super(EntityTypes_CM.MOSQUITO.get(), x, y, z, wroldIn, stack);
	}

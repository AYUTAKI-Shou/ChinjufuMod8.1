...
public abstract class AbstractAmmo_Entity extends Projectile {
	...
	protected AbstractAmmo_Entity(EntityType<? extends AbstractAmmo_Entity> type, Level worldIn, ItemStack stack) {
		this(type, worldIn);
		...
		if (unit != null) {
			this.pickup = AbstractAmmo_Entity.Pickup.CREATIVE_ONLY;
		}
	}

	protected AbstractAmmo_Entity(EntityType<? extends AbstractAmmo_Entity> type, double x, double y, double z, Level worldIn, ItemStack stack) {
		this(type, worldIn, stack);
		this.setPos(x, y, z);
	}

	protected AbstractAmmo_Entity(EntityType<? extends AbstractAmmo_Entity> type, LivingEntity shooter, Level worldIn, ItemStack stack) {
	...
	public void tick() {
		...
		if (this.inGround && !flag) {
			...
			if (this.isCritArrow()) {
				for (int i = 0; i < 4; i++) {
					this.level().addParticle((ParticleOptions) ParticleTypes_CM.AMMO_PT.get(), this.getX() + d5 * (double)i / 4.0, this.getY() + d6 * (double)i / 4.0, this.getZ() + d1 * (double)i / 4.0, -d5, -d6 + 0.2, -d1);
				}
			}
	...
	protected void onHitEntity(EntityHitResult result) {
		...
		int i = Mth.ceil(Mth.clamp((double)f * this.ammoDamage, 0.0, 2.147483647E9));
	...
	public void setOwner(@Nullable Entity entityIn) {
		...
		if (entityIn instanceof Player && this.pickup == AbstractAmmo_Entity.Pickup.DISALLOWED) {
			this.pickup = AbstractAmmo_Entity.Pickup.ALLOWED;
		}
	}

...
public abstract class AbstractAmmo_Entity extends Entity implements IProjectile {
	...
	private static final DataParameter<Byte> CRITICAL = EntityDataManager.<Byte>createKey(AbstractAmmo_Entity.class, DataSerializers.BYTE);
	protected int xTile;
	protected int yTile;
	protected int zTile;
	...
	public AbstractAmmo_Entity.PickupStatus pickupStatus;
	...
	public AbstractAmmo_Entity(World worldIn) {
		super(worldIn);
		this.xTile = -1;
		this.yTile = -1;
		this.zTile = -1;
		this.pickupStatus = AbstractAmmo_Entity.PickupStatus.DISALLOWED;
		
		/** Arrow = 2.0D, Craft Order and LUCK = 2.0D **/
		this.ammoDamage = 2.0D;
		this.setSize(0.5F, 0.5F);
	}

	public AbstractAmmo_Entity(World worldIn, double x, double y, double z) {
		this(worldIn);
		this.setPosition(x, y, z);
	}

	public AbstractAmmo_Entity(World worldIn, EntityLivingBase shooter) {
	...
	public void onUpdate() {
		...
		if (this.inGround) {
			...
			if (this.world.isRemote && this.getIsCritical()) {
				for (int k = 0; k < 4; ++k) {
					ChinjufuMod.PROXY.spawnParticle(ParticleTypes_CM.AMMO_PT, this.posX + this.motionX * (double)k / 4.0D, this.posY + this.motionY * (double)k / 4.0D, this.posZ + this.motionZ * (double)k / 4.0D, -this.motionX, -this.motionY + 0.2D, -this.motionZ);
				} // Displayed only on the client (this.world.isRemote). k < 4;
			}
	...
	public void onCollideWithPlayer(EntityPlayer entityIn) {
		...
		if (!this.world.isRemote && this.inGround && this.arrowShake <= 0) {
			boolean flag = this.pickupStatus == AbstractAmmo_Entity.PickupStatus.ALLOWED || this.pickupStatus == AbstractAmmo_Entity.PickupStatus.CREATIVE_ONLY && entityIn.capabilities.isCreativeMode;
	...
	public void setIsCritical(boolean critical) {
		...
		if (critical) { this.dataManager.set(CRITICAL, Byte.valueOf((byte)(b0 | 1))); }
		else { this.dataManager.set(CRITICAL, Byte.valueOf((byte)(b0 & -2))); }
	}

...
public abstract class AbstractAmmo_Entity extends AbstractArrow {
	...
	protected AbstractAmmo_Entity(EntityType<? extends AbstractArrow> type, Level worldIn) {
		super(type, worldIn);
	}

	protected AbstractAmmo_Entity(EntityType<? extends AbstractArrow> type, double x, double y, double z, Level worldIn) {
		this(type, worldIn);
		this.setPos(x, y, z);
	}

	protected AbstractAmmo_Entity(EntityType<? extends AbstractArrow> type, LivingEntity shooter, Level worldIn) {
	...
	public void tick() {
		...
		if (this.inGround && !flag) {
			...
			if (this.isCritArrow()) {
				for (int i = 0; i < 4; ++i) {
					this.level.addParticle((ParticleOptions) ParticleTypes_CM.AMMO_PT.get(), this.getX() + d5 * (double)i / 4.0D, this.getY() + d6 * (double)i / 4.0D, this.getZ() + d1 * (double)i / 4.0D, -d5, -d6 + 0.2D, -d1);
				}
			}
	...
	public void setOwner(@Nullable Entity entity) {
		...
		if (entity instanceof Player) {
			this.pickup = ((Player)entity).getAbilities().instabuild ? AbstractAmmo_Entity.Pickup.CREATIVE_ONLY : AbstractAmmo_Entity.Pickup.ALLOWED;
		}
	}

...
public class KB_SuiseiEntity extends AbstractKK_Entity {
	...
	private ItemStack projectile = new ItemStack(Items_Weapon.SUISEI.get());
	private double baseDamage = 8.0D; /** 彗星 爆装+8 **/
	
	public KB_SuiseiEntity(EntityType<KB_SuiseiEntity> type, Level worldIn) {
		super(type, worldIn);
	}

	public KB_SuiseiEntity(LivingEntity entityIn, Level world, ItemStack stack) {
		super(EntityTypes_CM.SUISEI.get(), entityIn, world, stack);
		this.projectile = stack.copy();
	}

	public KB_SuiseiEntity(Level wroldIn, double x, double y, double z, ItemStack stack) {
		super(EntityTypes_CM.SUISEI.get(), x, y, z, wroldIn, stack);
	}

...
public class Explosion_CM extends ServerExplosion {
	...
	public Explosion_CM(ServerLevel worldIn, @Nullable Entity entityIn, @Nullable DamageSource source, @Nullable ExplosionDamageCalculator context, Vec3 vec, float sizeIn, boolean fireIn, Explosion.BlockInteraction mode) {
			...
			if (!entity.ignoreExplosion(this)) {
				...
				if (d0 <= 1.0) {
					...
					if (d4 != 0.0) {
						...
						d2 *= d6;
						d3 *= d6;
						Vec3 vec3 = new Vec3(d1, d2 * 0.8, d3);
	...
	private void interactWithBlocks(List<BlockPos> object) {
		/** ブロックの破壊 **/
		List<Explosion_CM.StackCollector> list = new ArrayList<>();
		...
		for (Explosion_CM.StackCollector serverexplosion$stackcollector : list) {
	...
	private static void addOrAppendStack(List<Explosion_CM.StackCollector> stackCollectors, ItemStack stack, BlockPos pos) {
		for (Explosion_CM.StackCollector serverexplosion$stackcollector : stackCollectors) {
			serverexplosion$stackcollector.tryMerge(stack);
			if (stack.isEmpty()) { return; }
		}
		stackCollectors.add(new Explosion_CM.StackCollector(pos, stack));
	}

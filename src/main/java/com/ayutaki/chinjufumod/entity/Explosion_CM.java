	...
	public void finalizeExplosion(boolean spawnParticles) {
		...
		if (flag) {
			...
			for(BlockPos pos : this.toBlow) {
				...
				if (!state.isAir(this.level, pos)) {
					BlockPos pos1 = pos.immutable();
					this.level.getProfiler().push("explosion_blocks");
					if (state.canDropFromExplosion(this.level, pos, this) && this.level instanceof ServerWorld) {
						...
						state.getDrops(lootcontext$builder).forEach((p_229977_2_) -> {
							addBlockDrops(objectarraylist, p_229977_2_, pos1);
						});
					}
		...
		if (this.fire) {
			for(BlockPos pos2 : this.toBlow) {
				if (this.random.nextInt(3) == 0 && this.level.getBlockState(pos2).isAir() && this.level.getBlockState(pos2.below()).isSolidRender(this.level, pos2.below())) {
					this.level.setBlockAndUpdate(pos2, AbstractFireBlock.getState(this.level, pos2));
				}
			}
		}
	}

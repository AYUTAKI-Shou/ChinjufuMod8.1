...
public class KB_SBDEntity extends AbstractKK_Entity {

	private ItemStack projectile = new ItemStack(Items_Weapon.SBD.get());
	private double baseDamage = 7.0D; /** SBD 火力+1,爆装+6 **/
	
	public KB_SBDEntity(EntityType<KB_SBDEntity> type, Level worldIn) {
		super(type, worldIn);
	}

	public KB_SBDEntity(LivingEntity entityIn, Level world, ItemStack stack) {
		super(EntityTypes_CM.SBD.get(), entityIn, world, stack);
		this.projectile = stack.copy();
	}

	public KB_SBDEntity(Level wroldIn, double x, double y, double z, ItemStack stack) {
		super(EntityTypes_CM.SBD.get(), x, y, z, wroldIn, stack);
	}

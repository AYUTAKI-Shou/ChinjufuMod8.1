...
public class KK_BarracudaEntity extends AbstractKK_Entity {

	private ItemStack projectile = new ItemStack(Items_Weapon.BARRACUDA.get());
	private double baseDamage = 7.0D; /** Barracuda Mk.II 雷装+7 **/
	
	public KK_BarracudaEntity(EntityType<KK_BarracudaEntity> type, Level worldIn) {
		super(type, worldIn);
	}

	public KK_BarracudaEntity(LivingEntity entityIn, Level world, ItemStack stack) {
		super(EntityTypes_CM.BARRACUDA.get(), entityIn, world, stack);
		this.projectile = stack.copy();
	}

	public KK_BarracudaEntity(Level wroldIn, double x, double y, double z, ItemStack stack) {
		super(EntityTypes_CM.BARRACUDA.get(), x, y, z, wroldIn, stack);
	}
	...
	protected void onHitEntity_KK(EntityHitResult result) {
		...
		if (!level().isClientSide() && result.getEntity() instanceof LivingEntity && result.getEntity() != thrower) {
			...
			else { this.blueKOUGEKI(result, 10.5F, 5.0F); }
		}
	}

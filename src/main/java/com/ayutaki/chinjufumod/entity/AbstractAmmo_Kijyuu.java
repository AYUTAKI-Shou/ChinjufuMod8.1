...
public abstract class AbstractAmmo_Kijyuu extends Entity implements IProjectile {
	...
	private static final DataParameter<Byte> CRITICAL = EntityDataManager.createKey(AbstractAmmo_Kijyuu.class, DataSerializers.BYTE);
	protected static final DataParameter<Optional<UUID>> field_212362_a = EntityDataManager.createKey(AbstractAmmo_Kijyuu.class, DataSerializers.OPTIONAL_UNIQUE_ID);
	private static final DataParameter<Byte> PIERCE_LEVEL = EntityDataManager.createKey(AbstractAmmo_Kijyuu.class, DataSerializers.BYTE);
	@Nullable
	private BlockState inBlockState;
	protected boolean inGround;
	protected int timeInGround;
	public AbstractAmmo_Kijyuu.PickupStatus pickupStatus = AbstractAmmo_Kijyuu.PickupStatus.DISALLOWED;
	...
	protected AbstractAmmo_Kijyuu(EntityType<? extends AbstractAmmo_Kijyuu> type, World worldIn) {
		super(type, worldIn);
	}

	protected AbstractAmmo_Kijyuu(EntityType<? extends AbstractAmmo_Kijyuu> type, double x, double y, double z, World worldIn) {
		this(type, worldIn);
		this.setPosition(x, y, z);
	}

	protected AbstractAmmo_Kijyuu(EntityType<? extends AbstractAmmo_Kijyuu> type, LivingEntity shooter, World worldIn) {
	...
	protected void onHit(RayTraceResult raytraceResultIn) {
		...
		if (raytraceresult$type == RayTraceResult.Type.ENTITY) {
			...
			this.setHitSound(SoundEvents_CM.AM_HIT);
	...
	public void setShooter(@Nullable Entity entityIn) {
		...
		if (entityIn instanceof PlayerEntity) {
			this.pickupStatus = ((PlayerEntity)entityIn).abilities.isCreativeMode ? AbstractAmmo_Kijyuu.PickupStatus.CREATIVE_ONLY : AbstractAmmo_Kijyuu.PickupStatus.ALLOWED;
		}
	}
	...
	public void onCollideWithPlayer(PlayerEntity entityIn) {
		if (!this.world.isRemote && (this.inGround || this.getNoClip()) && this.arrowShake <= 0) {
			boolean flag = this.pickupStatus == AbstractAmmo_Kijyuu.PickupStatus.ALLOWED || this.pickupStatus == AbstractAmmo_Kijyuu.PickupStatus.CREATIVE_ONLY && entityIn.abilities.isCreativeMode || this.getNoClip() && this.getShooter().getUniqueID() == entityIn.getUniqueID();
	...
	public void handleStatusUpdate(byte id) {
		if (id == 3) {
			this.world.addParticle(ParticleTypes_CM.MARK_PT, this.getPosX(), this.getPosY(), this.getPosZ(), 0.0D, 0.0D, 0.0D); }
	}
}

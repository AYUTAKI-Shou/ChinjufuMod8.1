...
public class RenderAmmo_Large<T extends AbstractAmmo_Entity> extends EntityRenderer<T> {
	...
	public RenderAmmo_Large(EntityRendererProvider.Context renderManager) {
		super(renderManager);
	}
	...
	public void vertex(Matrix4f matrix, Matrix3f normals, VertexConsumer builder, int offsetX, int offsetY, int offsetZ, float textureX, float textureY, int p_113834_, int p_113835_, int p_113836_, int p_113837_) {
		builder.vertex(matrix, (float)offsetX, (float)offsetY, (float)offsetZ).color(255, 255, 255, 255).uv(textureX, textureY).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(p_113837_).normal((float)p_113834_, (float)p_113836_, (float)p_113835_).endVertex();
	}
}

...
public class KK_SwordfishEntity extends AbstractKK_Entity {

	private ItemStack projectile = new ItemStack(Items_Weapon.SWORDFISH.get());
	private double baseDamage = 5.0D; /** Swordfish 火力 +2, 雷装 +3 **/
	
	public KK_SwordfishEntity(EntityType<KK_SwordfishEntity> type, Level worldIn) {
		super(type, worldIn);
	}

	public KK_SwordfishEntity(LivingEntity entityIn, Level world, ItemStack stack) {
		super(EntityTypes_CM.SWORDFISH.get(), entityIn, world, stack);
		this.projectile = stack.copy();
	}

	public KK_SwordfishEntity(Level wroldIn, double x, double y, double z, ItemStack stack) {
		super(EntityTypes_CM.SWORDFISH.get(), x, y, z, wroldIn, stack);
	}

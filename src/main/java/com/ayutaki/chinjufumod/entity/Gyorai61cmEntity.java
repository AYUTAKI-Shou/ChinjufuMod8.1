...
public class Gyorai61cmEntity extends EntityThrowable {
	...
	protected void onImpact(@Nonnull RayTraceResult result) {
		...
		case ENTITY: {
			if (!world.isRemote && result.entityHit != null && result.entityHit instanceof EntityLivingBase && result.entityHit != getThrower()) {
				...
				float criticalA = (this.rand.nextInt(4) == 0)? 1.5F : 1.0F;
				float criticalB = (this.rand.nextInt(8) == 0)? 1.25F : 0.9F;
				float criGYORAI = this.isSuirai()? criticalA : criticalB;
				
				result.entityHit.attackEntityFrom(DamageSource.causeThrownDamage(this, this.getThrower()), (float)dealDamage * criGYORAI);
	...
	public void hitProcess() {
		if (!this.world.isRemote) {
			...
			Explosion_CM explosion = new Explosion_CM(this.world, this, this.posX, this.posY, this.posZ, size, false, false);

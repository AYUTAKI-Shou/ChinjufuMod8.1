...
public class AmmoEntity_Medium extends AbstractAmmo_Entity {
	...
	public AmmoEntity_Medium(Level worldIn, double x, double y, double z, ItemStack stack, @Nullable ItemStack stackS) {
		super(EntityTypes_CM.AMMO_M.get(), x, y, z, worldIn, stack, stackS);
	}

	public AmmoEntity_Medium(Level worldIn, LivingEntity shooter, ItemStack stack, @Nullable ItemStack stackS) {
		super(EntityTypes_CM.AMMO_M.get(), shooter, worldIn, stack, stackS);
	}
	...
	public void hitProcess() {
		if (!this.level().isClientSide()) {
			...
			this.createExplosion(this, this.position(), 1.25F, false, (blast == true)? Explosion.BlockInteraction.DESTROY : Explosion.BlockInteraction.KEEP); }

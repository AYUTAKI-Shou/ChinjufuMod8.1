...
public class Weak_Explosion extends ServerExplosion {
	...
	public Weak_Explosion(ServerLevel worldIn, @Nullable Entity entityIn, @Nullable DamageSource source, @Nullable ExplosionDamageCalculator context, Vec3 vec, float sizeIn, boolean fireIn, Explosion.BlockInteraction mode) {
		super(worldIn, entityIn, source, context, vec, sizeIn, fireIn, mode); //must
		this.level = worldIn;
	...
	private List<BlockPos> calculateExplodedPositions() {
		...
		/* ブロックの破壊に関する設定 Does the first part of the explosion (destroy blocks) */
		for (int j = 0; j < 16; j++) {
			for (int k = 0; k < 16; k++) {
				for (int l = 0; l < 16; l++) {
			...
			if (!entity.ignoreExplosion(this)) {
				...
				if (d0 <= 1.0) {
					...
					if (d4 != 0.0) {
						...
						if (flag) {
							entity.hurtServer(this.level, this.damageSource, 1.0F); //Weak
						}
						...
						d2 *= d6;
						d3 *= d6;
						Vec3 vec3 = new Vec3(d1 * 2.0, d2 * 0.5, d3 * 2.0);
	...
	/* explode() ---> */
	private void interactWithBlocks(List<BlockPos> object) {
		/** ブロックの破壊 **/
		List<Weak_Explosion.StackCollector> list = new ArrayList<>();
		Util.shuffle(object, this.level.random);

		for (BlockPos blockpos : object) {
			this.level.getBlockState(blockpos)
				.onExplosionHit(this.level, blockpos, this, (eStack, pos) -> addOrAppendStack(list, eStack, pos));
		}

		for (Weak_Explosion.StackCollector serverexplosion$stackcollector : list) {
			Block.popResource(this.level, serverexplosion$stackcollector.pos, serverexplosion$stackcollector.stack);
		}
	}

	/* explode() ---> */
	private void createFire(List<BlockPos> object) {
		for (BlockPos blockpos : object) {
	...
	private static void addOrAppendStack(List<Weak_Explosion.StackCollector> stackCollectors, ItemStack stack, BlockPos pos) {
		for (Weak_Explosion.StackCollector serverexplosion$stackcollector : stackCollectors) {
			serverexplosion$stackcollector.tryMerge(stack);
			if (stack.isEmpty()) { return; }
		}
		stackCollectors.add(new Weak_Explosion.StackCollector(pos, stack));
	}
	...
	public boolean canTriggerBlocks() {
		if (this.blockInteraction != Explosion.BlockInteraction.TRIGGER_BLOCK) {
			...
			return this.source != null && this.source.getType() == EntityType.BREEZE_WIND_CHARGE ? this.level.getGameRules()
						.getBoolean(GameRules.RULE_MOBGRIEFING) : true; }
	}
	...
	static class StackCollector {
		...
		StackCollector(BlockPos pos, ItemStack eStack) {
			this.pos = pos;
			this.stack = eStack; }

		public void tryMerge(ItemStack eStack) {
			if (ItemEntity.areMergable(this.stack, eStack)) {
				this.stack = ItemEntity.merge(this.stack, eStack, 16); }
		}
	}
}

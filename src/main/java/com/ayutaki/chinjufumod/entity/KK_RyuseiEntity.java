...
public class KK_RyuseiEntity extends AbstractKK_Entity {

	private ItemStack projectile = new ItemStack(Items_Weapon.RYUSEI.get());
	private double baseDamage = 10.0D; /** 流星 雷装+10 **/
	
	public KK_RyuseiEntity(EntityType<KK_RyuseiEntity> type, Level worldIn) {
		super(type, worldIn);
	}

	public KK_RyuseiEntity(LivingEntity entityIn, Level world, ItemStack stack) {
		super(EntityTypes_CM.RYUSEI.get(), entityIn, world, stack);
		this.projectile = stack.copy();
	}

	public KK_RyuseiEntity(Level wroldIn, double x, double y, double z, ItemStack stack) {
		super(EntityTypes_CM.RYUSEI.get(), x, y, z, wroldIn, stack);
	}
	...
	protected void onHitEntity_KK(EntityHitResult result) {
		...
		if (!level().isClientSide() && result.getEntity() instanceof LivingEntity && result.getEntity() != thrower) {
			...
			else { this.blueKOUGEKI(result, 12.0F, 6.5F); }
		}
	}

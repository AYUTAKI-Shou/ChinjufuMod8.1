	...
	public static void generate(final BiomeLoadingEvent event) {
		...
		if (Config_CM.INSTANCE.bauxiteGene.get() == true) {
			if (category != Biome.BiomeCategory.NETHER && category != Biome.BiomeCategory.THEEND) {
				builder.addFeature(GenerationStep.Decoration.UNDERGROUND_ORES, Placement_CM.BAUXITEORE_PLACE.getHolder().orElseThrow());
				builder.addFeature(GenerationStep.Decoration.UNDERGROUND_ORES, Placement_CM.BAUXITEORE_DEEPPLACE.getHolder().orElseThrow()); }
		}
